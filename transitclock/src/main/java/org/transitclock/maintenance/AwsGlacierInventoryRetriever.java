...
/**
 * For retrieving an AWS Glacier inventory file for a vault. The inventory file shows all of the
 * entries in the inventory along with the archiveId. This archiveId can be used to upload the data
 * stored in glacier.
 *
 * @author SkiBu Smith
 */
public class AwsGlacierInventoryRetriever {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AwsGlacierInventoryRetriever.class);
  ...
  public AwsGlacierInventoryRetriever(String region) {
    ...
    logger.debug(
        "Read in credentials AWSAccessKeyId={} AWSSecretKey={}...",
  ...
  /**
   * For retrieving vault inventory. For initializing SQS for determining when job completed. Does
   * nothing if member snsTopicName is null. Sets members sqsQueueURL, sqsQueueARN, and sqsClient.
   */
  private void setupSQS() {
    // If no sqsQueueName setup then simply return
    if (sqsQueueName == null) return;
  ...
  /**
   * For retrieving vault inventory. For initializing SNS for determining when job completed. Does
   * nothing if member snsTopicName is null. Sets members snsTopicARN and snsSubscriptionARN.
   */
  void setupSNS() {
    // If no snsTopicName setup then simply return
    if (snsTopicName == null) return;
   ...
   * @throws IOException
   */
  private void downloadJobOutput(String vaultName, String jobId, String outputFileName)
      throws IOException {
    logger.info(
        "Downloading job output from vaultName={} into " + "outputFileName={} for jobId={}",
        vaultName,
        outputFileName,
        jobId);
  ...
  /** To be called when done with this object. Cleans up notifications for when jobs complete. */
   ...
   *
   * @param vaultName Name of the AWS vault
   * @param outputFileName Name of inventory json file to store data in
   */
  public void getVaultInventory(String vaultName, String outputFileName) {
    logger.info(
        "Getting vault inventory for vaultName={} and storing " + "it into file {}",
        vaultName,
        outputFileName);
    ...
    try {
      ...
      if (!success) {
        logger.error(
            "Job did not complete successfully for " + "vaultName={} jobId=", vaultName, jobId);
        return;
      }
      ...
      logger.info(
          "Successfully downloaded vault inventory for "
      ...
      logger.error(
          "Exception getting vault inventory for vaultName={}" + "jobId=", vaultName, jobId);
    }
  }

  /**
   * Gets the inventory file for the specified vault and writes it to a file in the output
   * directory.
   *
   * @param args args[0] specifies vaultName. args[1] specifies the directory where the output file
   *     should be written.
   */
