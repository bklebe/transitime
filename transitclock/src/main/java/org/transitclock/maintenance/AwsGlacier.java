...
public class AwsGlacier {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AwsGlacier.class);
  ...
  public String upload(String vaultName, String fileName, String archiveDescription) {
    ...
    try {
      ...
      try {
        ...
        logger.error(
            "Exception occurred when trying to create " + "vaultName=\"{}\"", vaultName, e2);
        return null;
      }
    } catch (AmazonClientException | FileNotFoundException e) {
      logger.error(
          "Exception occurred when uploading file \"{}\" to " + "AWS Glacier. {}",
          fileName,
          e.getMessage(),
          e);
      return null;
  ...
  /**
   * Downloads the file with the specified archiveId from the specified vaultName. Note that this
   ...
   */
  public void download(String vaultName, String archiveId, String outputFileName) {
    logger.info(
        "Downloading to file=\"{}\" from vaultName=\"{}\" " + "archiveId=\"{}\"",
        outputFileName,
        vaultName,
        archiveId);
    ...
    try {
      atm.download(accountId, vaultName, archiveId, outputFile, progressListener);

      logger.info(
          "Successfully downloaded in {} minutes from " + "vaultName=\"{}\" archiveId=\"{}\"",
          timer.elapsedMsec() / Time.MS_PER_MIN,
          vaultName,
          archiveId);
    } catch (AmazonClientException e) {
      logger.error(
          "Exception when downloading from vaultName=\"{}\" " + "archiveId=\"{}\". {}",
          vaultName,
          archiveId,
          e.getMessage());
    }
  ...
  public static void main(String[] args) {
    ...
    String archiveId =
        "KcIOX6mmr0A1h4WQpeu7Vq4EbmO1Xp7vZJZZ5iAEEcYNY2KYxdZH3mIHATKIbdiVg0Nu6Od9FLScOpgdjrMUsaXe7pyI77MNBz5wd_hqaYhQ81DG_f5v5n6FSSFF4JJg90YT5khmrQ";
