...
/**
 * A module that runs in a separate thread that archives old log files as needed.
 *
 * @author SkiBu Smith
 */
public class ArchiveOldFilesModule extends Module {
  ...
  private static StringConfigValue timeToArchive =
      new StringConfigValue(
          "transitclock.maintenance.timeToArchive",
          "01:00:00", // 1 am
  ...
  private static StringConfigValue awsRegion =
      new StringConfigValue(
          "transitclock.maintenance.awsRegion",
          AwsGlacier.OREGON_REGION,
          "The region for Amazon AWS where files should be archive " + "to.");

  private static StringConfigValue logDirForInventoryFile =
      new StringConfigValue(
          "transitclock.maintenance.logDirForInventoryFile",
  ...
  private static StringConfigValue logFileBaseDir =
      new StringConfigValue(
          "transitclock.maintenance.logFileBaseDir",
  ...
  private static StringConfigValue awsVaultName =
      new StringConfigValue(
          "transitclock.maintenance.awsVaultName",
  ...
  private static StringConfigValue awsVaultName2 =
      new StringConfigValue(
          "transitclock.maintenance.awsVaultName2",
          null,
  ...
  private static StringConfigValue logFileSubDirectory =
      new StringConfigValue(
          "transitclock.maintenance.logFileSubDir",
  ...
  private static StringConfigValue logFileSubDirectory2 =
      new StringConfigValue(
          "transitclock.maintenance.logFileSubDir2",
          null,
          "For when making two separate archives. Can use two "
              + "archives if some files should be archived sooner "
              + "that others. Null means not using second archive.");
  ...
  private static final Logger logger = LoggerFactory.getLogger(ArchiveOldFilesModule.class);
  ...
  private void sleepTillAppropriateTime() {
    ...
    int nowSecsIntoDay = Core.getInstance().getTime().getMsecsIntoDay(new Date()) / Time.MS_PER_SEC;
    int secsToSleep = secondsIntoDay - nowSecsIntoDay;
    if (nowSecsIntoDay > secondsIntoDay) secsToSleep += 1 * Time.SEC_PER_DAY;
  ...
  private void archiveLogFiles() {
    logger.info(
        "ArchiveOldFilesModule reached appropriate time " + "of {} so archiving log files.",
        timeToArchive.getValue());

    // Create main archiver that can write the files to Amazon AWS Glacier
    AwsGlacierArchiver archiver =
        new AwsGlacierArchiver(
            awsRegion.getValue(), awsVaultName.getValue(), logDirForInventoryFile.getValue());

    // Archive the files in the specified directory
    ArchiveOldFiles.archive(
        archiver,
        logFileBaseDir.getValue(),
    ...
    if (awsVaultName2.getValue() != null) {
      ...
      // Amazon AWS Glacier
      AwsGlacierArchiver archiver2 =
          new AwsGlacierArchiver(
              awsRegion.getValue(), awsVaultName2.getValue(), logDirForInventoryFile.getValue());
