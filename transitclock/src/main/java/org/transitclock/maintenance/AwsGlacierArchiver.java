...
public class AwsGlacierArchiver implements ArchiverInterface {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AwsGlacierArchiver.class);
   ...
   *
   * @param region Region of the AWS Glacier vault
   * @param vaultName Name of the AWS Glacier vault
   * @param logDirectory Directory where log of uploads is written for the vault.
   */
  public AwsGlacierArchiver(String region, String vaultName, String logDirectory) {
  ...
  /**
   * Adds record of upload to the log file for the vault. This way don't need to read the vault
   * contents from Glacier, which is a nuisance since it takes hours.
   *
   * @param description
   * @param archiveId
   */
  private void addToArchiveLog(String description, String archiveId) {
    ...
    logger.info(
        "Logging for vaultName \"{}\" the description \"{}\" and "
    ...
    try {
      ...
      logger.error(
          "Exception occurred when adding description=\"{}\" "
  ...
  public String upload(String fileName, String description) {
    logger.info(
        "Uploading to Amazon AWS Glacier vaultName=\"{}\" the "
    ...
    if (archiveId != null) {
      ...
      logger.error(
          "Error occurred uploading to vaultName=\"{}\" the "
   ...
   *
   * @param args The command line arguments are fileName, description, region, vaultName,
   *     logDirectory.
   */
  public static void main(String[] args) {
    ...
    AwsGlacierArchiver archiver = new AwsGlacierArchiver(region, vaultName, logDirectory);
    String archiveId = archiver.upload(fileName, description);
    if (archiveId == null) System.exit(-1);
  }
}
