...
public class ArchiveOldFiles extends OldFileFinder {
   ...
   * @param subDirectory The subdirectory beyond the base directory. Specifies where to find the log
   *     files to be archived. This part of the file names is included in the file descriptions in
   *     the zip file.
   * @param daysOld If files older than daysOld found in a directory then that directory is
   *     archived.
   */
  private ArchiveOldFiles(
      ArchiverInterface archiver, String baseDirectory, String subDirectory, int daysOld) {
    super(daysOld);
  ...
  /**
   * Called for each file that is older than specified number of days. Compresses file if necessary.
   *
   * <p>(non-Javadoc)
   *
  ...
  /**
   * Recursively deletes all the files in the directory. This of course should only be called if the
  ...
  /**
   * Zips up all files in directory, archives them, and then deletes all files in directory. Called
   * after directory with old file has been processed.
   *
   * <p>(non-Javadoc)
   *
  ...
  /**
   * Called after directory with old file has been processed. If directory is empty then deletes it.
   * This way the higher level directories, such as the year and month ones, are automatically
