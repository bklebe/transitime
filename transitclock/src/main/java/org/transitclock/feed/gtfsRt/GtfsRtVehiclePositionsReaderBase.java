...
public abstract class GtfsRtVehiclePositionsReaderBase {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(GtfsRtVehiclePositionsReaderBase.class);
  ...
  /**
   * Returns the vehicleID. Returns null if no VehicleDescription associated with the vehicle or if
   * no ID associated with the VehicleDescription.
   *
   * @param vehicle
  ...
  public void process() {
    try {
      ...
      logger.info(
          "Parsing GTFS-realtime file into a FeedMessage took " + "{} msec", timer.elapsedMsec());
