...
public class VehicleState {
  ...
  /**
   * Sets the block assignment for vehicle. Also, this is how it is specified whether a vehicle is
   * predictable or not.
   *
   * @param newBlock The current block assignment for the vehicle. Set to null if vehicle not
   *     assigned.
   * @param assignmentMethod How vehicle was assigned (AVL feed, auto assigner, etc). Set to null if
   *     vehicle not assigned.
   * @param assignmentId Can be blockId, tripId, or tripShortName. Depends on type of assignment
  ...
  /**
   * Sets the block for this VehicleState to null. Also sets assignmentId to null and predictable to
   * false.
   *
   * @param assignmentMethod How vehicle was assigned (AVL feed, auto assigner, etc). Set to null if
   *     vehicle not assigned.
  ...
  /**
   * Determines if vehicle is currently getting assigned and it is getting assigned back to the same
   * block it was assigned to just a while ago. In other words this tells if vehicle might have
   * become unpredictable but then is getting reassigned again. In this kind of situation don't want
   * to for example determine arrivals/departures back to the beginning of the block because
   * probably already did so when vehicle was previously assigned.
   *
  ...
  /**
   * Sets the match for the vehicle into the history. If set to null then VehicleState.predictable
   * is set to false. Also resets numberOfBadMatches to 0.
   *
   * @param match
   */
  ...
  /**
   * Goes through the match history for the vehicle and returns match that is at least minimumAge
   * old. If there isn't a match that old then returns null.
   *
   * @param minimumAgeMsec The minimum age in msec of the match to be returned
   * @return TemporalMatch for vehicle that is at least minimumAge old, or null if there isn't such
   *     a match.
   */
  ...
  /**
   * To be called when predictable vehicle has no valid spatial/temporal match. Only allowed so many
   * of these before vehicle is made unpredictable.
   */
  ...
  /**
   * Returns if have exceeded the number of allowed bad matches. If so then vehicle should be made
   * unpredictable.
   *
   * @return
   */
  ...
  /**
   * Returns the number of sequential bad spatial/temporal matches that occurred while vehicle was
   * predictable.
   *
  ...
  /**
   * Returns true if the last AVL report was successfully matched to the assignment indicating that
   * can generate predictions and arrival/departure times etc.
   *
  ...
  /**
   * Stores the specified avlReport into the history for the vehicle. Makes sure that the AVL
   * history doesn't exceed maximum size.
   *
   * @param avlReport
   */
  ...
  public void putTripStartTime(Integer tripCounter, Long date) {
    ...
    if (this.tripStartTimesMap.get(tripCounter) == null && date > 0) {
      logger.debug(
          "Setting start time for vehicle {} for trip counter {} to {}.",
          this.getVehicleId(),
          tripCounter,
          new Date(date));
  ...
  /**
   * Increment trip counter is event it is arrival first stop on trip. Used for frequency based
   * services.
   *
   * @param event
   */
  public void incrementTripCounter(ArrivalDeparture event) {
    VehicleState vehicleState =
        VehicleStateManager.getInstance().getVehicleState(event.getVehicleId());

    if (event.getStopPathIndex() == 0) {
      if (event.isArrival()) {
        vehicleState.incrementTripCounter();
        logger.debug(
            "Setting vehicle counter to : {} because of event : {}",
            vehicleState.getTripCounter(),
            event);
      } else {
  ...
  /**
   * Returns the current Trip for the vehicle. Returns null if there is not current trip.
   *
   * @return Trip or null.
   */
  public Trip getTrip() {
  ...
  /**
   * Returns the current route ID for the vehicle. Returns null if not currently associated with a
   * trip.
   *
  ...
  /**
   * Returns the current GTFS route_short_name for the vehicle. Returns null if not currently
  ...
  /**
   * Returns the current name of the route that the vehicle is on. The route name is a combination
   * of the GTFS route_short_name and the GTFS route_long_name so get a full name of something like
   * "38 - Geary".
   *
  ...
  /**
   * Returns true if last temporal match for vehicle indicates that it is at a layover. A layover
   * stop is a wait stop where a vehicle can leave route path before departing this stop at the
   * scheduled time since the driver is taking a break.
   *
  ...
  /**
   * Returns true if last temporal match for vehicle indicates that it is at a wait stop. A wait
   * stop is when a vehicle is supposed to depart at a scheduled time. A layover is always a wait
   * stop but you can have a wait stop that is not a layover due to the vehicle not being allowed to
   * go away from the stop.
   *
  ...
  /**
   * Returns the next to last temporal match. Returns null if there isn't one. Useful for when need
   * to compare the previous to last match with the last one, such as for determining if vehicle has
   * crossed any stops.
   *
   * @return
   */
  ...
  /**
   * Looks in the AvlReport history for the most recent AvlReport that is at least
   * minDistanceFromCurrentReport from the current AvlReport. Also makes sure that previous report
   * isn't too old (more than 20 minutes old).
   *
   * @param minDistanceFromCurrentReport Distance in meters
   * @return The previous AvlReport, or null if there isn't one that far away within the last 20
   *     minutes.
   */
  ...
  /**
   * Goes through the AVL history for the vehicle and returns AVL report that is at least minimumAge
   * old. If there isn't a match that old then returns null.
   *
   * @param minimumAgeMsec The minimum age in msec of the AVL report to be returned
   * @return AvlReport for vehicle that is at least minimumAge old, or null if there isn't an old
   *     enough AVL report in the history.
   */
  ...
  /**
   * Returns the next to last AvlReport where successfully matched the vehicle. This isn't
   * necessarily simply the previous AvlReport since that report might not have been successfully
   * matched. It is important to use the proper AvlReport when matching a vehicle or such because
   * otherwise the elapsed time between the last successful match and the current match would be
   * wrong.
   *
  ...
  /**
   * Returns true if the AVL report has a different assignment than what is in the VehicleState. If
   * the avlReport does not have an assignment then false is returned such that the old assignment
   * continues to be used. If a block assignment then simply checks the new block ID from the AVL
   * report to the existing block ID. If using trip assignment in AVL feed then the trip is
   * converted to a block assignment for doing the comparison. When using a route assignment then
   * makes sure the route ID has not changed. For when reassigning a vehicle via the AVL feed.
   *
  ...
  public boolean hasNewAssignment(AvlReport avlReport) {
    ...
    // actually happen
    logger.error("Could not determine if vehicle has new assignment. {}", avlReport);
    return false;
  }

  /**
   * Returns true if previously the vehicle had the same assignment but that assignment was recently
   * removed due to a problem where the vehicle shouldn't be assigned to that assignment again. A
   * specific example is that this happens if an exclusive block assignment is grabbed by another
   * vehicle. Even though the original vehicle with the assignment might continue to get that
   * assignment via the AVL feed don't want to reassign it to the problem assignment again.
   *
   * <p>BUT WHAT ABOUT A VEHICLE SIMPLY BECOMING UNPREDICTABLE BECAUSE IT TEMPORARILY WENT OFF ROUTE
   * FOR 3 AVL REPORTS?? IN THAT CASE WANT VEHICLE TO MATCH ASSIGNMENT AGAIN. OR WHAT IF BLOCK
   * SIMPLY ENDED?? SEEMS THAT NEED TO REMEMBER IF VEHICLE WAS UNASSIGNED IN SUCH A WAY THAT
   * SHOULDN'T TAKE THAT ASSIGNMENT AGAIN FOR A WHILE.
   *
   * <p>An old assignment is considered recent if the unassignment happened within the last 2 hours.
   *
   * @param avlReport
   ...
   * the list will be not greater than MATCH_HISTORY_SIZE.
   *
  ...
  /**
   * The current block assignment. But will be null if vehicle not currently assigned.
   *
   * @return
   */
  ...
  /**
   * Can be the blockId, tripId, or tripShortName depending on the type of assignment received from
   * the AVL feed.
   *
  ...
  /**
   * Indicates how the vehicle was assigned (via block assignment, route assignment, auto
   * assignment, etc).
   *
   * @return
   */
  ...
  /**
   * Returns true if not a real vehicle but instead was created to produce schedule based
   * predictions.
   *
  ...
  /**
   * Records the specified arrival as one that still needs to be stored to the db. This is important
   * because can generate arrivals into the future but need to make sure that the arrival is before
   * the subsequent departure and can't do so until get additional AVL reports.
   *
   * @param arrival
   */
  ...
  /**
   * Sets the current list of predictions for the vehicle to the predictions parameter.
   *
   * @param predictions
   */
  ...
  /**
   * Returns the current real-time schedule adherence for the vehicle, or null if schedule adherence
   * not currently valid (vehicle is not predictable or running a non-schedule based assignment).
   *
   * @return The TemporalDifference representing schedule adherence, or null if vehicle not
  ...
  /**
   * Determines the heading of the vector that defines the stop path segment that the vehicle is
   * currently on. The heading will be between 0.0 and 360.0 degrees.
   *
   * @return Heading of vehicle according to path segment. NaN if not currently matched or there is
   *     no heading for that segment.
   */
  public float getPathHeading() {
    // If vehicle not currently matched then there is no path heading
    SpatialMatch match = getMatch();
    if (match == null) return Float.NaN;
    ...
    if (stopPath.isLayoverStop()) return Float.NaN;

    // Vehicle on non-layover path so return heading of that path.
    VectorWithHeading vector = stopPath.getSegmentVector(match.getSegmentIndex());
    return vector.getHeading();
  }

  /**
   * For when trying to get heading but vehicle is at a layover. Can't use heading of layover
   * because vehicle might be deadheading and therefore heading in a different direction of the
   * layover segment. Plus the layover segment is usually just a stub segment that might not have
   * valid direction at all. This method can be used to get the heading of the next path segment,
   * which should be valid. But only uses next path heading if vehicle is actually within 200m of
   * the layover. Otherwise the vehicle is deadheading and don't want to use the next path heading
  ...
  private float getNextPathHeadingIfAtLayover() {
    ...
    if (!match.isLayover()) return Float.NaN;
    ...
    // return NaN.
    double distanceToLayoverStop =
        match.getStopPath().getEndOfPathLocation().distance(getAvlReport().getLocation());
    if (distanceToLayoverStop > 200.0) return Float.NaN;
    ...
    if (getTrip().getNumberStopPaths() <= match.getStopPathIndex() + 1) return Float.NaN;
  ...
  /**
   * Looks in avlReportHistory and returns last valid heading. Will still return null in certain
   * situations such as there not being a history, the AVL report is old, never have valid heading
   * info, etc.
   *
   * <p>Reports are considered too old if they are more than 2 minutes old. This is useful
   * distinction because if report is too old then don't really know if the heading is valid. At
   * layovers it is likely that a vehicle turned a corner a bit before the layover point and don't
   * want to show the old heading before the turn.
   *
   * @return
   */
  ...
  /**
   * Normally uses the heading from getPathHeading(). But if that returns NaN then uses recent valid
   * heading from last AVL report, though that might be NaN as well. This can be better then always
   * using heading from AVL report since that often won't line up with the path and can make
   * vehicles be oriented in noticeably peculiar ways when drawn on an map.
   *
  ...
  public float getHeading() {
    ...
    if (!Float.isNaN(heading)) return heading;
  ...
  public String toString() {
    return "VehicleState ["
        ...
        + ", pathHeading="
        + StringUtils.twoDigitFormat(getHeading())
        + ", getMatch()="
        + getMatch()
        + ", getAvlReport()="
        + getAvlReport()
  ...
  /**
   * Stores the last arrival time so that can make sure that departure times are after the arrival
   * times.
   *
   * @param arrivalTime
   */
  ...
  /**
   * Returns the last stored arrival time so can make sure that departure times are after the
   * arrival times.
   *
   * @return
   */
