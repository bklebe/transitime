 ...
 * stops) that are read from db and processes the data into average travel and dwell times and
 * stores the data into maps. The travel times can then be written to the database.
 *
 * <p>The data is processed on a per tripId basis. With GTFS trips are unique across service IDs and
 * days of the week so this is adequate. But at some point really might want to divide up the data
 * by day of the week in order to get greater accuracy (assuming that buses might consistently
 * travel differently on Monday compared to Friday even though they have the same service ID.
 *
 * @author SkiBu Smith
 */
public class TravelTimesProcessor {
  ...
  private static final double STD_DEV_BIAS_FOR_FIRST_STOP = 1.5;
  ...
  private static final int STOP_TIME_BIAS_FOR_FIRST_STOP = 10000;
  ...
  private static final int MAX_SCHED_ADH_FOR_FIRST_STOP_TIME = 10 * Time.MS_PER_MIN;
  ...
  // of the average.
  private static DoubleConfigValue fractionLimitForStopTimes =
  ...
  // outliers if they are less than 0.7 or greater than 1/0.7
  // of the average.
  private static DoubleConfigValue fractionLimitForTravelTimes =
      new DoubleConfigValue(
  ...
  /**
   * Special MapKey class so that can make sure using the proper one for the associated maps in this
   * class. The key is made up of the tripId, the stopPathIndex, and the stopId. The stopId is
   * included so that can see if the historic data stopPathIndex and stopId match for the currently
   * configured trip. This is important for making sure that don't use historic data for a
   * stopPathIndex when stops have been removed from or added to the trip.
   */
  public static class ProcessedDataMapKey extends MapKey {
    public ProcessedDataMapKey(String tripId, int stopPathIndex, String stopId) {
  ...
  private static ProcessedDataMapKey getKey(String tripId, int stopPathIndex, String stopId) {
   ...
   */
  private static void addTravelTimesToMap(
      ProcessedDataMapKey mapKey, List<Integer> travelTimesForStopPath) {
  ...
  /**
   * For when the arrival/departure is for first stop of trip. If the schedule adherence isn't too
   * bad adds the stop time to the stop wait map.
   *
   * @param arrDep
   */
  private static void processFirstStopOfTrip(ArrivalDeparture arrDep) {
    ...
    int lateTimeMsec = (int) (arrDep.getTime() - arrDep.getScheduledTime());
  ...
  /**
   * Returns the matches for the particular stopPath for the service ID and trip.
   *
   * <p>Note: This is pretty inefficient. Seems that should be able to get list of matches for the
   * stopPath directly from the map instead of getting all the matches for the trip and then
   * filtering them.
   *
   * @param dataFetcher
   * @param arrDep
   ...
   */
  private static List<Match> getMatchesForStopPath(
      DataFetcher dataFetcher, ArrivalDeparture arrDep) {
    ...
    DbDataMapKey mapKey =
        dataFetcher.getKey(
            arrDep.getServiceId(), arrDep.getDate(), arrDep.getTripId(), arrDep.getVehicleId());
  ...
  /**
   * Internal structure for keeping track of matches that are between an departure and an arrival.
   * Needed for when there are multiple travel time segments within a stop path.
   */
  ...
  /**
   * Number of travel time segments for the stop path with the specified length.
   *
   * @param pathLength
  ...
  /**
   * The travel time length is the length of the stop path divided into equal segments such that the
   * segments are no longer than maxTravelTimeSegmentLength.
   *
   * @param trip For determining the StopPath
  ...
  /**
   * Gets the Matches that are associated with the stop path specified by arrDep2 (the path leading
   * up to that stop) for the same trip (makes sure it is for the same tripId for the same day of
   * the year). The returned matches will include the departure time from the first stop (arrDep1),
   * in between matches, and the arrival time as the second stop (arrDep2).
   *
   * @param dataFetcher
   * @param arrDep1 The departure stop
   * @param arrDep2 The arrival stop. Also defines which stop path working with.
  ...
  private static List<MatchPoint> getMatchPoints(
    ...
    List<Match> matchesForStopPath = getMatchesForStopPath(dataFetcher, arrDep2);
    ...
    matchPoints.add(
        new MatchPoint(
            arrDep2.getDate(), arrDep2.getStopPathLength(), MatchPoint.MatchPointReason.ARRIVAL));
  ...
  /**
   * Determines the travel times. If stop path is short enough such that it is only a single travel
   * time segment then returns single travel time set to the difference between the departure time
   * for arrDep1 and the arrival time for arrDep2. For when stop path is long enough such that there
   * are multiple travel time segments, for the particular stop path, as specified by the arrival
   * stop, looks up the associated Matches in order to determine how the vehicle travels along the
   * stop path. Determines when the travel time segment vertices are crossed and uses the vertices,
   * along with the departure time and the arrival times at the ends of the stop path, to determine
   * the travel time for each travel time segment for this particular trip.
   *
   * @param dataFetcher
   * @param arrDep1 The departure stop
   * @param arrDep2 The arrival stop. Also defines which stop path working with.
   * @return List of travel times in msec. There is a separate travel time for each travel time
   *     segment. If the match points are garbled and go backwards in time then null is returned.
   */
  private List<Integer> determineTravelTimesForStopPath(
    ...
    if (shouldResetEarlyTerminalDepartures()
      ...
      logger.debug(
          "Note: for {} using scheduled departure time instead "
    ...
    for (int i = 0; i < matchPoints.size() - 1; ++i) {
      ...
      if (pt2.getTime() < pt1.getTime()) {
        logger.error(
            "Encountered two match points that go backwards "
      ...
      int segIndex1 = (int) (pt1.distanceAlongStopPath / travelTimeSegmentLength);
      int segIndex2 = (int) ((pt2.distanceAlongStopPath - 0.0000001) / travelTimeSegmentLength);
      ...
      if (segIndex1 != segIndex2) {
        ...
        float distanceBtwnMatches = pt2.distanceAlongStopPath - pt1.distanceAlongStopPath;
        ...
        double distanceOfFirstVertex = (segIndex1 + 1) * travelTimeSegmentLength;
        double distanceToFirstVertex = distanceOfFirstVertex - pt1.distanceAlongStopPath;
        long crossingTimeForVertex = pt1.getTime() + (long) (distanceToFirstVertex / speed);
    ...
    for (int i = 0; i < vertexTimes.size() - 1; ++i) {
      ...
      if (segmentSpeedMps > maxSegmentSpeedMps.getValue()) {
        logger.error(
            ...
            arrDep2);
        segmentTime =
            (int) (travelTimeSegmentLength * Time.MS_PER_SEC / maxSegmentSpeedMps.getValue());
      }
  ...
  /**
   * For looking at travel time between two arrival/departures. It can be an arrival and then a
   * departure for the same stop or a departure for one stop and then an arrival for the subsequent
   * stop. If the schedule adherence is off too much (by MAX_SCHED_ADH_SECS) then the data is
   * ignored. If schedule adherence is acceptable then the resulting travel and stop/dwell times are
   * put into the stopTimesMap and travelTimesMap members for further processing.
   *
   * @param dataFetcher Contains the AVL based historic data in maps
   * @param arrDep1 The first arrival/departure
   * @param arrDep2 The second arrival/departure
   */
  private void processDataBetweenTwoArrivalDepartures(
    ...
    if (schedAdh == null) schedAdh = arrDep2.getScheduleAdherence();
    if (schedAdh != null && !schedAdh.isWithinBounds(MAX_SCHED_ADH_SECS, MAX_SCHED_ADH_SECS)) {
    ...
    ProcessedDataMapKey mapKeyForTravelTimes =
        getKey(arrDep2.getTripId(), arrDep2.getStopPathIndex(), arrDep2.getStopId());
    ...
    if (arrDep1.getStopPathIndex() == arrDep2.getStopPathIndex()
      ...
      // Add this stop time to map so it can be averaged
      if (dwellTimeMsec >= 0) addStopTimeToMap(mapKeyForTravelTimes, dwellTimeMsec);
      else
        logger.error(
            "Ignoring negative dwell time={} for stop path "
    ...
    if (arrDep1.getStopPathIndex() - arrDep2.getStopPathIndex() != 1
      ...
      List<Integer> travelTimesForStopPath =
          determineTravelTimesForStopPath(dataFetcher, arrDep1, arrDep2);
      ...
      if (travelTimesForStopPath == null) return;
      for (Integer travelTimeForSegment : travelTimesForStopPath) {
        if (travelTimeForSegment < 0) {
          logger.error(
              "Ignoring negative travel times={} for stop path "
  ...
  /**
   * Process historic data from database for single trip. Puts resulting data into stopTimesMap and
   * travelTimesMap.
   *
   * @param dataFetcher Contains arrival/departures and matches fetched from database
   * @param arrDepList List of ArrivalDepartures for vehicle for a trip
   */
  private void aggregateTripDataIntoMaps(
      DataFetcher dataFetcher, List<ArrivalDeparture> arrDepList) {
  ...
  /**
   * Converts the list of travel times such that times are grouped by segment instead of by single
   * trips. This allows the times to be more easily processed when determining outliers, averages,
   * etc.
   *
   * <p>Only uses historic travel time data if the number of travel times segments in the data match
   * the number of travel time segments needed for the current trip configuration (which depends on
   * the current path length for the trip). This is important because configuration can change
   * (stops can move or the path between stops can change). Therefore need to make sure that only
   * using data if it applies to the current config (has same number of travel time segments). This
   * way can use as much historic info as possible, yet not try to use data that doesn't pertain.
   *
   * @param historicTravelTimes List of List of times where outer List is by single trip and inner
   *     List is travel time segment.
   * @param trip Contains config info for the trip that the data is for. Used to determine if the
   *     historic data for the trip has the proper number of travel time segments.
   * @param stopPathIndex Used to determine if the historic data for the trip has the proper number
   *     of travel time segments.
   * @return List of List of times but the outer List is by travel time segment and there is an
   *     inner List with a value per single trip, or null if there is no valid historic data for the
   *     trip.
   */
  private static List<List<Integer>> bySegment(
    ...
    // length.
    int expectedTravelTimeSegments = getNumTravelTimeSegments(trip, stopPathIndex);
    ...
    // memory use.
    List<List<Integer>> timesBySegment = new ArrayList<List<Integer>>(expectedTravelTimeSegments);
  ...
  /**
   * Takes the data from the stopTimesMap and travelTimesMap and creates corresponding travel times.
   * Puts those travel times into the TravelTimeInfoMap that is returned.
   *
   * @param tripMap contains all the trips that are configured and that need travel times for.
  ...
  public TravelTimeInfoMap createTravelTimesFromMaps(Map<String, Trip> tripMap) {
    ...
    for (ProcessedDataMapKey mapKey : combinedKeySet) {
      ...
      if (mapKey.getStopPathIndex() >= trip.getStopPaths().size()) {
        logger.error(
            "Problem with stopPathIndex for historical data. "
      ...
      String stopIdFromTrip = trip.getStopPath(mapKey.getStopPathIndex()).getStopId();
      ...
      List<List<Integer>> travelTimesForStopPathForTrip = travelTimesMap.get(mapKey);
      ...
      if (travelTimesForStopPathForTrip != null) {
        ...
        List<List<Integer>> travelTimesBySegment =
            bySegment(travelTimesForStopPathForTrip, trip, mapKey.getStopPathIndex());
      ...
      List<Integer> stopTimesForStopPathForTrip = stopTimesMap.get(mapKey);
      if (stopTimesForStopPathForTrip != null) {
        ...
        if (mapKey.getStopPathIndex() == 0) {
          ...
          averagedStopTime =
              Statistics.filteredMean(
                  stopTimesForStopPathForTrip, fractionLimitForStopTimes.getValue());
        }
      } else {
        ...
        if (mapKey.getStopPathIndex() != trip.getNumberStopPaths() - 1) {
          logger.debug(
              "No stop times for {} even though there are " + "travel times for that map key",
              mapKey);
        } else {
          unmatched++;
        }
      ...
      double travelTimeSegLength = getTravelTimeSegmentLength(trip, mapKey.getStopPathIndex());
    ...
    logger.info(
        "Processing data (total={} matched={} unmatched={} invalid={}) into a TravelTimeInfoMap took {} msec.",
        setSize,
        matched,
        unmatched,
        invalid,
  ...
  /**
   * Reads in the Matches and the ArrivalDepartures from the database for the time specified. Puts
   * the data into the stopTimesMap and the travelTimesMap for further processing.
   *
   * @param projectId
