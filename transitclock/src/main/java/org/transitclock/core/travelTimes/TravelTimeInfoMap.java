...
public class TravelTimeInfoMap {
  ...
  /**
   * Adds the new TravelTimeInfo object to the static travelTimesByTripPattern map so that can find
   * best match for trip for when there is no data for the actual trip.
   *
   ...
   * historic data for the same service ID that has the nearest time. If no historic data for a trip
   * with the same service ID then will look at trips with other service IDs.
   *
  ...
  public TravelTimeInfoWithHowSet getBestMatch(Trip trip, int stopPathIndex) {
    ...
    for (TravelTimeInfo travelTimeInfo : timesForTripPatternAndStop) {
      ...
      if (thisTrip.getServiceId().equals(trip.getServiceId())) {
        int timeDiffOfTripSchedTime =
            Time.getTimeDifference(thisTrip.getStartTime(), trip.getStartTime());
