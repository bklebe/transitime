...
public class DataFetcher {
  ...
  private static IntegerConfigValue pageSize =
      new IntegerConfigValue(
          "transitclock.updates.pageSize", 50000, "Number of records to read in at a time");
   ...
   * as Fridays for weekday service.
   *
   * @param dbName
  ...
  /**
   * Takes the date and returns the day into the year. Useful for keeping track of trip data one day
   * at a time.
   *
   * @param date
   * @return
  ...
  /**
   * Special MapKey class so that can make sure using the proper key for the associated maps in this
   * class.
   */
  ...
  /**
   * Returns a key for use in a map. They key consists of the serviceId, an optional day of the week
   * if the day specified by the date parameter matches the calendar specified by the service ID,
   * the tripId, and the vehicleId. This way can separate out data for special days like Fridays for
   * a weekday service class.
   *
   * @param serviceId
   * @param date Used to determine the day of week to be used in conjunction with the service class.
   * @param tripId
   * @param vehicleId
   * @return
   */
  public DbDataMapKey getKey(String serviceId, Date date, String tripId, String vehicleId) {
  ...
  private void addArrivalDepartureToMap(
    ...
    DbDataMapKey key =
        getKey(arrDep.getServiceId(), arrDep.getDate(), arrDep.getTripId(), arrDep.getVehicleId());
  ...
  public Map<DbDataMapKey, List<ArrivalDeparture>> readArrivalsDepartures(
    ...
    Long count =
        ArrivalDeparture.getArrivalsDeparturesCountFromDb(dbName, beginTime, endTime, null);
  ...
  private void addMatchToMap(Map<DbDataMapKey, List<Match>> map, Match match) {
    DbDataMapKey key =
        getKey(match.getServiceId(), match.getDate(), match.getTripId(), match.getVehicleId());
  ...
  private Map<DbDataMapKey, List<Match>> readMatches(
    ...
    if (!pageDbReads()) {
      ...
      do {
        ...
        matchBatchList =
            Match.getMatchesFromDb(projectId, pageBeginTime, pageEndTime, sqlClause, null, null);
        // Add arrivals/departures to map
  ...
  /**
   * Reads arrival/departure times and matches from the db and puts the data into the
   * arrivalDepartureMap and matchesMap members.
   *
   * @param agencyId
   * @param beginTime
   * @param endTime
   */
  public void readData(String agencyId, Date beginTime, Date endTime) {
    ...
    arrivalDepartureMap = readArrivalsDepartures(agencyId, beginTime, endTime);
  }

  /**
   * Provides the arrival/departure data in a map. The values in the map are Lists of
   * ArrivalDeparture times, one list for each trip where there was historic data.
   *
  ...
  /**
   * Provides the Match data in a map. The values in the map are Lists of Match objects, one list
   * for each trip where there was historic data.
   *
   * @return match data
   */
  public Map<DbDataMapKey, List<Match>> getMatchesMap() {
    ...
    if (matchesMap == null)
      throw new RuntimeException(
          "Called getMatchesMap() before " + "data was read in using readData().");
