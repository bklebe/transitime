...
/**
 * King County Metro has a bug in there GTFS-RT feed that prepends digits to stopIds. Below is a
 * quick-and-dirty method to strip them.
 */
public class KCMTranslator implements GTFSRealtimeTranslator {

  private static final Logger logger = LoggerFactory.getLogger(KCMTranslator.class);
  ...
  public Date parseFeedHeaderTimestamp(FeedHeader header) {
    ...
    if (Math.abs(System.currentTimeMillis() - feedDate.getTime()) > 1 * 60 * 1000) {
      ...
      logger.error(
          "Feed has clock skew of {} mins, time {}",
          (System.currentTimeMillis() - feedDate.getTime()) / 60000,
          feedDate);
      return new Date();
    }
    return feedDate;
