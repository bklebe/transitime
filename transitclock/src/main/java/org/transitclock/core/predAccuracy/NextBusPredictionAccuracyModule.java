 ...
 * inheriting from PredictionAccuracyModule) and stores the data in memory. Then when
 * arrivals/departures occur the prediction accuracy can be determined and stored.
 *
 * @author SkiBu Smith
 */
public class NextBusPredictionAccuracyModule extends PredictionAccuracyModule {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(NextBusPredictionAccuracyModule.class);

  /********************** Config Params **************************/

  private static final StringConfigValue externalPredictionApiUrl =
      new StringConfigValue(
          "transitclock.predAccuracy.externalPredictionApiUrl",
  ...
  private static final StringConfigValue nextBusAgencyIdForApi =
      new StringConfigValue(
          "transitclock.predAccuracy.nextBusAgencyIdForApi", null, "Name of agency for API");
  ...
  private String getUrl(RouteAndStops routeAndStops) {
    ...
    String fullUrl =
        getExternalPredictionApiUrl()
            + "command=predictionsForMultiStops"
            + "&a="
            + getNextBusAgencyIdForApi();
  ...
  /**
   * Gets XML data for a route from API an returns an XML Document object containing the resulting
   * data.
   *
   * @param routeAndStops Specifies which route and stops to read data for
  ...
  private Document getExternalPredictionsForRoute(RouteAndStops routeAndStops) {
    ...
    try {
      ...
      if (httpResponseCode != HttpURLConnection.HTTP_OK) {
        ...
        logger.error(
            "Error when getting predictions. Response " + "code was {} for URL={}",
            httpResponseCode,
            fullUrl);
        return null;
      }
      ...
      logger.error("Problem when getting data for route for URL={}", fullUrl, e);
      return null;
    }
  }
   ...
   * predictions.
   *
   * @param routeAndStops which route and stops getting predictions for
   * @param doc The returned XML document
   * @param predictionsReadTime
   */
  private void processExternalPredictionsForRoute(
      RouteAndStops routeAndStops, Document doc, Date predictionsReadTime) {
      ...
      for (Element direction : directions) {
        ...
        for (Element prediction : predictions) {
          ...
          boolean affectedByWaitStop =
              "true".equals(prediction.getAttributeValue("affectedByLayover"));
          ...
          if (tripId != null) {
            ...
            if (trip != null) {
              ...
              logger.error(
                  "Got tripId={} from NextBus feed but " + "no such trip in the configuration.",
                  tripId);
            }
          }
   ...
   *     determining length of predictions. Should be the same for all predictions read in during a
   *     polling cycle even if the predictions are read at slightly different times. By using the
   *     same time can easily see from data in db which internal and external predictions are
  ...
  protected void getAndProcessData(List<RouteAndStops> routesAndStops, Date predictionsReadTime) {
    ...
    logger.debug("Calling NextBusPredictionReaderModule." + "getAndProcessData()");
