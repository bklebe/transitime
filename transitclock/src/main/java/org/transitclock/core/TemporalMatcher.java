 ...
 * block assignment. Looks at spatial matches and determines which one makes the most sense
 * temporally.
 *
 * @author SkiBu Smith
 */
public class TemporalMatcher {
   ...
   * based on the start time of the trip plus the travel time from the beginning of the trip. This
   ...
   *     needed because for layovers want to make them less likely to match, but only if not the
   *     first spatial match. This way if matching a vehicle to middle of trip then will more likely
   *     match to the middle instead of the layover at the end of the trip. This is important when
   *     starting up the system while vehicles are already running.
  ...
  private static TemporalDifference determineHowFarOffScheduledTime(
    ...
    int travelTimeForCurrentTrip =
        TravelTimes.getInstance()
            .expectedTravelTimeBetweenMatches(
                vehicleId, tripStartTimeSecs, beginningOfTrip, spatialMatch);
    ...
    if (deltaFromSchedule.isWithinBoundsForInitialMatching()) {
      ...
      if (spatialMatch.isLayover() && !isFirstSpatialMatch) {
        deltaFromSchedule.addTime(
            CoreConfig.getAllowableLateSecondsForInitialMatching() * Time.MS_PER_SEC);
      }

      logger.debug(
          "For vehicleId={} determineHowFarOffScheduledTime() "
  ...
  /**
   * For handling complicated special layover case. Special layover case. If already was at this
   * layover then the expected travel time will be 0. For this situation need to determine if still
   * within the layover time, meaning that the time difference is 0, or if the vehicle is instead
  ...
  private TemporalDifference temporalDifferenceForSpecialLayover(
    ...
    if (expectedTravelTimeMsec == 0) {
      ...
      long scheduledDepartureTime =
          Core.getInstance().getTime().getEpochTime(departureTimeSecs, avlTime);
      if (avlTime.getTime() > scheduledDepartureTime) {
        // Vehicle should have already left so it is late
        logger.debug(
            "For vehicleId={} match at layover stop but "
  ...
  /**
   * Returns whether the currentSpatialMatch is a better match than the bestTemporalMatchSoFar. Uses
   * differenceFromExpectedTime for the current match to determine how good the new match is
   * temporally.
   *
   * @param bestTemporalMatchSoFar Can be null
   * @param currentSpatialMatch Can be null
   * @param differenceFromExpectedTime The TimeDifference for the current match. Can be null if
  ...
  private static boolean currentMatchIsBetter(
    ...
    if (currentSpatialMatch == null || differenceFromExpectedTime == null) return false;

    // The current match is valid. If there was no previous best match then
    // the current one must be better.
    if (bestTemporalMatchSoFar == null) return true;
  ...
  /**
   * There is a complication with vehicles leaving a layover slightly early where might not
   * temporally match to a match past the layover due to system thinking that vehicle first needs to
   * go back to the layover and then to the spatial match. This leads to an overly long expected
   * travel time and the result would be that the system thinks the vehicle is at the match earlier
   * than expected. At the same time it will think that it is late with respect to the layover, but
   * since late is considered a better match than early vehicle could be wrongly temporally matched
   * to layover again. As the vehicle moves further along the trip this only gets worse since system
   * will expect a longer and longer travel time for vehicle to go back to layover and then to the
   * spatial match. So could get incorrect layover match for quiet a while, until the layover
   * distance transitclock.core.layoverDistance, which can be large, is exceeded.
   *
   * <p>Therefore need to filter out such problematic layover matches. This is done by seeing if
   * past schedule time for the layover match (before the schedule time vehicles should be able to
   * roam around), the previous match for vehicle is same layover as the current spatial match, the
   * distance from the layover stop is increasing, and there is a subsequent non-layover spatial
   * match that could be used. If all these conditions are true then the current layover spatial
   * match is ignored so that will properly match vehicle to subsequent non-layover spatial match.
   *
   ...
   */
  private boolean isProblematicLayover(
      VehicleState vehicleState, List<SpatialMatch> spatialMatches, int matchIdx) {
    ...
    if (!previousMatch.isLayover()) return false;

    // If current spatial match is not the same layover then false
    if (!previousMatch.getIndices().equalStopPath(spatialMatch.getIndices())) return false;
    ...
    if (spatialMatch.getDistanceToSegment() <= previousMatch.getDistanceToSegment()) return false;
  ...
  /**
   * For the spatial matches passed in determines the one that temporally makes the most sense.
   * Compares the time elapsed between AVL reports and compares that to the expected travel time
   * between the previous and the current spatial match. The spatial match which corresponds most to
   * the expected travel time is returned as the best temporal match.
   *
   * @param vehicleState
   * @param spatialMatches
   ...
   */
  public TemporalMatch getBestTemporalMatch(
      VehicleState vehicleState, List<SpatialMatch> spatialMatches) {
    // Convenience variables
    SpatialMatch previousMatch = vehicleState.getMatch();
    Date previousAvlTime = vehicleState.getPreviousAvlReportFromSuccessfulMatch().getDate();
    AvlReport avlReport = vehicleState.getAvlReport();
    Date avlTime = avlReport.getDate();
    ...
    for (int matchIdx = 0; matchIdx < spatialMatches.size(); ++matchIdx) {
      ...
      if (isProblematicLayover(vehicleState, spatialMatches, matchIdx)) {
        ...
        logger.warn(
            "Ignoring special case layover spatial match "
                ...
                + "past the layover for a long time. {}",
            spatialMatch);
        continue;
      }
      ...
      // TODO - Check why it has to look backwards. Useful for freq based trips. Currently breaks
      ...
      if (spatialMatch.isLayover()
          && !previousMatch.getIndices().equalStopPath(spatialMatch.getIndices())) {
        expectedTravelTimeMsec +=
      ...
      if (differenceFromExpectedTime != null
        ...
        logger.debug(
            "Rejecting temporal match {} because it is not " + "within the allowable bounds.",
            differenceFromExpectedTime);
      }

      // If this temporal match is better than the previous best one
      // then remember it.
      if (currentMatchIsBetter(bestTemporalMatchSoFar, spatialMatch, differenceFromExpectedTime)) {
        bestTemporalMatchSoFar = new TemporalMatch(spatialMatch, differenceFromExpectedTime);
      } else {
        ...
        if (bestTemporalMatchSoFar != null) {
          logger.debug(
              "For vehicleId={} temporal match is getting "
  ...
  /**
   * From the list of spatial matches passed in, determines which one has the best valid temporal
   * match. Intended to be used when first matching a vehicle to an assignment. Does not use
   * previous AVL report to determine if there is a match.
   *
   * @param spatialMatches The spatial matches to examine
   * @return The match passed in that best matches temporally where vehicle should be. Returns null
   *     if no adequate temporal match.
   */
  public TemporalMatch getBestTemporalMatchComparedToSchedule(
    ...
    for (int i = 0; i < spatialMatches.size(); ++i) {
      ...
      TemporalDifference differenceFromExpectedTime =
          determineHowFarOffScheduledTime(
              avlReport.getVehicleId(), avlReport.getDate(), spatialMatch, isFirstSpatialMatch);
      ...
      if (differenceFromExpectedTime != null
        ...
        logger.debug(
            "For vehicleId={} differenceFromExpectedTime={} " + "is best yet so remembering such",
    ...
    if (bestSpatialMatch != null && bestDifferenceFromExpectedTime != null) {
      ...
      logger.debug(
          "For vehicleId={} getBestTemporalMatch() returning "
  ...
  private static boolean canDeadheadToBeginningOfTripInTime(AvlReport avlReport, Trip trip) {
    ...
    long msecsIntoDay =
        Core.getInstance().getTime().getMsecsIntoDay(avlReport.getDate(), tripStartTimeMsecs);
    ...
    logger.debug(
        "For vehicleId={} tripIndex={} tripId={} is not in "
  ...
  /**
   * Returns the next trip that the vehicle could get to in time. Returns null if can't get there.
   *
   * <p>Don't want to look through all trips for a block because then might match to first block the
   * next day, which would be a problem. Instead need to only look at trips that are considered to
   * be valid. Therefore should only pass in upcoming trips as the potentialTrips parameter.
   *
   * @param avlReport
   * @param potentialTrips specifies which trips to examine
   * @return
   */
  public Trip matchToLayoverStopEvenIfOffRoute(AvlReport avlReport, List<Trip> potentialTrips) {
    // Determine upcoming wait stop
    for (Trip trip : potentialTrips) {
      if (canDeadheadToBeginningOfTripInTime(avlReport, trip)) return trip;
    }
