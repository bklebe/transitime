...
public class HoldingTimeGeneratorDefaultImpl implements HoldingTimeGenerator {
  ...
  protected static BooleanConfigValue storeHoldingTimes =
      new BooleanConfigValue(
          "transitclock.holding.storeHoldingTimes",
          true,
  ...
  protected static BooleanConfigValue useArrivalEvents =
      new BooleanConfigValue(
          "transitclock.holding.usearrivalevents",
          true,
          "Generate a holding time on arrival events.");

  protected static BooleanConfigValue useArrivalPredictions =
      new BooleanConfigValue(
          "transitclock.holding.usearrivalpredictions",
          true,
          "Generate a holding time on arrival predictions.");

  protected static BooleanConfigValue regenerateOnDeparture =
      new BooleanConfigValue(
  ...
  protected static IntegerConfigValue plannedHeadwayMsec =
      new IntegerConfigValue(
          "transitclock.holding.plannedHeadwayMsec", 60 * 1000 * 9, "Planned Headway");
  protected static StringListConfigValue controlStopList =
      new StringListConfigValue(
          "transitclock.holding.controlStops",
          null,
    ...
    if (event.isArrival() && isControlStop(event.getStopId())) {
      ...
      for (IpcVehicleComplete currentVehicle : VehicleDataCache.getInstance().getVehicles()) {
        if (predictionCache.getPredictionForVehicle(
            ...
            != null) {
          predictions.add(
              predictionCache.getPredictionForVehicle(
                  currentVehicle.getId(), event.getRouteId(), event.getStopId()));
        }
      }
      ...
      if (predictions.size() > 0
          && predictions.get(0).getVehicleId().equals(event.getVehicleId())) {
        predictions.remove(0);
      }

      logger.debug("Have {} predictions for stops {}.", predictions.size(), event.getStopId());

      IpcArrivalDeparture lastVehicleDeparture =
          getLastVehicleDepartureTime(
              event.getVehicleId(),
              event.getTripId(),
              event.getStopId(),
      ...
      HoldingTime lastVehicleDepartureByHoldingTime =
          getNextDepartureByHoldingTime(
              event.getVehicleId(), getCurrentHoldingTimesForStop(event.getStopId()));
      ...
      if (lastVehicleDepartureByHoldingTime != null) {
        logger.debug(
            "Found waiting vehicle with holding time: {}", lastVehicleDepartureByHoldingTime);
        if (predictions.size() > 1) {
          ...
          for (int i = 0;
              i < predictions.size()
                  && counter < maxPredictionsForHoldingTimeCalculation.getValue();
              i++) {
            logger.debug(
                "Prediction for N-{} {}: {} ",
                counter + 1,
          ...
          long holdingTimeValue =
              calculateHoldingTime(
                  current_vehicle_arrival_time,
                  lastVehicleDepartureByHoldingTime.getTimeToLeave(event.getTime()).getTime(),
                  N,
                  maxPredictionsForHoldingTimeCalculation.getValue());
          ...
          if (storeHoldingTimes.getValue()) Core.getInstance().getDbLogger().add(holdingTime);
        ...
        if (predictions.size() > 1) {
          ...
          long holdingTimeValue =
              calculateHoldingTime(
                  current_vehicle_arrival_time, lastVehicleDeparture.getTime().getTime());
  ...
  protected HoldingTime getNextDepartureByHoldingTime(
      String currentVehicleId, List<HoldingTime> holdingTimes) {
      ...
      if (!holdingTime.getVehicleId().equals(currentVehicleId)
        ...
        if (holdingTime.isArrivalPredictionUsed() == false) {
          if (nextDeparture == null
              || holdingTime.getHoldingTime().before(nextDeparture.getHoldingTime())) {
            nextDeparture = holdingTime;
          }
        } else {
          logger.debug(
              "Holding time not considered as it is a predicted holding time. {}", holdingTime);
        }
      }
    }
  ...
  private IpcArrivalDeparture getLastVehicleDepartureTime(
    ...
    if (currentStopList != null) {
      ...
      for (IpcArrivalDeparture event : currentStopList) {
        // if(event.isDeparture() && event.getTripId().equals(tripId)) TODO This is because of the
        // GTFS config of Atlanta Streetcar. Could we use route_id instead?
  ...
  private IpcArrivalDeparture getLastVehicleArrivalEvent(
      String stopid, String vehicleid, Date time) {
    StopArrivalDepartureCacheKey currentStopKey = new StopArrivalDepartureCacheKey(stopid, time);
            ...
            if (event.getTime().getTime() > time.getTime()) {
              if (closestArrivalEvent == null) {
                closestArrivalEvent = event;
                  ...
                  < Math.abs(time.getTime() - closestArrivalEvent.getTime().getTime())) {
                closestArrivalEvent = event;
              }
            }
          }
  ...
  public ArrayList<IpcArrivalDeparture> addArrivalTimesForVehiclesAtStop(
      String stopid, String vehicleid, Date time, ArrayList<Long> arrivalTimes) {
  ...
  private static Long calculateHoldingTime(
      Long current_vehicle_arrival_time,
  ...
  private long calculateHoldingTime(
      long current_vehicle_arrival_time, long last_vehicle_departure_time) {
  ...
  @Override
  public HoldingTime generateHoldingTime(
      VehicleState vehicleState, IpcPrediction arrivalPrediction) {
    ...
    if (arrivalPrediction.isArrival() && isControlStop(arrivalPrediction.getStopId())) {
      ...
      IpcPrediction forwardDeparturePrediction =
          getForwardVehicleDeparturePrediction(arrivalPrediction);
      if (forwardDeparturePrediction == null)
        logger.debug(
            "Cannot find last departure prediction for : {}", arrivalPrediction.toString());
      else
        logger.debug(
            "Found last vehicle predicted departure event: {}",
            forwardDeparturePrediction.toString());

      IpcArrivalDeparture lastDeparture =
          getLastVehicleDepartureTime(
      ...
      else logger.debug("Found last vehicle departure event: {}", lastDeparture.toString());

      List<IpcPrediction> backwardArrivalPredictions =
          getBackwardArrivalPredictions(arrivalPrediction);
      ...
      if ((forwardDeparturePrediction != null || lastDeparture != null)
          ...
          && backwardArrivalPredictions.size() > 0) {
        ...
        if (lastDeparture != null && forwardDeparturePrediction != null) {
          ...
          } else if (lastDeparture.getTime().getTime()
              <= forwardDeparturePrediction.getPredictionTime()) {
            long holdingTimeValue =
                calculateHoldingTime(
                    arrivalPrediction.getPredictionTime(),
        ...
        if (forwardDeparturePrediction != null || lastDeparture != null) {
          ...
          if (lastDeparture != null && forwardDeparturePrediction != null) {
            ...
            holdingTimeValue =
                calculateHoldingTime(
                    arrivalPrediction.getPredictionTime(), lastDeparture.getTime().getTime());
          } else if (lastDeparture == null && forwardDeparturePrediction != null) {
            holdingTimeValue =
                calculateHoldingTime(
                    arrivalPrediction.getPredictionTime(),
  ...
  protected List<IpcPrediction> getBackwardArrivalPredictions(IpcPrediction predictionEvent) {
    ...
    for (IpcPredictionsForRouteStopDest predictionForRouteStopDest : predictionsForRouteStopDests) {
      for (IpcPrediction prediction : predictionForRouteStopDest.getPredictionsForRouteStop()) {
        ...
        // The end of the route it seems to only generate departure predictions which points towards
        // an issue.
  ...
  ArrayList<Long> predictionsToLongArrayList(
      List<IpcPrediction> predictions, ArrayList<Long> list) {
  ...
  public static void main(String[] args) {
    {
      ...
      Long result =
          calculateHoldingTime(5L, 0L, N, maxPredictionsForHoldingTimeCalculation.getValue());
    ...
    }
    {
      ...
      Long result =
          calculateHoldingTime(4L, 0L, N, maxPredictionsForHoldingTimeCalculation.getValue());
...

    {
      ...
      Long result =
          calculateHoldingTime(7L, 0L, N, maxPredictionsForHoldingTimeCalculation.getValue());
    ...
    }
    {
      ...
      Long result =
          calculateHoldingTime(3L, 0L, N, maxPredictionsForHoldingTimeCalculation.getValue());
  ...
  public void handleDeparture(VehicleState vehicleState, ArrivalDeparture arrivalDeparture) {
    ...
    if (arrivalDeparture.isDeparture() && isControlStop(arrivalDeparture.getStopId())) {
      ...
      logger.debug(
          "Removing holding time {} due to departure {}.",
          vehicleState.getHoldingTime(),
          arrivalDeparture);
      vehicleState.setHoldingTime(null);

      if (regenerateOnDeparture.getValue()) {
        for (HoldingTime holdingTime :
            getCurrentHoldingTimesForStop(arrivalDeparture.getStopId())) {
          VehicleState otherState =
              VehicleStateManager.getInstance().getVehicleState(holdingTime.getVehicleId());

          IpcArrivalDeparture lastArrival =
              getLastVehicleArrivalEvent(
                  arrivalDeparture.getStopId(),
                  otherState.getVehicleId(),
                  arrivalDeparture.getAvlTime());

          HoldingTime otherHoldingTime =
              HoldingTimeGeneratorFactory.getInstance()
                  .generateHoldingTime(otherState, lastArrival);
