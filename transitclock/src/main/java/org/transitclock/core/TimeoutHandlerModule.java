...
public class TimeoutHandlerModule extends Module {
  ...
  /**
   * For regular predictable vehicle that is not a schedule based prediction nor a vehicle at a wait
   * stop. If haven't reported in too long makes the vehicle unpredictable and logs situation.
   *
   * @param vehicleState
   * @param now
   * @param mapIterator
    ...
    if (now > vehicleState.getAvlReport().getTime() + maxNoAvl) {
      ...
      AvlProcessor.getInstance()
          .makeVehicleUnpredictable(
              vehicleState.getVehicleId(), eventDescription, VehicleEvent.TIMEOUT);
  ...
  /**
   * For not predictable vehicle. If not removing vehicles from cache, removes the vehicle from the
   * map to avoid looking at it again. If configured to remove timed out vehicles from cache, and
   * haven't reported in too long, removes the vehicle from map and cache.
   *
  ...
  private void handleNotPredictablePossibleTimeout(
    ...
    if (now > vehicleState.getAvlReport().getTime() + maxNoAvl) {
      ...
      // Log the situation
      logger.info(
          "For not predictable vehicleId={} generated timeout " + "event.",
  ...
  /**
   * It is a wait stop which means that vehicle can be stopped and turned off for a while such that
   * don't expect to get any AVL reports. Only timeout if past more that the allowed time for wait
   * stops
   *
   * @param vehicleState
   * @param now
   * @param mapIterator
   */
  private void handleWaitStopPossibleTimeout(
      VehicleState vehicleState, long now, Iterator<AvlReport> mapIterator) {
