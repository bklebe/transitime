...
public class VehicleAtStopInfo extends Indices {
  ...
  public VehicleAtStopInfo(Indices indices) {
    super(
        indices.getBlock(), indices.getTripIndex(), indices.getStopPathIndex(), 0); // segment index
  }
  ...
  /**
   * For schedule based assignment returns true if tripIndex and stopPathIndex are for the very last
   * trip, path, segment for the block assignment. Had to override Indices.atEndOfBlock() because
   * this class doesn't use the segment index while Indices does.
   *
