...
public class BlocksInfo {
  ...
  private static IntegerConfigValue blockactiveForTimeBeforeSecs =
  ...
  private static IntegerConfigValue blockactiveForTimeAfterSecs =
      new IntegerConfigValue(
  ...
  public static List<Block> getCurrentlyActiveBlocks() {
    return getCurrentlyActiveBlocks(
        null,
        null,
        blockactiveForTimeBeforeSecs.getValue(),
        blockactiveForTimeAfterSecs.getValue());
  }

  /**
   * Returns list of blocks that are currently active for the specified routes.
   *
   * @param routeIds Collection of routes IDs that want blocks for. Use null to indicate all routes.
  ...
  public static List<Block> getCurrentlyActiveBlocks(
    ...
    if (secsInDayForAvlReport < 4 * Time.HOUR_IN_SECS) {
      List<String> previousDayServiceIds =
          core.getServiceUtils().getServiceIdsForDay(now - Time.DAY_IN_MSECS);
    ...
    for (String serviceId : serviceIds) {
      ...
      for (Block block : blocks) {
        ...
        // add it to the list
        if (block.isActive(now, allowableBeforeTimeSecs, allowableAfterStartTimeSecs)
            && forSpecifiedRoute) activeBlocks.add(block);
      }
    }
