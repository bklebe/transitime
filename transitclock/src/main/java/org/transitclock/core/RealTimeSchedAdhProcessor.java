...
public class RealTimeSchedAdhProcessor {
   ...
   * scheduled departure time then the schedule adherence for that stop is returned. Otherwise will
   * look at when the vehicle is expected to be at the next stop with a scheduled time and provide
   * the expected schedule adherence for that stop. Doing it this way is useful because it allows
   * the schedule adherence to be updated while the vehicle is in between stops.
   *
   * @param vehicleState
   * @return The real-time schedule adherence for the vehicle, or null if vehicle is not predictable
   *     or there are no upcoming stops with a schedule time.
   */
  public static TemporalDifference generate(VehicleState vehicleState) {
    ...
    // that stop
    int travelTimeToStopMsec =
        TravelTimes.getInstance()
            .expectedTravelTimeBetweenMatches(
                vehicleId, avlTime, match, matchAtStopWithScheduleTime);
    ...
    long departureEpochTime =
        Core.getInstance().getTime().getEpochTime(scheduleTime.getTime(), avlTime);
    TemporalDifference scheduleAdherence =
  ...
  /**
   * We define effective schedule time as where the bus currently falls in the schedule based on its
   * current position.
   */
  public static TemporalDifference generateEffectiveScheduleDifference(VehicleState vehicleState) {
    ...
    if (previousStopPathIndex < 0) {
      ...
      Long departureEpoch =
          Core.getInstance().getTime().getEpochTime(trip.getScheduleTime(0).getTime(), avlTime);
      ...
      if (difference < 0) {
        ...
        if (tripIndex > 0) {
          ...
          Long epochEndTime =
              Core.getInstance().getTime().getEpochTime(prevTrip.getEndTime(), avlTime);

          difference = Math.min(0, avlTime - epochEndTime);

          logger.debug(
              "vehicleId {} has schedDev before trip set by previous trip of {}",
              vehicleId,
              difference);
        }
      }
    ...
    if (match.isAtStop()) {
      ...
      logger.debug(
          "vehicleId {} has schedDev at stop of {}", vehicleId, (avlTime - departureEpoch));
    ...
    Long effectiveScheduleTimeEpoch =
        Core.getInstance().getTime().getEpochTime(effectiveStopTimeSec, avlTime);
