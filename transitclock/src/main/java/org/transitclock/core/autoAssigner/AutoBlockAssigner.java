...
public class AutoBlockAssigner {
   ...
   * Constructor
   *
   * @param vehicleState Info on the vehicle to match
   */
  ...
  /**
   * Determines if a block doesn't have a non-schedule based vehicle associated with it. This means
   * that the block assignment is available for trying to automatically assign a vehicle to it.
   * Schedule based vehicles don't count because even when have a schedule based vehicle still want
   * to assign a real vehicle to that assignment when possible.
   *
  ...
  private boolean isBlockUnassigned(String blockId) {
    ...
    // unassigned.
    if (vehicleIdsForBlock.isEmpty()) return true;
  ...
  /**
   * Determines the best match by looking at both the current AVL report and the previous one. Only
   * for block assignments that do not have a schedule.
   *
   * @param block
  ...
  private TemporalMatch bestNoScheduleMatch(Block block) {
    ...
    List<SpatialMatch> spatialMatches =
        SpatialMatcher.getSpatialMatchesForAutoAssigning(getAvlReport(), block, potentialTrips);
    if (spatialMatches.isEmpty()) return null;
    ...
    List<SpatialMatch> prevSpatialMatches =
        SpatialMatcher.getSpatialMatchesForAutoAssigning(previousAvlReport, block, potentialTrips);
    if (prevSpatialMatches.isEmpty()) return null;

    // Determine params needed in the following for loop
    int timeOfDayInSecs =
        Core.getInstance().getTime().getSecondsIntoDay(previousAvlReport.getTime());
    ...
    for (SpatialMatch prevSpatialMatch : prevSpatialMatches) {
      for (SpatialMatch spatialMatch : spatialMatches) {
        ...
        int expectedTravelTimeMsec =
            TravelTimes.getInstance()
                .expectedTravelTimeBetweenMatches(
                    avlReport.getVehicleId(), timeOfDayInSecs, prevSpatialMatch, spatialMatch);
        ...
        if (!differenceFromExpectedTime.isWithinBounds(
            allowableEarlySeconds.getValue(), allowableLateSeconds.getValue())) continue;
  ...
  private List<SpatialMatch> getSpatialMatches(AvlReport avlReport, Block block) {
    ...
    for (Trip trip : activeTrips) {
      ...
      logger.debug(
          "For vehicleId={} checking tripId={} with "
      ...
      if (spatialMatchCache.containsKey(tripPatternId)) {
        ...
        if (previouslyFoundMatch != null) {
          ...
          // info
          SpatialMatch matchWithProperBlock = new SpatialMatch(previouslyFoundMatch, trip);
    ...
    List<SpatialMatch> newSpatialMatches =
        SpatialMatcher.getSpatialMatchesForAutoAssigning(avlReport, block, tripsNeedToInvestigate);
   ...
   * investigating the current AVL report. Therefore this method is useful for checking previous AVL
   * reports.
   *
   * @param avlReport The AVL report to be matched
   * @param block The block to match the AVL report to
   ...
   * spatial matches and then finds one that best matches to the schedule. If the match is adequate
   * spatial and temporally then the match is returned. Intended for schedule based blocks only.
   *
   * @param avlReport The AVL report to be matched
   * @param block The block to match the AVL report to
  ...
  private TemporalMatch bestTemporalMatch(AvlReport avlReport, Block block, boolean useCache) {
    ...
    if (bestMatch != null) {
      ...
      boolean isWithinBounds =
          diff.isWithinBounds(allowableEarlySeconds.getValue(), allowableLateSeconds.getValue());
      if (!isWithinBounds) {
  ...
  private TemporalMatch bestScheduleMatch(Block block) {
    ...
    logger.debug(
        "For vehicleId={} and blockId={} calling "
    ...
    logger.debug(
        "For vehicleId={} did NOT get valid match for "
  ...
  /**
   * Goes through all the currently active blocks and tries to match the AVL report to them. Returns
   * list of valid temporal matches. Ignores layover matches since they are too lenient to indicate
   * a valid match. Also requires a previous AVL report to match appropriately to make sure that
   * vehicle really matches and isn't just sitting there and isn't going in other direction or
   * crossing route and matching only momentarily.
   *
  ...
  private List<TemporalMatch> determineTemporalMatches() {
    ...
    if (getPreviousAvlReport() == null) {
      ...
      logger.info(
          ...
          vehicleId,
          minDistanceFromCurrentReport.getValue(),
          getAvlReport());
      return validMatches;
    }
    ...
    List<Block> blocksToExamine =
        CoreConfig.exclusiveBlockAssignments()
    ...
    if (blocksToExamine.isEmpty()) {
      ...
      logger.info(
          "For vehicleId={} examining {} blocks for matches.", vehicleId, blocksToExamine.size());
    }
    ...
    for (Block block : blocksToExamine) {
      ...
      if (logger.isDebugEnabled()) {
        ...
        logger.debug(
            "For vehicleId={} examining blockId={} for match. "
      ...
      TemporalMatch bestMatch =
          block.isNoSchedule() ? bestNoScheduleMatch(block) : bestScheduleMatch(block);
      if (bestMatch != null) validMatches.add(bestMatch);

      logger.debug(
          "For vehicleId={} checking blockId={} took {}msec", vehicleId, block.getId(), blockTimer);
    }

    // Return the valid matches that were found
    logger.info(
        "Total time for determining possible auto assignment "
  ...
  private static boolean tooRecent(VehicleState vehicleState) {
    ...
    boolean tooRecent = elapsedSecs < minTimeBetweenAutoAssigningSecs.getValue();
   ...
   * active blocks and tries to match the AVL report to them. Returns a TemporalMatch if there is a
   * single block that can be successfully matched to. Ignores layover matches since they are too
   * lenient to indicate a valid match. Also requires a previous AVL report to match appropriately
   * to make sure that vehicle really matches and isn't just sitting there and isn't going in other
  ...
  public TemporalMatch autoAssignVehicleToBlockIfEnabled() {
    ...
    // the match
    if (!autoAssignerEnabled.getValue()) return null;
    ...
    if (matches.size() > 1) {
      logger.info(
          "Found multiple matches ({}) for vehicleId={}. "
