...
public class ArrivalDepartureGeneratorDefaultImpl implements ArrivalDepartureGenerator {
   ...
   * determine arrival/departure times for earlier stops so that won't miss recording data for them.
   * But only want to go so far. Otherwise could be generating fake arrival/departure times when
   * vehicle did not actually traverse that stop.
   */
  ...
  /**
   * Returns whether going from oldMatch to newMatch traverses so many stops during the elapsed AVL
   * time that it isn't reasonable to think that the vehicle did so. If too many stops would be
   * traversed then logs an error message indicating that this isn't reasonable. When true is
   ...
   * accurate causing the vehicle to match to the wrong direction, or if there is a software problem
   * that causes an improper match.
   *
  ...
  /**
   * Determines if need to determine arrival/departure times due to vehicle having traversed a stop.
   *
   ...
   * @return
   */
  private boolean shouldProcessArrivedOrDepartedStops(
      SpatialMatch oldMatch, SpatialMatch newMatch) {
   ...
   * VehicleState so that can make sure that subsequent departures are after the last arrival time.
   *
   * @param vehicleState
   * @param arrivalTime
   * @param block
  ...
  protected Arrival createArrivalTime(
      VehicleState vehicleState, long arrivalTime, Block block, int tripIndex, int stopPathIndex) {
  ...
  private void updateCache(VehicleState vehicleState, ArrivalDeparture arrivalDeparture) {
    ...
    if (HoldingTimeGeneratorFactory.getInstance() != null) {
      ...
      try {
        holdingTime =
            HoldingTimeGeneratorFactory.getInstance()
  ...
  /**
   * For making sure that the arrival/departure time is reasonably close to the AVL time. Otherwise
   * this indicates there was a problem determining the arrival/departure time.
   *
   * @param time
   * @param avlReport
  ...
  private boolean timeReasonable(ArrivalDeparture arrivalDeparture) {
    long delta =
        Math.abs(arrivalDeparture.getAvlTime().getTime() - arrivalDeparture.getDate().getTime());
  ...
  /**
   * Stores the specified ArrivalDeparture object into the db and log to the ArrivalsDeparatures log
   * file that the object was created.
   *
  ...
  protected void storeInDbAndLog(ArrivalDeparture arrivalDeparture) {
    ...
    VehicleState vehicleState =
        VehicleStateManager.getInstance().getVehicleState(arrivalDeparture.getVehicleId());
  ...
  /**
   * If vehicle departs terminal too early or too late then log an event so that the problem is made
   * more obvious.
   *
   * @param vehicleState
   * @param arrivalDeparture
   */
  private void logEventIfVehicleDepartedEarlyOrLate(
      VehicleState vehicleState, Departure departure) {
   ...
   * meaning that it didn't traverse the earlier stops and so shouldn't fake arrival/departure times
   * for the earlier stops since there is a good chance they never happened.
   *
   * @param vehicleState
   * @return List of ArrivalDepartures created
   */
  private void estimateArrivalsDeparturesWithoutPreviousMatch(VehicleState vehicleState) {
    ...
    if (newMatch.getTripIndex() == 0
      ...
      long travelTimeFromFirstStopToMatch =
          TravelTimes.getInstance()
              .expectedTravelTimeBetweenMatches(
                  vehicleId, avlReportTime, beginningOfTrip, newMatch);
      ...
      if (!newMatch.isAtStop(tripIndex, stopPathIndex)) {
        storeInDbAndLog(
            createDepartureTime(vehicleState, departureTime, block, tripIndex, stopPathIndex));
      }
      ...
      for (stopPathIndex = 1; stopPathIndex < newMatch.getStopPathIndex(); ++stopPathIndex) {
        ...
        storeInDbAndLog(
            createArrivalTime(vehicleState, arrivalTime, block, tripIndex, stopPathIndex));
  ...
  /**
   * Makes sure that the departure time is after the arrival time. Also handles the situation where
   * couldn't store the previous arrival time because wasn't certain about it because it was
  ...
  private long adjustDepartureSoAfterArrival(
      long departureTime, long departureTimeBasedOnNewMatch, VehicleState vehicleState) {
    String vehicleId = vehicleState.getVehicleId();
    ...
    if (arrivalToStoreInDb != null) {
      ...
      if (arrivalToStoreInDb.getTime() >= departureTime) {
        long originalTimeBetweenOldAvlAndArrival =
            arrivalToStoreInDb.getTime() - previousAvlReport.getTime();
        ...
        arrivalToStoreInDb = arrivalToStoreInDb.withUpdatedTime(new Date(newArrivalTime));
      }
  ...
  /**
   * Handles the case where the old match indicates that vehicle has departed a stop. Determines the
   * appropriate departure time.
   *
   * @param vehicleState For obtaining match and AVL info
   * @return The time to be used as the beginTime for determining arrivals/departures for
   *     intermediate stops. Will be the time of the new AVL report if vehicle is not at a stop. If
   *     it is at a stop then it will be the expected departure time at that stop.
   */
  private long handleVehicleDepartingStop(VehicleState vehicleState) {
    ...
    logger.debug(
        "vehicleId={} was at stop {}  previous AVL report "
    ...
    SpatialMatch matchJustAfterStop = oldMatch.getMatchAdjustedToBeginningOfPath();
    ...
    SpatialMatch newMatch = vehicleState.getMatch();
    int travelTimeToNewMatchMsec =
        TravelTimes.getInstance()
            .expectedTravelTimeBetweenMatches(
                vehicleId, previousAvlReport.getDate(), matchJustAfterStop, newMatch);
    AvlReport avlReport = vehicleState.getAvlReport();
    ...
    if (matchJustAfterStop.lessThanOrEqualTo(oldMatch)) {
      ...
      int travelTimeFromStopToOldMatchMsec =
          TravelTimes.getInstance()
              .expectedTravelTimeBetweenMatches(
      ...
      int travelTimeFromOldMatchToStopMsec =
          TravelTimes.getInstance()
              .expectedTravelTimeBetweenMatches(
    ...
    departureTime =
        adjustDepartureSoAfterArrival(departureTime, departureTimeBasedOnNewMatch, vehicleState);
   ...
   * @return The time to be used as the endTime for determining arrivals/departures for intermediate
   *     stops. Will be the time of the new AVL report if vehicle is not at a stop. If it is at a
   *     stop then it will be the expected arrival time at that stop.
   */
  private long handleVehicleArrivingAtStop(VehicleState vehicleState, long beginTime) {
    String vehicleId = vehicleState.getVehicleId();
    ...
    // report
    SpatialMatch oldMatch = vehicleState.getPreviousMatch();
    int travelTimeFromOldMatchMsec =
        TravelTimes.getInstance()
            .expectedTravelTimeBetweenMatches(
                vehicleId, avlReport.getDate(), oldMatch, matchJustBeforeStop);
    ...
    if (newMatch.lessThanOrEqualTo(matchJustBeforeStop)) {
      ...
      int travelTimeFromNewMatchToStopMsec =
          TravelTimes.getInstance()
              .expectedTravelTimeBetweenMatches(
                  vehicleId, avlReport.getDate(), newMatch, matchJustBeforeStop);
      ...
      int travelTimeFromStoptoNewMatchMsec =
          TravelTimes.getInstance()
              .expectedTravelTimeBetweenMatches(
                  vehicleId, avlReport.getDate(), matchJustAfterStop, newMatch);
   ...
   * travel or stop time, meaning that the arrival/departure times will need to be adjusted by 1msec
   * to make sure that each time is unique for a vehicle.
   *
   * @param oldMatch
   * @param newMatch
  ...
  /**
   * Determine arrival/departure info for in the between stops between the previous and the current
   * match.
   *
   * @param vehicleState For obtaining match and AVL info
   ...
   */
  private void handleIntermediateStops(VehicleState vehicleState, long beginTime, long endTime) {
    ...
    VehicleAtStopInfo newVehicleAtStopInfo = newMatch.getAtStop();
    Indices endIndices =
        newVehicleAtStopInfo != null ? newVehicleAtStopInfo.clone() : newMatch.getIndices();
    ...
    logger.debug(
        "For vehicleId={} determining if it traversed stops "
    ...
    while (indices.isEarlierStopPathThan(endIndices)) {
      ...
      double pathTravelTime =
          block.getStopPathTravelTime(indices.getTripIndex(), indices.getStopPathIndex());
      if (pathTravelTime * speedRatio < 1.0) pathTravelTime = 1.0 / speedRatio;
  ...
  public void generate(VehicleState vehicleState) {
    ...
    if (newMatch == null) {
      logger.error(
          "Vehicle was not matched when trying to process " + "arrival/departure times. {}",
          vehicleState);
      // Return empty arrivalDepartures list
      return;
    }
