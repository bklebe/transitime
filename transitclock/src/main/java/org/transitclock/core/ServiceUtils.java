...
public class ServiceUtils {
  ...
  /**
   * ServiceUtils constructor. Creates reusable GregorianCalendar and sets the timezone so that the
  ...
  /**
   * Gets list of currently active calendars. If all Calendars have expired then will still use the
   * ones that end at the latest date. This way if someone forgets to update the GTFS Calendars or
   * if someone forgets to process the latest GTFS data in time, the system will still run using the
   * old Calendars. This is very important because it is unfortunately somewhat common for the
   * Calendars to expire.
   *
   ...
   *
   * @param epochTime The current time that determining service IDs for
   * @return List of service IDs that are active for the specified time, includes ones for previous
   *     day if epochTime specifies it is early in the morning.
   */
  public Collection<String> getServiceIds(Date epochTime) {
    ...
    if (time.getSecondsIntoDay(epochTime)
        > minutesIntoMorningToIncludePreviousServiceIds.getValue() * Time.MIN_IN_SECS)
