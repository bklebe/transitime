...
public class KalmanPrediction {
   ...
   * @param historical_segments The last 3 days for info relating to the time taken for the vehicle
   *     handling the same service/trip took.
   ...
   * @return KalmanPredictionResult which contains the predicted time and the last_prediction_error
   *     to be used in the next prediction calculation.
   * @throws Exception
   */
  private static final BooleanConfigValue useaverage =
  ...
  public KalmanPredictionResult predict(
      TripSegment last_vehicle_segment,
  ...
  private double historicalAverage(TripSegment historical_segments[]) throws Exception {
    if (historical_segments.length > 0) {
      ...
      for (int i = 0; i < historical_segments.length; i++) {
        long duration =
            historical_segments[i].getDestination().getTime()
  ...
  private double prediction(
    ...
    if (useaverage.getValue() == false) {
      historical_duration =
          historical_segments[historical_segments.length - 1].getDestination().getTime()
              - historical_segments[historical_segments.length - 1].getOrigin().getTime();
    }

    long last_vehicle_duration =
        last_vechicle_segment.getDestination().getTime()
