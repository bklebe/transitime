...
public class HistoricalAveragePredictionGeneratorImpl extends LastVehiclePredictionGeneratorImpl
  ...
  private static final IntegerConfigValue minDays =
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(HistoricalAveragePredictionGeneratorImpl.class);
  ...
  public long getTravelTimeForPath(
    ...
    if (vehicleState.getTripStartTime(vehicleState.getTripCounter()) != null) {
      ...
      Integer time =
          FrequencyBasedHistoricalAverageCache.secondsFromMidnight(avlReport.getDate(), 2);
      ...
      StopPathCacheKey historicalAverageCacheKey =
          new StopPathCacheKey(
              indices.getTrip().getId(), indices.getStopPathIndex(), true, time.longValue());
      ...
      if (average != null && average.getCount() >= minDays.getValue()) {
        ...
        logger.debug(
            "Using historical average algorithm for prediction : "
                + average.toString()
                + " for : "
                + indices.toString());
    ...
    // logger.debug("No historical average found, generating prediction using lastvehicle algorithm:
    // " + historicalAverageCacheKey.toString());
  ...
  public long expectedTravelTimeFromMatchToEndOfStopPath(AvlReport avlReport, SpatialMatch match) {
    ...
    Integer time =
        FrequencyBasedHistoricalAverageCache.secondsFromMidnight(new Date(match.getAvlTime()), 2);
    ...
    if (average != null && average.getCount() >= minDays.getValue()) {
      double fractionofstoppathlefttotravel =
          (match.getStopPath().getLength() - match.getDistanceAlongStopPath())
  ...
  public long getStopTimeForPath(Indices indices, AvlReport avlReport, VehicleState vehicleState) {
    ...
    if (vehicleState.getTripStartTime(vehicleState.getTripCounter()) != null) {
      ...
      if (average != null && average.getCount() >= minDays.getValue()) {
        ...
        logger.debug(
            "Using historical average alogrithm for dwell time : "
                + average.toString()
                + " for : "
                + indices.toString());
        return (long) average.getAverage();
