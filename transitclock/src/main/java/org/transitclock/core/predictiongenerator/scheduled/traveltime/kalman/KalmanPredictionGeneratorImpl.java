...
public class KalmanPredictionGeneratorImpl extends PredictionGeneratorDefaultImpl
   ...
   * historical value.
   */
  private static final IntegerConfigValue minKalmanDays =
  ...
  private static final IntegerConfigValue maxKalmanDays =
      new IntegerConfigValue(
  ...
  private static final IntegerConfigValue maxKalmanDaysToSearch =
      new IntegerConfigValue(
  ...
  private static final DoubleConfigValue initialErrorValue =
  ...
  private static final BooleanConfigValue useKalmanForPartialStopPaths =
  ...
  private static final IntegerConfigValue percentagePredictionMethodDifferenceneEventLog =
      new IntegerConfigValue(
  ...
  private static final IntegerConfigValue tresholdForDifferenceEventLog =
      new IntegerConfigValue(
  ...
  public long getTravelTimeForPath(
    ...
    VehicleState currentVehicleState =
        vehicleStateManager.getVehicleState(avlReport.getVehicleId());

    try {
      TravelTimeDetails travelTimeDetails =
          this.getLastVehicleTravelTime(currentVehicleState, indices);
      ...
      if (travelTimeDetails != null) {
        ...
        logger.debug(
            "Kalman has last vehicle info for : " + indices.toString() + " : " + travelTimeDetails);
        ...
        if (lastDaysTimes != null && lastDaysTimes.size() >= minKalmanDays.getValue().intValue()) {
          ...
          try {
            ...
            for (int i = 0; i < lastDaysTimes.size() && i < maxKalmanDays.getValue(); i++) {
              ...
              logger.debug(
                  "Kalman is using historical value : "
                      + lastDaysTimes.get(i)
                      + " for : "
                      + indices.toString());

              VehicleStopDetail destinationDetail =
                  new VehicleStopDetail(null, lastDaysTimes.get(i).getTravelTime(), vehicle);
            ...
            logger.debug(
                "Using last vehicle value: " + travelTimeDetails + " for : " + indices.toString());

            kalmanPredictionResult =
                kalmanPrediction.predict(
            ...
            if (((percentageDifferecence * alternatePrediction) / 100)
              ...
              if (percentageDifferecence
                  > percentagePredictionMethodDifferenceneEventLog.getValue()) {
                String description =
                    "Kalman predicts : "
                        + predictionTime
                        + " Super predicts : "
  ...
  public long expectedTravelTimeFromMatchToEndOfStopPath(AvlReport avlReport, SpatialMatch match) {
    ...
    if (useKalmanForPartialStopPaths.getValue().booleanValue()) {
      ...
      long remainingtime =
          (long) (fulltime * ((stopPathLength - distanceAlongStopPath) / stopPathLength));
  ...
  private KalmanError lastVehiclePredictionError(ErrorCache cache, Indices indices) {
    ...
    try {
      ...
      if (result == null) {
        logger.debug(
            "Kalman Error value set to default: "
