...
public class SpatialMatch {
  ...
  /** based on the current trip/stop path/sgement compute the predicted vehicle location. */
  ...
  /** based on the indices and distance along segment compute the predicted vehicle location. */
  ...
  /**
   * For making a copy of the SpatialMatch for the same trip pattern but for a new trip/block. This
   * is useful when doing things by TripPattern, such as spatial matching. Can then make copies for
   * the other trips that use that TripPattern.
   *
   * @param toCopy The SpatialMatch to copy (except for the trip/block info)
   * @param newTrip The new trip to use for the copy. It is assumed to be for the same trip pattern.
   *     It can be for a separate block.
   */
  public SpatialMatch(SpatialMatch toCopy, Trip newTrip) {
    if (toCopy.getTrip().getTripPattern() != newTrip.getTripPattern())
      logger.error(
          ...
          toCopy,
          toCopy.getTrip().getTripPattern().toShortString(),
    ...
    this.predictedLocation =
        toCopy.computeLocation(toCopy.getIndices(), toCopy.distanceAlongSegment);
  }

  /**
   * Constructs a new SpatialMatch but at the new indices specified. Useful for create a match that
   * is just before or after a stop, which is useful for determining travel time from stop to the
   * next match.
   *
   * @param toCopy
   * @param newIndices
  ...
  public double distanceFromBeginningOfTrip() {
    ...
    if (stopPathIndex != 0) distanceFromFirstTerminal += getDistanceAlongStopPath();
    return distanceFromFirstTerminal;
  }

  /**
   * Determines if this SpatialMatch is further away then the specified distance from the terminal
   * at the beginning and end of the trip. This can be useful when doing things like matching a
   * vehicle to a route where one needs to first be sure that the vehicle is not at a terminal
   * because at the terminal one cannot determine appropriate spatial match.
   *
   * @param distance
   * @return True if match is further away than distance away from terminals of trip
   */
  ...
  /**
   * Returns whether this SpatialMatch is within the specified distance along the path from the end
   * of the trip.
   *
   * @param distance
  ...
  public boolean withinDistanceOfEndOfTrip(double distance) {
    ...
    double distanceFromLastTerminal = tripLength - distanceFromFirstTerminal;
  ...
  /**
   * Returns true if the match is to the stop at the end of the stop path. This can be important
   * because atStop can also be set to the previous stop, which is something very different. If
   * match is not for at a stop then null is returned.
   *
  ...
  /**
   * Returns true if the match is to the stop for the previous stop path. This can be important
   * because atStop can also be set to the stop for the current stop path, which is very different.
   * If match is not for at a stop then null is returned.
   *
  ...
  /**
   * For when need to determine arrival time when vehicle matches to a stop. Returns a SpatialMatch
   * that corresponds to the end of the stop path for the stop indicated by the current
   * SpatialMatch. If the match is for stop for the previous stop path (the vehicle is matching to
   * just beyond the stop) the the previous stop path is used. Only to be called when vehicle is
   * matched to a stop. Logs error and returns null if match is not for at a stop.
   *
  ...
  public SpatialMatch getMatchAdjustedToEndOfPath() {
    ...
    Indices endOfStopPathIndices =
        new Indices(block, indices.getTripIndex(), indices.getStopPathIndex(), segmentIndex);
    return new SpatialMatch(this, endOfStopPathIndices, distanceAlongSegment);
  }

  /**
   * Determines the next StopPath after this match that has a schedule time. Useful for determining
   * things such as real-time schedule adherence.
   *
   * @return
   */
  public SpatialMatch getMatchAtNextStopWithScheduleTime() {
    ...
    List<StopPath> stopPaths = getTrip().getTripPattern().getStopPaths();
    ...
    Indices indicesAtStopWithScheduleTime =
        new Indices(
            getBlock(),
            getTripIndex(),
            stopPathIndex,
  ...
  /**
   * For when need to determine departure time at a stop. Returns a SpatialMatch that corresponds to
   * the beginning of the next stop path for the stop indicated by the current SpatialMatch. Only to
   * be called when vehicle is matched to a stop. Logs error and returns null if match is not for at
   * a stop.
   *
  ...
  public SpatialMatch getMatchAdjustedToBeginningOfPath() {
    ...
    Indices beginningOfStopPathIndices =
        new Indices(block, indices.getTripIndex(), indices.getStopPathIndex(), segmentIndex);
    return new SpatialMatch(this, beginningOfStopPathIndices, distanceAlongSegment);
  }

  /**
   * Returns a SpatialMatch that corresponds to the current match but will be moved to after a stop
   * if the current match is just before the stop. This way can pass in this modified match to the
   * methods that determine travel time and the layover/stop time for the stop where the
   * SpatialMatch is will not be included in the calculations. If the current object is not just
   * before a stop then the original object is returned.
   *
  ...
  private SpatialMatch getMatchAfterStopIfAtStop() {
    ...
    if (!atEndOfPathStop()) return this;
  ...
  /**
   * Returns a SpatialMatch that corresponds to just before the next stop. Useful if need to
   * determine travel time to next stop.
   *
   * @return
   */
  public SpatialMatch getMatchAtJustBeforeNextStop() {
    ...
    Vector segmentVector =
        block.getSegmentVector(m.getTripIndex(), m.getStopPathIndex(), segmentIndex);
  ...
  /**
   * Returns a SpatialMatch that corresponds to the current match but will be moved to before a stop
   * if the current match is just after the stop. This way can pass in this modified match to the
   * methods that determine travel time and the layover/stop time for the stop where the
   * SpatialMatch is will not be included in the calculations. If the current object is not just
   * after a stop then the original object is returned.
   *
  ...
  public SpatialMatch getMatchBeforeStopIfAtStop() {
    ...
    // spatialMatch.
    if (!atBeginningOfPathStop()) return this;
    ...
    return new SpatialMatch(this, previousPathIndices, previousPathIndices.getSegment().length());
  }

  /**
   * Returns a SpatialMatch that corresponds to the previous stop. The returned match will be just
   * before the previous stop. Useful if need to determine travel time from previous stop.
   *
  ...
  public SpatialMatch getMatchAtPreviousStop() {
    ...
    if (indices.beforeBeginningOfBlock()) return null;
    ...
    Vector segmentVector =
        indices
            .getBlock()
            .getSegmentVector(
                indices.getTripIndex(), indices.getStopPathIndex(), indices.getSegmentIndex());
  ...
  /**
   * Determines if the match means that the vehicle is considered to be at a stop. Looks forward and
   * back from the match to see if it is within the allowable distance of the stop. Intended to be
   * called by constructor to set the atStop member variable so that only have to determine if
   * SpatialMatch is at the stop once.
   *
   * @return VehicleAtStopInfo object containing trip and path index of stop that match is nearby.
   *     Returns null if match is not near a stop.
   */
  private VehicleAtStopInfo atStop() {
    ...
    if (!previousPathIndices.beforeBeginningOfBlock()) {
      ...
      if (previousStopPath != null
          && getDistanceAlongStopPath() < previousStopPath.getAfterStopDistance())
  ...
  /**
   * A layover stop is when a vehicle can leave route path before departing this stop since the
   * driver is taking a break.
   *
   * @return true if this spatial match is for a layover
   */
  public boolean isLayover() {
    if (block.isNoSchedule())
      return false; // Frequency-based unscheduled blocks can't have layovers
    return block.isLayover(tripIndex, stopPathIndex);
  }

  /**
   * Wait stop is when a vehicle is not supposed to depart the stop until the scheduled departure
   * time.
   *
   ...
   *
   * @return the scheduled time vehicle is to leave the stop. Time is seconds into day. Returns -1
   *     if not at a stop or the stop doesn't have a scheduled departure time.
   */
  public int getScheduledWaitStopTimeSecs() {
    try {
      ...
      logger.error(
          "Tried to get wait stop time for a stop that didn't " + "have one. {} {}", this, e, e);
      return -1;
    }
  }
  ...
  public long getScheduledWaitStopTime() {
    ...
    long scheduledDepartureTime =
        Core.getInstance().getTime().getEpochTime(secondsIntoDay, avlTime);
  ...
  public double distanceBetweenMatches(SpatialMatch otherSpatialMatch) {
    ...
    double distanceAlongLastPath = otherSpatialMatch.getDistanceAlongStopPath();
    ...
    double distanceBetweenMatches =
        totalStopPathDistances - distanceAlongFirstPath + distanceAlongLastPath;
    return distanceBetweenMatches;
  }

  /**
   * Determines if when going from this spatial match to the otherSpatialMatch whether a wait stop
   * is traversed. Also returns true if just sitting at a wait stop.
   *
   * @param otherSpatialMatch
  ...
  public boolean traversedWaitStop(SpatialMatch otherSpatialMatch) {
    ...
    if (atStop != null && atStop.isWaitStop()) return true;
    VehicleAtStopInfo endAtStop = otherSpatialMatch.getAtStop();
    if (endAtStop != null && endAtStop.isWaitStop()) return true;
  ...
  /**
   * Determines the indices for the position the specified distance before this match. Useful for
   * seeing if looking how far past a stop the match is.
   *
  ...
  /**
   * Returns true if this is before or equal to the other SpatialMatch passed in.
   *
   * @param other The Spatial Match to compare to
  ...
  /**
   * Returns the total number of stops traversed between the two matches.
   *
   * @param match1
   * @param match2
   * @return Number of stops
   */
  public static int numberStopsBetweenMatches(SpatialMatch match1, SpatialMatch match2) {
    ...
    for (int tripIndex = match1.getTripIndex(); tripIndex < match2.getTripIndex(); ++tripIndex) {
      numIntermediateTripsStops += block.getTrip(tripIndex).getNumberStopPaths();
    }
    ...
    int totalStopsBetweenMatches =
        stopIdxInLastTrip - stopIdxInFirstTrip + numIntermediateTripsStops;
  ...
  /**
   * Returns copy of the indices of the match as an Indices object. The Indices object is newly
   * created and independent so it can be changed (incremented and decremented) at will.
   *
   * @return
   */
  ...
  /**
   * Can be either at stop at beginning of the stop path or at the stop at the end of the stop path.
   *
   * @return VehicleAtStopInfo object containing trip and path index of stop that match is nearby.
   *     Returns null if match is not near a stop.
   */
  ...
  /**
   * Returns the VehicleAtStopInfo if match indicates that vehicle is at the stop at the end of the
   * path.
   *
   * @return VehicleAtStopInfo if vehicle at stop at end of stop path. Otherwise null.
   */
  ...
  /**
   * Returns the VehicleAtStopInfo if match indicates that vehicle is at the stop at the beginning
   * of the path.
   *
  ...
  /**
   * Returns true if vehicle is at or near a stop. Can be either at stop at beginning of the stop
   * path or at the stop at the end of the stop path.
   *
  ...
  /**
   * Returns true if this match indicates that the vehicle is at the stop indicated by the tripIndex
   * and stopPathIndex parameters.
   *
   * @param tripIndex
   * @param stopPathIndex
  ...
  public boolean isAtStop(int tripIndex, int stopPathIndex) {
    return atStop != null && atStop.equals(atStop.getBlock().getId(), tripIndex, stopPathIndex);
  }

  /**
   * Returns the epoch time of the AVL report for which this match was created.
   *
