...
public class MatchProcessor {
    ...
    if (CoreConfig.getMaxPredictionsTimeForDbSecs() > 0) {
      for (IpcPrediction prediction : newPredictions) {
        ...
        if (prediction.getPredictionTime() - prediction.getAvlTime()
          ...
          logger.debug(
              "Difference in predictionTiem and AVLTime is {} and is greater than getMaxPredictionsTimeForDbSecs {}.",
  ...
  /**
   * Generates the arrival/departure info based on the new match stored in the vehicle state. Stores
   * the arrival/departure info into database.
   *
   * @param vehicleState
   */
  ...
  public void generateResultsOfMatch(VehicleState vehicleState) {
    ...
    if (!vehicleState.isPredictable()) {
      logger.error(
          "Vehicle was unpredictable when "
