...
public class TemporalDifference implements Serializable {
  ...
  /**
   * @param temporalDifferenceMsec Positive means vehicle is ahead of where expected, negative means
   *     behind.
   */
  ...
  /**
   * Often working with longs for time so it is convenient to have a constructor that will accept a
   * long as the temporal difference.
   *
   * @param temporalDifferenceMsec Positive means vehicle is ahead of where expected, negative means
   *     behind.
   */
   ...
   * vehicle. Need to be more restrictive than for other matching since the initial matching is more
   * difficult. Need to only match if it is a reasonable match since vehicles do really peculiar
   * things.
   *
   * @return true if within bounds
   */
  public boolean isWithinBoundsForInitialMatching() {
    return temporalDifferenceMsec
            < CoreConfig.getAllowableEarlySecondsForInitialMatching() * Time.MS_PER_SEC
  ...
  /**
   * Returns an easily comparable positive value for the temporal difference that takes into account
   * that being early is far worse and less likely than being late.
   *
   * @return
   */
  ...
  /**
   * Returns the temporalDifferenceMsec in msec. Positive means vehicle is early and negative means
   * that vehicle is late.
   *
   * @return
   */
  ...
  public String toString() {
    ...
    if (temporalDifferenceMsec > 0) str += " (early)";
    else if (temporalDifferenceMsec == 0) str += " (ontime)";
    else str += " (late)";
