...
public class AvlProcessor {
   ...
   */
  public void makeVehicleUnpredictable(
  ...
  public void matchNewFixForPredictableVehicle(VehicleState vehicleState) {
    ...
    if (!vehicleState.isPredictable() || vehicleState.getMatch() == null) {
      throw new RuntimeException(
          "Called AvlProcessor.matchNewFix() "
    ...
    logger.debug(
        "Matching already predictable vehicle using new AVL "
    ...
    logger.info(
        "For vehicleId={} the best match is {}", vehicleState.getVehicleId(), bestTemporalMatch);
    ...
    if (bestTemporalMatch != null || vehicleState.overLimitOfBadMatches()) {
      ...
      if (bestTemporalMatch != null || !vehicleState.overLimitOfBadMatches()) {
        ...
        makeVehicleUnpredictable(
            vehicleState.getVehicleId(), eventDescription, VehicleEvent.NO_MATCH);
  ...
  /**
   * When matching a vehicle to a route we are currently assuming that we cannot make predictions or
   * match a vehicle to a specific trip until after vehicle has started on its trip. This is because
   * there will be multiple trips per route and we cannot tell which one the vehicle is on time wise
   ...
   *
   * @param bestMatch The TemporalMatch that the vehicle was matched to. If set then vehicle will be
   *     made predictable and if block assignments are to be exclusive then any old vehicle on that
   *     assignment will be have its assignment removed. If null then vehicle will be configured to
   *     be not predictable.
  ...
  private void updateVehicleStateFromAssignment(
    ...
    if (bestMatch != null) {
      ...
      String eventDescription =
          "Vehicle successfully matched to "
              + assignmentType
  ...
  /**
   * compare the match to the avl location and log if they differ greatly. Note we just log this, we
   * do not make the vehicle unpredictable.
   *
   * @param bestMatch
   * @param vehicleState
   */
  private void logConflictingSpatialAssigment(TemporalMatch bestMatch, VehicleState vehicleState) {
    ...
    if (vehicleState.isPredictable() && deltaDistance > getMaxMatchDistanceFromAVLRecord()) {
      String eventDescription =
          "Vehicle match conflict from AVL report of "
   ...
   * Updates the VehicleState with the new block assignment and match. These will be null if vehicle
   * could not successfully be matched to route.
   *
   * @param routeId
   * @param vehicleState
  ...
  private boolean matchVehicleToRouteAssignment(String routeId, VehicleState vehicleState) {
    ...
    if (routeId == null) {
      logger.error(
          "matchVehicleToRouteAssignment() called with null " + "routeId. {}", vehicleState);
    }
    ...
    for (String serviceId : serviceIds) {
      List<Block> blocksForService =
          Core.getInstance().getDbConfig().getBlocksForRoute(serviceId, routeId);
    ...
    for (Block block : allBlocksForRoute) {
      ...
      List<SpatialMatch> spatialMatchesForBlock =
          SpatialMatcher.getSpatialMatches(
              vehicleState.getAvlReport(),
              block,
              potentialTrips,
   ...
   * new block assignment and match. These will be null if vehicle could not successfully be matched
   * to block.
   *
   * @param block
   * @param vehicleState
  ...
  private boolean matchVehicleToBlockAssignment(Block block, VehicleState vehicleState) {
    ...
    logger.debug(
        "Matching unassigned vehicle to block assignment {}. {}", block.getId(), vehicleState);
    ...
    TemporalMatch bestMatch =
        TemporalMatcher.getInstance()
            .getBestTemporalMatchComparedToSchedule(avlReport, spatialMatches);
    ...
    if (bestMatch == null) {
      ...
      Trip trip =
          TemporalMatcher.getInstance().matchToLayoverStopEvenIfOffRoute(avlReport, potentialTrips);
      if (trip != null) {
    ...
    if (bestMatch != null
        && matchProblematicDueOtherVehicleHavingAssignment(bestMatch, vehicleState)) {
      logger.error(
   ...
   * vehicle seems to be more appropriate. Match is problematic if 1) exclusive matching is enabled,
   * 2) other non-schedule based vehicle already has the assignment, 3) the other vehicle that
   * already has the assignment isn't having any problems such as vehicle being delayed, and 4) the
   * new match is far away from the route which implies that it might be a mistaken login.
   *
   ...
   * the route and vehicle 8151 already had that assignment and actually was on the route. This can
   * happen if driver enters wrong assignment, or perhaps if they never log out.
   *
   * @param match
   * @param vehicleState
  ...
  private boolean matchProblematicDueOtherVehicleHavingAssignment(
    ...
    for (String vehicleId : vehiclesAssignedToBlock) {
      ...
      VehicleState otherVehicleState =
          VehicleStateManager.getInstance().getVehicleState(otherVehicleId);
  ...
  /**
   * If the block assignment is supposed to be exclusive then looks for any vehicles assigned to the
   * specified block and removes the assignment from them. This of course needs to be called before
   * a vehicle is assigned to a block since *ALL* vehicles assigned to the block will have their
   * assignment removed.
   *
   * @param block
      ...
      if (block.shouldBeExclusive() || vehicleState.isForSchedBasedPreds()) {
        String description =
            "Assigning vehicleId="
                ...
                + ".";
        logger.info(description);
        makeVehicleUnpredictableAndGrabAssignment(
            vehicleState, description, VehicleEvent.ASSIGNMENT_GRABBED);
      }
    }
  }
  ...
  private void handlePredictableVehicleWithoutAvlAssignment(VehicleState vehicleState) {
    ...
    if (vehicleState.getBadAssignmentsInARow() < allowableBadAssignments.getValue()) {
      ...
      makeVehicleUnpredictable(
          vehicleState.getVehicleId(), eventDescription, VehicleEvent.ASSIGNMENT_CHANGED);
    }
  }
   ...
   * should try to automatically assign the vehicle based on how it matches to a currently unmatched
   * block. If it can match the vehicle then this method fully processes the match, generating
  ...
  private boolean automaticalyMatchVehicleToAssignment(VehicleState vehicleState) {
    ...
    if (bestMatch != null) {
      ...
      logger.info(
          "Auto matched vehicleId={} to a block assignment. {}",
          vehicleState.getVehicleId(),
          bestMatch);
   ...
   * gotten too many bad assignments in a row then simply use the old assignment. This is handy for
   * when the assignment portion of the AVL feed does not send assignment data for every report.
   *
   * @param vehicleState
   */
  ...
  private boolean handlePossibleEndOfBlock(VehicleState vehicleState) {
    ...
    if (temporalMatch != null) {
      ...
      if (atStopInfo != null && atStopInfo.atEndOfBlock()) {
        logger.info(
            "For vehicleId={} the end of the block={} "
  ...
  /**
   * If schedule adherence is not within bounds then will try to match the vehicle to the assignment
   * again. This can be important if system is run for a while and then paused and then started up
   * again. Vehicle might continue to match to the pre-paused match, but by then the vehicle might
   * be on a whole different trip, causing schedule adherence to be really far off. To prevent this
   * the vehicle is re-matched to the assignment.
   *
  ...
  private void verifyRealTimeSchAdh(VehicleState vehicleState) {
    ...
    logger.debug(
        "Confirming real-time schedule adherence for vehicleId={}", vehicleState.getVehicleId());
    ...
    if (scheduleAdherence != null && !scheduleAdherence.isWithinBounds()) {
      ...
      String eventDescription =
          "Vehicle had schedule adherence of "
              + scheduleAdherence
  ...
  private void lowLevelProcessAvlReport(AvlReport avlReport, boolean recursiveCall) {
    ...
    synchronized (vehicleState) {
      ...
      boolean matchToNewAssignment =
          avlReport.hasValidAssignment()
              && (!vehicleState.isPredictable() || vehicleState.hasNewAssignment(avlReport))
   ...
   * continues to feed old data that data will be ignored. In other words, the last AVL time will be
   * for that last valid AVL report.
   *
  ...
  /**
   * For storing the last regular (non-schedule based) AvlReport so can determine if the AVL feed is
   * working. Makes sure that report is newer than the previous last regular report so that ignore
   ...
   * update VehicleState AVL report but don't want to actually process the report, such as for when
   * get data too frequently and only want to fully process some of it yet still use latest vehicle
  ...
  public void cacheAvlReportWithoutProcessing(AvlReport avlReport) {
    VehicleState vehicleState =
        VehicleStateManager.getInstance().getVehicleState(avlReport.getVehicleId());
   ...
   * report by matching to the assignment and generating predictions and such. Sets VehicleState for
   * the vehicle based on the results. Also stores AVL report into the database (if not in playback
   * mode).
   *
   * @param avlReport The new AVL report to be processed
   */
  public void processAvlReport(AvlReport avlReport) {
    ...
    if (AvlConfig.shouldLogToStdOut()) {
      System.err.println(
          "Processing avlReport for vehicleId="
              + avlReport.getVehicleId()
              +
