 ...
 * get from one point to another on the assignment. Heavily used both for doing temporal matching
...
public class TravelTimes {
  ...
  /**
   * Determines travel time as the crows flies to cover the distance. Intended to be used for seeing
   * if have enough time to deadhead to a breakpoint. Just a very crude approximation since the
   * distance is as the crow flies but vehicle will actually be traveling on various roads. And we
   * don't know how fast it can really travel on those roads. But doing the best we can here.
   *
   * <p>Uses a two step approach since sometimes vehicles can travel fast on highways if they are
   * far away but will likely travel slower when closer.
   *
  ...
  public static int travelTimeAsTheCrowFlies(double distance) {
    ...
    double veryRoughTravelTimeSecs =
        shortDistanceTravel / SHORT_DISTANCE_SPEED + longDistanceTravel / LONG_DISTANCE_SPEED;
  ...
  public static int travelTimeFromLayoverArrivalToNewLoc(
    ...
    Location endOfTripLocation = matchAtPreviousStop.getStopPath().getEndOfPathLocation();
   ...
   * @return
   */
  private static int adjustTravelTimeForWaitStop(
      int timeOfDaySecs, int travelTimeMsec, Indices indices) {
  ...
  /**
   * Returns the scheduled epoch time vehicle is scheduled leave a wait stop. Does not take into
   * account whether vehicle can make it to the wait stop in time.
   *
   * @param indices Describes which stop
   * @param referenceTime For converting seconds into day schedule time to an epoch time. Just needs
   *     to be somewhat close to the time.
  ...
  public static long scheduledDepartureTime(Indices indices, long referenceTime) {
    ...
    if (!indices.isWaitStop()) {
      logger.error(
          "Called scheduledDepartureTimePlusWaitTime() for stop " + "that is not a wait stop. {}",
          indices);
      return -1;
    }
    ...
    if (scheduleTime == null) {
      logger.error(
          "Called scheduledDepartureTimePlusWaitTime() for stop "
              + "that doesn't have a schedule time. {}",
          indices);
      return -1;
    }
    Integer scheduledDepartureTimeSecs = scheduleTime.getDepartureTime();
    ...
    long scheduleEpochTime =
        timeUtil.getEpochTime(scheduledDepartureTimeSecs, new Date(referenceTime));
  ...
  private TimeTravelInfo travelTimeInfoForPartialPath(SpatialMatch match) {
    ...
    int numTravelTimeSegmentsInPath = travelTimesForStopPath.getNumberTravelTimeSegments();
    double travelTimeSegmentLength = stopPathLength / numTravelTimeSegmentsInPath;

    double distanceAlongStopPath = match.getDistanceAlongStopPath();
    int indexOfPartialTimeTravelSegment = (int) (distanceAlongStopPath / travelTimeSegmentLength);
    ...
    double distanceCompletedOfTimeTravelSegment =
        distanceAlongStopPath - indexOfPartialTimeTravelSegment * travelTimeSegmentLength;
  ...
  /**
   * Returns the time vehicle is expected to take to travel from the spatialMatch to the stop at the
   * end of the current stop path. Does not include the stop time for the stop at the end of the
   * stop path.
   *
   * @param match The starting point
  ...
  /**
   * Returns the time vehicle is expected to take to travel from the beginning of the stop path to
   * the spatialMatch. Only includes travel time, not time expected to be at the stop.
   *
   * @param match The end point
   * @return Expected travel time in msec
   */
  public int expectedTravelTimeFromBeginningOfStopPathToMatch(SpatialMatch match) {
    ...
    int travelTimeForPartialSegment =
        travelTimesForStopPath.getTravelTimeSegmentMsec(timeTravelInfo.indexOfPartialSegment);
    int travelTimeInPartialSegmentToMatch =
        (int) (travelTimeForPartialSegment * timeTravelInfo.fractionCompleted);
  ...
  /**
   * Returns travel time for the path specified by the indices parameter. Does not include the stop
   * time.
   *
   * @param indices Specifies which path
   * @return Expected travel time in msec
   */
  ...
  /**
   * Returns how long vehicle is expected to stop for the stop at the end of the path specified by
   * the indices parameter.
   *
   * @param indices Which path to return stop time for.
  ...
  /**
   * Determines expected travel time in msec between the two matches based on the travel times from
   * the database.
   *
  ...
  public int expectedTravelTimeBetweenMatches(
    ...
    if (indices.equalStopPath(endIndices)) {
      ...
      logger.debug(
          "For vehicleId={} both matches are on same stop "
    ...
    // complicated!
    boolean specialNoSchedCase =
        indices.getBlock().isNoSchedule()
    ...
    if (!specialNoSchedCase) {
      ...
      while (indices.isEarlierStopPathThan(endIndices)) {
        ...
        logger.debug(
            "For vehicleId={} adding stop path travel time={} "
        ...
        travelTimeMsec += stopTimeMsec;
        logger.debug(
            "For vehicleId={} adding stop time={} msec so " + "travel time now is {} msec for {}",
            vehicleId,
            stopTimeMsec,
            travelTimeMsec,
            indices);
    ...
    int travelTimeForPartialLastStopPath = expectedTravelTimeFromBeginningOfStopPathToMatch(match2);
    travelTimeMsec += travelTimeForPartialLastStopPath;
    logger.debug(
        "For vehicleId={} added travel time for last "
            + "partial stop path of {} msec so now travel time is {} msec."
            + "match2 indices={}",
        vehicleId,
        travelTimeForPartialLastStopPath,
        travelTimeMsec,
        endIndices);

    // Return the results
    logger.debug(
        "For vehicleId={} returning total travel time={} msec. {}",
        vehicleId,
        travelTimeMsec,
        match2);
    return travelTimeMsec;
  ...
  /**
   * Determines expected travel time in msec between the two matches based on the travel times from
   * the database. Same as other travelTimeBetweenMatches() method but uses Date to determine
   * seconds into day. Therefore it can be simpler to use since AvlReports and such have a Date.
   * Returns travel time in msec.
   *
   * @param vehicleId for logging messages
   * @param time
   * @param match1
   ...
   */
  public int expectedTravelTimeBetweenMatches(
      String vehicleId, Date time, SpatialMatch match1, SpatialMatch match2) {
