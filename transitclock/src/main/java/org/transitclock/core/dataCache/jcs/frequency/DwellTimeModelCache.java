...
public class DwellTimeModelCache
  ...
  private static IntegerConfigValue maxDwellTimeAllowedInModel =
  ...
  public synchronized void addSample(ArrivalDeparture event, Headway headway, long dwellTime) {
    ...
    Integer time =
        FrequencyBasedHistoricalAverageCache.secondsFromMidnight(event.getFreqStartTime(), 2);
    ...
    StopPathCacheKey key =
        new StopPathCacheKey(event.getTripId(), event.getStopPathIndex(), false, new Long(time));
        ...
        if (!event.getVehicleId().equals(arrival.getVehicleId())) {
          if (!event.getTripId().equals(arrival.getTripId())) {
            if (event.getStopId().equals(arrival.getStopId())) {
              if (event.getTime().getTime() < arrival.getTime().getTime()) return event;
            }
          }
