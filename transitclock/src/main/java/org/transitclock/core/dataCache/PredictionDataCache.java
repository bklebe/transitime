 ...
 *
 * <p>Concurrency and thread safety is very important in this class since will have multiple threads
 * writing and reading predictions. Need to make sure that the predictions obtained are always
 * complete, even if another thread is simultaneously updating predictions by removing one and then
 * adding an updated prediction. Otherwise might obtain predictions when the old one has been
 * removed but the new one has not yet been written, causing a prediction to be missed.
 *
 * <p>For concurrency and thread safety using ConcurrentHashMap instead of HashMap.
 *
 * <p>getPredictions() returns a copy of the list of predictions for the route/stop. This way the
 * contents will always be coherent and the caller does not need to synchronize, which would be
...
public class PredictionDataCache {
  ...
  protected static BooleanConfigValue returnArrivalPredictionForEndOfTrip =
  ...
  /**
   * Returns copy of the PredictionsForRouteStop object. This is the low-level method that actually
   * gets the appropriate predictions. A clone is used so that it can be accessed as needed without
   * worrying about another thread writing to it. The list of predictions should be relatively small
   * so cloning is not very costly. And this way the caller of this method doesn't have to
   * synchronize or such.
   *
   * @param routeIdOrShortName route_id or route_short_name, or null to specify all routes for stop.
   * @param directionId Optional parameter for specifying that only want predictions for a specific
   *     directionId. Set to null if want predictions for all directions.
   * @param stopIdOrCode stop_id or stop_code
  ...
  public List<IpcPredictionsForRouteStopDest> getPredictions(
    ...
    if (routeIdOrShortName != null) {
      ...
      if (route != null)
        ...
        if (dbConfig.getRouteByShortName(routeIdOrShortName) == null) {
          ...
          throw new IllegalArgumentException("Route " + routeIdOrShortName + " not valid");
        }
      }
    }
    ...
    if (dbConfig.getStop(stopIdOrCode) == null) {
      try {
        ...
        throw new IllegalArgumentException("Stop " + stopIdOrCode + " not a valid integer");
      }
    }
    ...
    for (IpcPredictionsForRouteStopDest predictions : predictionsForRouteStop) {
      for (IpcPrediction preds : predictions.getPredictionsForRouteStop()) {
        if (preds.isAtEndOfTrip()) endOfTripPredFound = true;
        else nonEndOfTripPredFound = true;
      }
    }
    ...
    boolean shouldFilterOutEndOfTripPreds =
        (endOfTripPredFound
            && nonEndOfTripPredFound
            && !returnArrivalPredictionForEndOfTrip.getValue());
    ...
    for (IpcPredictionsForRouteStopDest predictions : predictionsForRouteStop) {
      ...
      if (directionId != null && !directionId.equals(predictions.getDirectionId())) continue;
      ...
      if (shouldFilterOutEndOfTripPreds) {
        if (predictions.getPredictionsForRouteStop().size() > 0) {
          ...
          if (allPredsForEndOfTrip) continue;
        }
      }

      // Direction ID is OK so clone prediction and add to list
      IpcPredictionsForRouteStopDest clone =
          predictions.getClone(maxPredictionsPerStop, maxPredictionEpochTime, distanceToStop);
    ...
    if (clonedPredictions.size() == 0) {
      IpcPredictionsForRouteStopDest pred =
          new IpcPredictionsForRouteStopDest(
              routeShortName, directionId, stopIdOrCode, distanceToStop);
  ...
  /**
   * Returns copy of the PredictionsForRouteStop object. A clone is used so that it can be accessed
   * as needed without worrying about another thread writing to it. The list of predictions should
   * be relatively small so cloning is not very costly. And this way the caller of this method
  ...
  public List<IpcPredictionsForRouteStopDest> getPredictions(
    ...
    return getPredictions(
        routeIdOrShortName, directionId, stopIdOrCode, maxPredictionsPerStop, Double.NaN);
  }

  /**
   * Returns copy of all predictions currently associated with the stop. Uses routeShortName instead
   * of the GTFS routeId to identify the stop.
   *
  ...
  public List<IpcPredictionsForRouteStopDest> getPredictions(
    ...
    return getPredictions(routeIdOrShortName, directionId, stopId, Integer.MAX_VALUE);
  }
  ...
  /**
   * Returns copy of List<PredictionsForRouteStop> objects for each route/stop specified.
   *
   * @param routeStops Specified using route_short_name or route_id, and stop_id or stop_code
  ...
  public List<IpcPredictionsForRouteStopDest> getPredictions(
    ...
    for (RouteStop routeStop : routeStops) {
      List<IpcPredictionsForRouteStopDest> predsForStop =
          getPredictions(
              routeStop.getRouteIdOrShortName(),
              null,
              routeStop.getStopIdOrCode(),
              predictionsPerStop);
  ...
  /**
   * Gets predictions for specified route/stop and returns the first one for the specified vehicle.
   *
   * @param vehicleId
   * @param routeShortName
   * @param stopId
   * @return IpcPrediction for the specified vehicle/route/stop or null if there are no such
   *     predictions
   */
  public IpcPrediction getPredictionForVehicle(
      String vehicleId, String routeShortName, String stopId) {
    ...
    List<IpcPredictionsForRouteStopDest> predsList =
        getPredictions(routeShortName, null, stopId, 5 /* maxPredictionsPerStop */);
  ...
  /**
   * Returns copy of all predictions for system, limited by maxPredictionsPerStop and
   * maxPredictionTime.
   *
   * @param maxPredictionsPerStop Maximum number of predictions per route/stop/destination to clone.
   * @param maxSystemTimeForPrediction Max point in future want predictions for. This way can limit
  ...
  /**
   * Updates predictions in the cache that are associated with a vehicle. Removes any that are in
   * oldPredictionsForVehicle and adds all the ones in newPredictionsForVehicle.
   *
   * @param oldPredictionsForVehicle The old predictions to be removed.
   * @param newPredictionsForVehicle The new predictions. Can be null if only removing old
   *     predictions
   */
  ...
  /**
   * Removes old prediction from the map. For when there is no new prediction for the vehicle for
   * the route/stop.
   *
   * @param oldPrediction
   */
  ...
  /**
   * Gets the prediction list for the route/stop/destination, synchronizes it so that changes are
   * threadsafe, and then updates the list with the new predictions. Each route/stop will usually
   * get only a single prediction but there are situations where a vehicle will hit a stop more than
   * once with the max time that predictions are generated for. For such a case need to add all of
   * those predictions at once.
   *
   * @param newPredsForVehicleForRouteStopDest the new predictions to be set for the
  ...
  private void updatePredictionsForVehicle(List<IpcPrediction> newPredsForVehicleForRouteStopDest) {
    ...
    logger.debug(
        "Adding predictions for the route/stop/destination: {}",
  ...
  /**
   * Returns List of PredictionsForRouteStop objects associated with the specified route/stop.
   * Returns a list because there is a separate PredictionsForRouteStop for each destination and
   * some route directions have multiple destinations.
   *
   * @param routeShortName The route short name. Set to null to get predictions for all routes for
   *     the stop.
   * @param stopId
  ...
  /**
   * Returns PredictionsForRouteStop object associated with the specified route/stop/destination
   * specified by the trip and stopId parameters.
   *
   * <p>The returned PredictionsForRouteStop objects needs to be synchronized when predictions
   * accessed. Otherwise might access temporarily corrupt state when a prediction is being updated.
   *
   * @param trip
   * @param stopId
  ...
  /**
   * Returns PredictionsForRouteStop object associated with the specified route/stop/destination
   * specified by the prediction parameter.
   *
   * @param prediction for specifying route and stop for which to get the prediction list for.
   * @return The PredictionsForRouteStop for the specified route/stop/destination
   */
  private IpcPredictionsForRouteStopDest getPredictionsForRouteStopDestination(
      IpcPrediction prediction) {
    return getPredictionsForRouteStopDestination(prediction.getTrip(), prediction.getStopId());
  }
