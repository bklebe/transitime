...
public class FrequencyBasedHistoricalAverageCache {
  ...
  private static FrequencyBasedHistoricalAverageCache singleton =
      new FrequencyBasedHistoricalAverageCache();

  private static final Logger logger =
      LoggerFactory.getLogger(FrequencyBasedHistoricalAverageCache.class);

  private static IntegerConfigValue minTravelTimeFilterValue =
  ...
  private static IntegerConfigValue maxTravelTimeFilterValue =
      new IntegerConfigValue(
  ...
  private static IntegerConfigValue minDwellTimeFilterValue =
      new IntegerConfigValue(
  ...
  private static IntegerConfigValue maxDwellTimeFilterValue =
      new IntegerConfigValue(
  ...
  public synchronized HistoricalAverage getAverage(StopPathCacheKey key) {
    ...
    logger.debug(
        "Looking for average for : {} in FrequencyBasedHistoricalAverageCache cache.", key);
    ...
    if (result != null) {
      ...
      if (key.getStartTime() != null) {
        SortedMap<Long, HistoricalAverage> subresult =
            result.subMap(
                key.getStartTime(), key.getStartTime() + getCacheIncrementsForFrequencyService());

        if (subresult.size() == 1) {
          logger.debug(
              "Found average for : {} in FrequencyBasedHistoricalAverageCache cache with a value : {}",
              key,
              subresult.get(subresult.lastKey()));
          return subresult.get(subresult.lastKey());
        } else {
          logger.debug(
              "No historical data within time range ({} to {}) for this trip {} in FrequencyBasedHistoricalAverageCache cache.",
              key.getStartTime(),
              key.getStartTime() + getCacheIncrementsForFrequencyService(),
              key);
        }
      }
    }
  ...
  public synchronized void putArrivalDeparture(ArrivalDeparture arrivalDeparture) throws Exception {
    ...
    if (trip != null && trip.isNoSchedule()) {
      ...
      TravelTimeResult pathDuration =
          getLastPathDuration(new IpcArrivalDeparture(arrivalDeparture), trip);

      if (pathDuration != null
          && pathDuration.getDuration() > minTravelTimeFilterValue.getValue()
          && pathDuration.getDuration() < maxTravelTimeFilterValue.getValue()) {
        if (trip.isNoSchedule()) {
          StopPathCacheKey historicalAverageCacheKey =
              new StopPathCacheKey(
          ...
          logger.debug(
              "Putting : {} in FrequencyBasedHistoricalAverageCache cache for key : {} which results in : {}.",
              pathDuration,
      ...
      DwellTimeResult stopDuration =
          getLastStopDuration(new IpcArrivalDeparture(arrivalDeparture), trip);
      if (stopDuration != null
        ...
        StopPathCacheKey historicalAverageCacheKey =
            new StopPathCacheKey(
                trip.getId(),
                stopDuration.getDeparture().getStopPathIndex(),
                false,
                new Long(time));
      ...
      if (stopDuration == null && pathDuration == null) {
        logger.debug(
            "Cannot add to FrequencyBasedHistoricalAverageCache as cannot calculate stopDuration or pathDuration. : {}",
            arrivalDeparture);
      }
      if (pathDuration != null
          && (pathDuration.getDuration() < minTravelTimeFilterValue.getValue()
              || pathDuration.getDuration() > maxTravelTimeFilterValue.getValue())) {
        logger.debug(
            "Cannot add to FrequencyBasedHistoricalAverageCache as pathDuration: {} is outside parameters. : {}",
            pathDuration,
            arrivalDeparture);
      }
      if (stopDuration != null
          && (stopDuration.getDuration() < minDwellTimeFilterValue.getValue()
              || stopDuration.getDuration() > maxDwellTimeFilterValue.getValue())) {
        logger.debug(
            "Cannot add to FrequencyBasedHistoricalAverageCache as stopDuration: {} is outside parameters. : {}",
            stopDuration,
            arrivalDeparture);
      }
    } else {
      logger.debug(
          "Cannot add to FrequencyBasedHistoricalAverageCache as no start time set : {}",
          arrivalDeparture);
    }
  }
  ...
  private TravelTimeResult getLastPathDuration(IpcArrivalDeparture arrivalDeparture, Trip trip) {
    Date nearestDay =
        DateUtils.truncate(new Date(arrivalDeparture.getTime().getTime()), Calendar.DAY_OF_MONTH);
    ...
    if (arrivalDepartures != null && arrivalDepartures.size() > 0 && arrivalDeparture.isArrival()) {
      IpcArrivalDeparture previousEvent =
  ...
  private DwellTimeResult getLastStopDuration(IpcArrivalDeparture arrivalDeparture, Trip trip) {
    ...
    if (arrivalDepartures != null
        && arrivalDepartures.size() > 0
        && arrivalDeparture.isDeparture()) {
      IpcArrivalDeparture previousEvent =
  ...
  private class TravelTimeResult {
    ...
    public TravelTimeResult(
        IpcArrivalDeparture previousEvent, IpcArrivalDeparture arrivalDeparture) {
