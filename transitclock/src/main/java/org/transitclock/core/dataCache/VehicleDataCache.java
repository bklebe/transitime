 ...
 * VehicleState is changed. The info is stored in an immutable IpcExtVehicle object since
 * VehicleState changes dynamically and is therefore not always coherent. This way the info
 * transmitted to clients will always be coherent without having to synchronize VehicleState for
 * when converting to a IpcExtVehicle. Organizes vehicles info by vehicle ID but also by route so
 * can easily determine which vehicles are associated with a route.
 *
 * @author SkiBu Smith
 */
public class VehicleDataCache {
  ...
  /**
   * Reads in vehicle config data from db. Unsynchronized since the calling methods are expected to
   * sync.
   */
  ...
  /**
   * Reads in vehicle config data from db if more than 5 minutes since last db read. Useful for when
   * vehicle data has been updated in db.
   */
  ...
  /**
   * To be called when vehicle encountered in AVL feed. Adds the vehicle to the cache and stores in
   * database if haven't done so yet.
   *
   * @param avlReport
   */
  public void cacheVehicleConfig(AvlReport avlReport) {
    ...
    if (absent == null) {
      logger.info(
          "Encountered new vehicle where vehicleId={} so "
  ...
  /**
   * Returns the VehicleConfig for the specified trackerId. Useful for when getting a GPS feed that
   * has a tracker ID, like an IMEI or phone #, instead of a vehicle ID. Allows the corresponding
   * vehicleId to be determined from the VehicleConfig object.
   *
   * <p>If trackerId not found in VehicleConfig data then the VehicleData is reread from the db.
   * This way if a new vehicle is configured don't need to restart the core system in order to
   * determine the vehicle ID.
   *
   * @param trackerId
   * @return VehicleConfig for the trackerId, or null if there isn't one configured.
   */
  ...
  /**
   * Filters out vehicle info if last GPS report is too old. Doesn't filter out vehicles at layovers
   * though because for those won't get another report for a long time. This includes schedule based
   * vehicles.
   *
   * @param vehicles
   * @return
  ...
  /**
   * This is intended to be used, when the vehicle maps are read, in order to remove schedule based
   * vehicles from a collection.
   *
   * @param vehicles collection of vehicles to investigate
  ...
  /**
   * Returns Collection of Vehicles currently associated with specified route. Filters out info more
   * than MAX_AGE_MSEC since it means that the info is obsolete and shouldn't be displayed. Returns
   * null if no vehicles for specified route.
   *
   * @param routeIdOrShortName Specifies which route to return vehicle data for. Can be a
   *     route_short_name or a route_id. Can also be null or empty string to retrieve vehicles that
   *     are not assigned to a route.
   * @return Collection of IpcExtVehicle for vehicles on route, or null if no vehicles for the
   *     route.
   */
  public Collection<IpcVehicleComplete> getVehiclesForRoute(String routeIdOrShortName) {
    ...
    if (routeShortName != null && routeShortName.isEmpty()) routeShortName = null;
  ...
  /**
   * Returns Collection of vehicles currently associated with specified routes. Filters out info
   * more than MAX_AGE_MSEC since it means that the info is obsolete and shouldn't be displayed.
   * Needs to return a collection because there are situations, such as unscheduled assignments,
   * where multiple vehicles can be assigned to a block.
   *
   * @param routeIdsOrShortNames
   * @return Collection of vehicles for the route. Empty collection if there are none.
   */
  public Collection<IpcVehicleComplete> getVehiclesForRoute(
    ...
    for (String routeIdOrShortName : routeIdsOrShortNames) {
      ...
      if (vehiclesForRoute != null) vehicles.addAll(vehiclesForRoute);
    }
    return vehicles;
  }

  /**
   * Returns collection of vehicles whose vehicleIds were specified using the vehiclesIds parameter.
   * No filtering of old vehicles is done since requesting info on specific vehicles.
   *
  ...
  /**
   * Returns Vehicle info for the vehicleId specified. No filtering of old vehicles is done since
   * requesting info on specific vehicle.
   *
   * @param vehicleId
   * @return
   */
  ...
  /**
   * Returns Vehicle info for all vehicles. Filter out schedule based predictions if trip time has
   * already passed since should always filter out such vehicles. But don't filter out stale
   * vehicles since this command could be useful to see all vehicles, including ones in the bus yard
   * that have been turned off for a while.
   *
   * @return
   */
  ...
  /**
   * Returns copy of list of vehicle IDs that are currently assigned to the specified block. A copy
   * is returned since the list is quite small and will often need to iterate over it while calling
   * methods that modify the underlying list. Using a copy makes sure that don't get a
   * ConcurrentModificationException. Will return empty list if no vehicles assigned to that block
   * (won't return null). Usually there will only be a single vehicle associated with a block
   * assignment but there are cases, such as unscheduled assignments, where there could be multiple
   * vehicles. Therefore this method returns a List. No filtering of vehicles is done since dealing
   * with vehicle IDs, not IpcExtVehicle objects, and therefore harder to tell if vehicle is stale.
   *
   * @param blockId
   * @return Copy of list of vehicle IDs associated with the specified block Id. Returns empty list
   *     instead of null if no vehicles associated with the block ID.
   */
   ...
   *
   * @param originalVehicle For getting the previous block ID for the vehicle. Can be null if there
   *     was no previous vehicle info
   * @param vehicle For getting the current block ID for the vehicle.
   */
  private void updateVehicleIdsByBlockMap(
      IpcVehicleComplete originalVehicle, IpcVehicleComplete vehicle) {
    // Handle old assignment
    if (originalVehicle != null) {
      // If block assignment is same as before don't need to update the
      // block map
      if (Objects.equals(originalVehicle.getBlockId(), vehicle.getBlockId())) return;
      ...
      if (vehicleIdsForOldBlock != null) vehicleIdsForOldBlock.remove(originalVehicle.getId());
    }
   ...
   * @param vehicle
   */
  private void updateVehiclesByRouteMap(
      IpcVehicleComplete originalVehicle, IpcVehicleComplete vehicle) {
  ...
  /**
   * Updates vehiclesMap. Usually will add the IpcExtVehicle to the vehiclesMap. But there is a
   * special case where a schedule based vehicle is being made unpredictable. For this situation
   * actually need to remove the vehicle from the vehicles map so that it won't show up requesting
