...
public abstract class PredictionGenerator {
  ...
  private static final IntegerConfigValue closestVehicleStopsAhead =
  ...
  protected static BooleanConfigValue storeTravelTimeStopPathPredictions =
  ...
  protected static BooleanConfigValue storeDwellTimeStopPathPredictions =
      new BooleanConfigValue(
  ...
  protected TravelTimeDetails getLastVehicleTravelTime(
      VehicleState currentVehicleState, Indices indices) throws Exception {

    StopArrivalDepartureCacheKey nextStopKey =
        new StopArrivalDepartureCacheKey(
    ...
    if (!indices.atBeginningOfTrip()) {
      ...
      StopArrivalDepartureCacheKey currentStopKey =
          new StopArrivalDepartureCacheKey(
          ...
          if (currentArrivalDeparture.isDeparture()
            ...
            if ((found = findMatchInList(nextStopList, currentArrivalDeparture)) != null) {
              TravelTimeDetails travelTimeDetails =
                  new TravelTimeDetails(currentArrivalDeparture, found);
          ...
          if (currentArrivalDeparture.isDeparture()
            ...
            if ((found = findMatchInList(nextStopList, currentArrivalDeparture)) != null) {
              if (found.getTime().getTime() - currentArrivalDeparture.getTime().getTime() > 0) {
                ...
                currentBlock =
                    dbConfig.getBlock(
                        currentArrivalDeparture.getServiceId(),
                        currentArrivalDeparture.getBlockId());
  ...
  protected VehicleState getClosetVechicle(
    ...
    for (VehicleState vehicle : vehiclesOnRoute) {
      ...
      if (numAfter != null
          && numAfter > closestVehicleStopsAhead.getValue()
          && numAfter < closest) {
        closest = numAfter;
        result = vehicle;
      }
  ...
  protected List<TravelTimeDetails> lastDaysTimes(
    ...
    for (int i = 0; i < num_days_look_back && num_found < num_days; i++) {
      ...
      Date nearestDay =
          DateUtils.truncate(DateUtils.addDays(startDate, (i + 1) * -1), Calendar.DAY_OF_MONTH);
        ...
        if (arrival != null) {
          IpcArrivalDeparture departure =
              TripDataHistoryCacheFactory.getInstance()
                  .findPreviousDepartureEvent(results, arrival);
