...
public class PredictionGeneratorDefaultImpl extends PredictionGenerator
  ...
  private static BooleanConfigValue useHoldingTimeInPrediction =
      new BooleanConfigValue(
          "useHoldingTimeInPrediction", false, "Add holding time to prediction.");
   ...
   * prediction if at the end of the trip or the useArrivalTimes parameter is set to true, and it is
   * not at a waitStop (since at waitStop always want a departure prediction. For departures the
   * prediction time will be the sum of the parameters predictionTime and timeStoppedMsec.
   *
   ...
   * @param lateSoMarkAsUncertain Indicates that vehicle is late and now generating predictions for
   *     a subsequent trip. Use to indicate that the predictions are less certain.
  ...
  protected IpcPrediction generatePredictionForStop(
    ...
    if (BiasAdjusterFactory.getInstance() != null) {
      ...
      predictionTime =
          avlReport.getTime() + adjuster.adjustPrediction(predictionTime - avlReport.getTime());
    }
    ...
    if ((indices.atEndOfTrip() || useArrivalTimes) && !indices.isWaitStop()) {
      ...
      if (indices.isWaitStop()) {
        ...
        // departure.
        double distanceToWaitStop =
            Geo.distance(avlReport.getLocation(), indices.getStopPath().getStopLocation());
        ...
        long expectedDepartureTime =
            Math.max(arrivalTime + expectedStopTimeMsec, scheduledDepartureTime);
        ...
        if (useExactSchedTimeForWaitStops.getValue()) {
          ...
          if (!deadheadingSoNoDriverBreak) {
            expectedDepartureTimeWithoutStopWaitTime =
                Math.max(
                    expectedDepartureTimeWithoutStopWaitTime,
                    path.getBreakTimeSec() * Time.MS_PER_SEC);
          }
   ...
   *
   * @param vehicleState Contains the new match for the vehicle that the predictions are to be based
   *     on.
  ...
  public List<IpcPrediction> generate(VehicleState vehicleState) {
    ...
    while (schedBasedPreds
        || predictionTime < avlTime + maxPredictionsTimeSecs.getValue() * Time.MS_PER_SEC) {
      ...
      Integer delay =
          RealTimeSchedAdhProcessor.generateEffectiveScheduleDifference(vehicleState)
                  .getTemporalDifference()
              / 1000;

      // Determine the new prediction
      IpcPrediction predictionForStop =
          generatePredictionForStop(
              avlReport,
              indices,
              predictionTime,
              useArrivalPreds,
      ...
      if ((predictionForStop.getPredictionTime() - Core.getInstance().getSystemTime())
        ...
        if (HoldingTimeGeneratorFactory.getInstance() != null) {
          ...
          if (holdingTime != null) {
            // HoldingTimeCache.getInstance().putHoldingTimeExlusiveByStop(holdingTime, new
            // Date(Core.getInstance().getSystemTime()));
      ...
      logger.debug(
          "For vehicleId={} generated prediction {}",
      ...
      if (!lastStopOfNonSchedBasedTrip && predictionForStop.getPredictionTime() > now) {
        ...
        if (indices.atEndOfTrip() || indices.atBeginningOfTrip()) {
          // Deals with case where a vehicle transitions from one trip to another and the lastStop
          // then becomes the firstSTop
          // This occassionally leads to duplicate predictions. This works around the problem by
          // creating a hash of predictions
          ...
          if (filteredPredictions.containsKey(predictionKey)
            ...
            if (predictionForStop.getTrip().getStartTime()
                > filteredPredictions.get(predictionKey).getTrip().getStartTime()) {
              logger.debug(
                  "Found multiple predictions for Prediction with routeId={}, stopId={}, and vehicleId={} ",
      ...
      if (predictionForStop.isArrival()) {
        ...
        if (useHoldingTimeInPrediction.getValue()
            && HoldingTimeGeneratorFactory.getInstance() != null) {
          HoldingTime holdingTime =
              HoldingTimeGeneratorFactory.getInstance()
                  .generateHoldingTime(vehicleState, predictionForStop);

          if (holdingTime != null) {
            long holdingTimeMsec =
                holdingTime.getHoldingTime().getTime() - holdingTime.getArrivalTime().getTime();
  ...
  public long getTravelTimeForPath(
      Indices indices, AvlReport avlReport, VehicleState vehicleState) {
    ...
    if (storeTravelTimeStopPathPredictions.getValue()) {
      PredictionForStopPath predictionForStopPath =
          new PredictionForStopPath(
              vehicleState.getVehicleId(),
  ...
  private int lastStopPredictionHash(IpcPrediction prediction) {
    ...
    result =
        prime * result + ((prediction.getStopId() == null) ? 0 : prediction.getStopId().hashCode());
    result =
        prime * result
