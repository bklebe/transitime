...
public class Indices implements Serializable {
    ...
    if (block != null) {
      ...
      if (trip == null)
        throw new IndexOutOfBoundsException(
            "tripIndex " + tripIndex + " invalid for block " + block);
  ...
  /**
   * Creates a copy of the Indices parameter. Useful if need to increment() or decrement() but don't
   * want to affect the original object.
   *
   ...
   * around the same trips so can't really tell if one if one indices is earlier than another so for
   * that case returns true if the stop paths are simply different.
   *
   * @param indices
   * @return
   */
  ...
  /**
   * Increments the indices to the next segment. If reached end of path then will increment the path
   * index. And if reach end of trip then will increment the trip index.
   *
  ...
  /**
   * Decrements the indices to the previous segment. If going past beginning of path then path index
   * is decremented. And if go past beginning of trip then the trip index is decremented.
   *
   * @return The resulting Indices object
   */
  ...
  /**
   * Decrements indices so it points to the previous path. Updates the trip and path indices. Also,
   * updates segmentIndex so that it points to last segment on the new path.
   *
   * @return
   */
   ...
   *
   * @return true if path is for a layover where need to include layover time as part of travel time
   */
