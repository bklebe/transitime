...
/**
 * For parsing Trimble TAIP format GPS commands into a Java object. It only deals with simple RPC
 * commands.
 *
 * <p>It should be noted that TAIP only provides seconds into the day. The epoch time is determined
 * by using the current system time to determine the day. This means that if processing old data for
 * a different day that could bet an epoch time for the wrong day.
 *
 * @author SkiBu Smith
 */
public class TaipGpsLocation {
  ...
  private static final Logger logger = LoggerFactory.getLogger(TaipGpsLocation.class);

  /********************** Member Functions **************************/

  /**
   * Simple constructor. Declared private because should use get() to create a TaipGpsLocation
   * object from a TAIP string.
   *
   * @param fixEpochTime
   * @param latitude
   * @param longitude
  ...
  private static boolean validate(String s) {
    ...
    if (s.length() < 35) {
      logger.warn(
          "TAIP string \"{}\" isn't for RPV command since " + "less than 35 characters long", s);
      return false;
    }
  ...
  /**
   * Processes the TAIP string and returns corresponding TaipGpsLocation object
   *
   * @param s The TAIP string
   * @return A TaipGpsLocation object containing all the processed info from the TAIP string, or
  ...
  public static TaipGpsLocation get(String s) {
    ...
    // 1 = [31,32] GPS source [0=2D-GPS, 1=3D-GPS, 2=2D-DGPS, 3=3D-DGPS, 6=DR, 8=Degraded-DR,
    // 9=Unknown]
    ...
    // get it right.
    long fixEpochTime =
        Time.getStartOfDay(new Date(), gmtTimeZone) + secondsIntoDay * Time.MS_PER_SEC;
    long currentEpochTime = System.currentTimeMillis();
    if (fixEpochTime - currentEpochTime > 30 * Time.MS_PER_MIN) fixEpochTime -= Time.MS_PER_DAY;
    ...
    return new TaipGpsLocation(
        fixEpochTime, latitude, longitude, heading, speedMetersPerSecond, gpsSourceStr, ageStr);
  }
  ...
  public String toString() {
    return "TaipGpsLocation ["
        + "fixTime="
        + Time.dateTimeStrMsec(fixEpochTime)
        + ", fixEpochTime="
        + fixEpochTime
        + ", latitude="
        + latitude
