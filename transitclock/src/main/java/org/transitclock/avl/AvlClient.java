...
public class AvlClient implements Runnable {
  ...
  /**
   * Filters out problematic AVL reports (such as for having invalid data, being in the past, or too
   * recent) and processes the ones that are good.
   *
   * <p>(non-Javadoc)
   *
    ...
    try {
      ...
      synchronized (avlReports) {
        ...
        if (previousReportForVehicle != null
          ...
          logger.warn(
              "Throwing away AVL report because it is same time "
        ...
        if (previousReportForVehicle != null) {
          ...
          if (timeBetweenReportsSecs < AvlConfig.getMinTimeBetweenAvlReportsSecs()) {
            ...
            logger.debug(
                "AVL report for vehicleId={} for time {} is "
            ...
            AvlProcessor.getInstance().cacheAvlReportWithoutProcessing(avlReport);
      ...
      // Process the report
      logger.info(
          "Thread={} AvlClient processing AVL data {}",
