...
public class AmigoWebsockets {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AmigoWebsockets.class);

  /********************** Member Functions **************************/

  /**
   * Constructor for a real-time data feed. Determines the websocket_token that is used in
   * connect().
   *
   * @param userID
   * @param projectID
   ...
   */
  public AmigoWebsockets(
      long userID, long projectID, long datasetID, String apiToken, AmigoWebsocketListener listener)
      throws JSONException {
    ...
    try {
      ...
      logger.info("Using amigocloud websocket_session={}", websocket_token);
  ...
  public boolean connect() {
    if (websocket_token == null) return false;
    ...
    try {
      ...
      mWs =
          new WebSocketClient(new URI(uri), new Draft_10()) {
            @Override
            ...
            public void onError(Exception ex) {
              logger.error("Error with amigocloud websocket: {}", ex.getMessage(), ex);
            }
          };
      ...
      while (!isConnected() && counter > 0) {
        try {
          ...
          logger.error("Error with amigocloud websocket: {}", e.getMessage(), e);
        }
        counter--;
      }
