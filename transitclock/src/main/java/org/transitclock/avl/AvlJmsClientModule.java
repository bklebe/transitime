...
/**
 * Reads AVL data from JMS topic and processes it. Can use multiple threads to do the processing.
 *
 * @author SkiBu Smith
 */
public class AvlJmsClientModule extends Module {
  ...
  private static final Logger logger = LoggerFactory.getLogger(AvlJmsClientModule.class);
   ...
   *
   * @param agencyId Specifies name of JMS topic to read AVL data from. Topic name is clientName +
   *     "-AVLTopic".
   * @throws NamingException
   * @throws JMSException
   */
  public AvlJmsClientModule(String agencyId) throws JMSException, NamingException {
    super(agencyId);
    ...
    logger.info(
        "Starting AvlClient for agencyId={} with " + "maxAVLQueueSize={} and numberThreads={}",
        agencyId,
        maxAVLQueueSize,
        numberThreads);
  ...
  /** Creates the JMS message consumer. If there is a problem then msgConsumer will be null. */
  private void createMessageConsumer() {
    ...
    try {
      ...
      logger.error(
          "JMSException when getting JMS Wrapper. "
      ...
      logger.error(
          "NamingException when getting JMS Wrapper. "
              + "Make sure the HornetQ/JMS server is running!!! "
              + ""
              + "AVL feed terminated.",
          e1);
  ...
  /**
   * So that Runnable interface used for each thread. Makes sure that runs continuously even if an
   * exception is thrown. Should not be called directly.
   */
  @Override
  public void run() {
  ...
  /**
   * Infinite loop that actually processes the AVL data by reading it from the JMS topic. Intended
   * to only be called from run().
   */
  private void processAVLDataFromJMSTopic() {
    ...
    while (true) {
      try {
        ...
        logger.debug(
            "Thread={} About to read AVL data from JMS topic", Thread.currentThread().getName());
        AvlReport avlReport = (AvlReport) JMSWrapper.receiveObjectMessage(msgConsumer);

        // Log the AVL report
        logger.debug(
            "Thread={} Processing AVL report: {}", Thread.currentThread().getName(), avlReport);
        ...
        logger.error("Error when waiting for AVL message. {}", e.getMessage());
        Time.sleep(2000);
