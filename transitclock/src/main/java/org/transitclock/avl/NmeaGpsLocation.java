 ...
 *
 * <p>Based on code at https://github.com/ktuukkan/marine-api
 *
 * <p>Useful documentation on NMEA format is available at
...
public class NmeaGpsLocation {
  ...
  private static final DateFormat dateFormatter = new SimpleDateFormat("ddMMyy");
  ...
  private static final Logger logger = LoggerFactory.getLogger(NmeaGpsLocation.class);
   ...
   * @param time
   */
  private NmeaGpsLocation(double lat, double lon, float speed, float heading, long time) {
    super();
    this.lat = lat;
    this.lon = lon;
    this.speed = speed;
  ...
  /**
   * Calculates XOR checksum of given NMEA string from between the '$' and the checksum. Resulting
   * hex value is returned as a String in two digit format, padded with a leading zero if necessary.
   *
   * @param nmeaString
  ...
  private static String calculateChecksum(String nmeaString) {
    ...
    if (nmeaString.indexOf(CHECKSUM_DELIMITER) > 0)
      partialNmeaStr = nmeaString.substring(1, nmeaString.indexOf(CHECKSUM_DELIMITER));
    else partialNmeaStr = nmeaString.substring(1);
  ...
  private static boolean checksumOk(String nmeaString) {
    ...
    if (i > 0) {
      ...
      if (!checksumOk) {
        logger.debug("NMEA string checksum is no valid. String is: {}", nmeaString);
      }
      return checksumOk;
    } else return true;
  }
  ...
  /**
   * Makes sure the NMEA string is valid. Only can handle $GPRMC commands. Checksum must be OK or
   * not included.
   *
   * @param nmeaString
  ...
  private static boolean isValid(String nmeaString, boolean ignoreChecksum) {
    ...
    if (!nmeaString.startsWith("$GPRMC")) return false;
  ...
  private static double getLatitude(String nmeaComponents[]) {
    ...
    if (hemisphereLat.equals("S")) latitude = -latitude;
  ...
  private static double getLongitude(String nmeaComponents[]) {
    ...
    if (hemisphereLon.equals("W")) longitude = -longitude;
  ...
  private static float getHeading(String nmeaComponents[]) {
    if (nmeaComponents[ANGLE].isEmpty()) return Float.NaN;
   ...
   */
  private static long getTime(String nmeaComponents[]) throws ParseException {
    ...
    if (decimal > 0) msec = Integer.parseInt(timeStr.substring(decimal + 1));
   ...
   *
   * @param nmeaString
   * @param ignoreChecksum So can disable checksum checking if it is not valid
  ...
  private static NmeaGpsLocation parse(String nmeaString, boolean ignoreChecksum) {
    ...
    if (!isValid(nmeaString, ignoreChecksum)) return null;
    ...
    try {
      ...
      logger.debug("Could not parse NMEA string. {} \"{}\"", e.getMessage(), nmeaString, e);
      return null;
    }
  }
  ...
  public static void main(String args[]) {
    NmeaGpsLocation loc;
    loc =
        parseIgnoringChecksum(
            "$GPRMC,221813.000,A,3346.89942,N,8423.75712,W,8.15,265.24,030315,,*0C,TABLET");
    System.out.println("loc=" + loc);
    loc = parse("$GPRMC,123519,A,4807.038,N,01131.000,E,022.4,084.4,230394,003.1,W*6A");
    System.out.println("loc=" + loc);
    loc =
        parseIgnoringChecksum(
            "$GPRMC,061605.000,A,3347.24214,N,8424.34422,W,0.01,,040315,,*0C,TABLET");
    System.out.println("loc=" + loc);
  }
}
