...
public class AvlExecutor {
    ...
    RejectedExecutionHandler rejectedHandler =
        new RejectedExecutionHandler() {
          ...
          public void rejectedExecution(Runnable arg0, ThreadPoolExecutor arg1) {
            String message =
                "Rejected AVL report in AvlExecutor for agencyId="
  ...
  /**
   * Returns singleton instance. Not synchronized since it is OK if an executor is replaced by a new
   * one.
   *
  ...
  /**
   * Instead of writing AVL report to JMS topic this method directly processes it. By doing this one
   * can bypass the need for a JMS server. Uses a thread executor so that can both use multiple
   * threads and queue up requests. This is especially important if getting a dump of AVL data from
   * either polling a feed or from an AVL feed hitting the Transitime web server and the AVL data
   * getting then pushed to the core system in batches.
   *
