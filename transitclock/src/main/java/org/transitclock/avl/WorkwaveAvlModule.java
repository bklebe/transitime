...
public class WorkwaveAvlModule extends PollUrlAvlModule {
  ...
  private static final String AVL_URL = "http://gps.workwave.com/php/points.php";

  private static final String GET_SESSION_URL = "http://gps.workwave.com/map/view-only?api-key=";

  private static final DateFormat workwaveTimeFormat = new SimpleDateFormat("MM/dd/yy hh:mm:ss a");

  private static StringConfigValue apiKey =
      new StringConfigValue(
          "transitclock.avl.workwaveApiKey",
          "The API key to use when getting session ID for Workwave " + "AVL feed.");

  // The authorization cookie to be provided when requesting AVL data
  String sessionId = null;

  private static final Logger logger = LoggerFactory.getLogger(TranslocAvlModule.class);
  ...
  /**
   * Access the page GET_SESSION_URL to get the cookie PHPSESSID and stores the resulting cookie in
   * sessionId member. Then when getting AVL data can send the PHPSESSID cookie as authentication.
   */
  private void setSessionId() {
    // Make sure the API key is configured
    if (apiKey.getValue() == null) {
      logger.error(
          "API key not set for WorkwaveAvlModule. Needs to be " + "set via Java property {} .",
          apiKey.getID());
      return;
    }
    ...
    try {
      ...
      for (String cookie : cookies) {
        if (cookie.contains("PHPSESSID")) {
          ...
          logger.info("Session ID for Workwave AVL feed is " + "PHPSESSID={}", sessionId);
        }
      }
    } catch (Exception e) {
  ...
  protected void setRequestHeaders(URLConnection con) {
    if (sessionId == null) logger.error("PHPSESSID not set when trying to fetch AVL data.");
  ...
  protected Collection<AvlReport> processData(InputStream in) throws Exception {
    ...
    try {
      ...
      if (!success) {
        ...
        logger.error(
            "Error occurred when getting AVL data. URL={} and " + "PHPSESSID={}. {}",
            getUrl(),
            sessionId,
            message);
      ...
      for (int i = 0; i < features.length(); ++i) {
        ...
        float heading = (float) Double.parseDouble(properties.getString("heading"));
        float speed = (float) Double.parseDouble(properties.getString("velocity"));
        ...
        AvlReport avlReport =
            new AvlReport(
                vehicleId, gpsTime.getTime(), latitude, longitude, speed, heading, "WorkWave");
