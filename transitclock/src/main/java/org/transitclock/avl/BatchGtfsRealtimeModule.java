...
public class BatchGtfsRealtimeModule extends Module {
  ...
  /**
   * Zhengzhou had trouble providing valid GTFS-RT data. So this method can be used for Zhengzhou to
   * add logging info so that can see how many valid reports there are. Also filters out AVL reports
   * that are not within 15km of downtown Zhengzhou. It is only for debugging.
   */
  private void debugZhengzhou() {
    for (String uri : getGtfsRealtimeURIs()) {
      ...
      logger.info(
          ...
          avlReports.size());
      logger.info(
          "For Zhengzhou found {} vehicles={}", zhengzhouVehicles.size(), zhengzhouVehicles);
      logger.info("For Zhengzhou found {} routes={}", zhengzhouRoutes.size(), zhengzhouRoutes);
      logger.info(
          "Earliest AVL time was {} and latest was {}",
