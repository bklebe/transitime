 ...
 *
 * <p>Documentation on the Transloc API is at https://www.mashape.com/transloc/openapi-1-2#
 *
 * @author SkiBu Smith
 */
public class TranslocAvlModule extends PollUrlAvlModule {
  ...
  private static StringConfigValue apiKey =
      new StringConfigValue(
          "transitclock.avl.transloc.apiKey",
          "tXHlJmqevJmsh4q37xKuv3k2vfZ5p1VbAvPjsnwErq18jMCSmb",
          "The API key for the Transloc API.");

  private static final DateFormat translocTimeFormat =
  ...
  protected String getUrl() {
    return feedUrl.getValue() + "vehicles.json?agencies=" + feedAgencyId.getValue();
  }
  ...
  /**
   * Reads in the JSON data from the InputStream and creates and then processes an AvlReport.
   *
   * @param in
  ...
  protected Collection<AvlReport> processData(InputStream in) throws Exception {
    ...
    try {
      ...
      if (dataObj.isNull(feedAgencyId.getValue())) return new ArrayList<AvlReport>();
      ...
      for (int i = 0; i < jsonArray.length(); ++i) {
        ...
        // an error.
        if (vehicleData.isNull("location")) continue;
        ...
        // Process AVL report
        AvlReport avlReport =
            new AvlReport(vehicleId, gpsTime.getTime(), lat, lon, speed, heading, "Transloc");
