...
/**
 * For running the system in "playback mode" where AVL data is read from the database instead of
 * from a realtime AVL feed. Useful for debugging the system software because can easily debug what
 * is happening for a particular vehicle at a particular time.
 *
 * @author SkiBu Smith
 */
public class PlaybackModule extends Module {
  ...
  private static final long DB_POLLING_TIME_MSEC = 5 * Time.MS_PER_MIN;
  ...
  private static StringConfigValue playbackVehicleId =
      new StringConfigValue("transitclock.avl.playbackVehicleId", "", "ID of vehicle to playback.");
  ...
  protected static StringConfigValue playbackStartTimeStr =
      new StringConfigValue(
          "transitclock.avl.playbackStartTime", "", "Date and time of when to start the playback.");

  protected static StringConfigValue playbackEndTimeStr =
      new StringConfigValue(
          "transitclock.avl.playbackEndTime", "", "Date and time of when to end the playback.");

  protected static BooleanConfigValue playbackRealtime =
      new BooleanConfigValue(
          "transitclock.avl.playbackRealtime",
          false,
  ...
  private static long parsePlaybackStartTime(String playbackStartTimeStr) {
    try {
      ...
      if (playbackStartTime > System.currentTimeMillis()) {
        logger.error(
            "Playback start time \"{}\" specified by "
  ...
  protected static long parsePlaybackEndTime(String playbackEndTimeStr) {
    try {
      ...
      if (playbackEndTime > System.currentTimeMillis()) {
        logger.error(
            "Playback end time \"{}\" specified by "
  ...
  protected List<AvlReport> getBatchOfAvlReportsFromDb() {
    ...
    logger.info(
        "PlaybackModule getting batch of AVLReports for "
    ...
    List<AvlReport> avlReports =
        AvlReport.getAvlReportsFromDb(
            new Date(start), new Date(end), getPlaybackVehicleId(), "ORDER BY time");
  ...
  public void run() {
    ...
    while (dbReadBeginTime < System.currentTimeMillis()
        && (playbackEndTimeStr.getValue().length() == 0
            || dbReadBeginTime < parsePlaybackEndTime(playbackEndTimeStr.getValue()))) {
      ...
      for (AvlReport avlReport : avlReports) {
        ...
        if (playbackRealtime.getValue() == true) {
          if (last_avl_time > -1) {
            try {
              // only sleep if values less than playbackSkipIntervalMinutes minutes. This is to
              // allow it skip days/hours of missing data.
              if ((avlReport.getTime() - last_avl_time)
                  < (playbackSkipIntervalMinutes.getValue() * Time.MS_PER_MIN)) {
                // Thread.sleep(avlReport.getTime()-last_avl_time);
                sleepInIncrements(
                    Core.getInstance().getSystemTime(),
                    avlReport.getTime() - last_avl_time,
                    10 * Time.MS_PER_SEC);
              }
    ...
    logger.info(
        "Processed AVL from playbackStartTimeStr:{} to playbackEndTimeStr:{} in {} secs.",
        playbackStartTimeStr,
        playbackEndTimeStr,
        Time.secondsStr(timer.elapsedMsec()));
    ...
    logger.info(
        "Read in AVL in playback mode all the way up to current " + "time so done. Exiting.");
