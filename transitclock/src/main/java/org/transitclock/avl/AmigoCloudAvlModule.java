...
public class AmigoCloudAvlModule extends AvlModule {
  ...
  private static LongConfigValue projectId =
      new LongConfigValue(
          "transitclock.avl.amigocloud.projectId",
          661L,
          "The amigocloud ID of the agency that getting AVL data " + "from.");

  private static LongConfigValue datasetId =
      new LongConfigValue(
          "transitclock.avl.amigocloud.datasetId",
          12556L,
          "The amigocloud ID of the dataset for the agency that " + "getting AVL data from.");

  private static final Logger logger = LoggerFactory.getLogger(AmigoCloudAvlModule.class);

  /********************** Member Functions **************************/

  /**
   * Constructor. Declared public so can work via Module.start() using reflection
   *
   * @param agencyId
   */
  ...
  /** Inner listener class. onMessage() is called for each AVL report from the websocket feed */
  private static class MyAmigoWebsocketListener implements AmigoWebsocketListener {
    private AmigoCloudAvlModule avlModule;

    /**
     * Constructor. So can store the AmigoCloudAvlModule so that it can be accessed to call
     * processAvlReport().
     *
     * @param avlModule
     */
     ...
     * Restarts the connection after a few seconds. The delay might be important to make sure that
     * don't keep repeatedly reconnecting. Plus the old connection might not be completely closed
     * when onClose() is called since can't tell from the documentation on WebSocket.
     *
     * @param code
     * @param reason
     */
    ...
    public void onClose(int code, String reason) {
      logger.error(
          "AmigoCloudAvlModule.onClose() called so will restart "
      ...
      try {
        ...
        for (int i = 0; i < argsArray.length(); ++i) {
          ...
          for (int j = 0; j < dataArray.length(); ++j) {
            ...
            long timestamp = Long.parseLong(data.getString("timestamp")) * Time.MS_PER_SEC;
            ...
            if (heading == 0.0) heading = Float.NaN;
            ...
            float speed = Float.parseFloat(data.getString("speed")) * Geo.MPH_TO_MPS;
            ...
            AvlReport avlReport =
                new AvlReport(
                    vehicleId, timestamp, latitude, longitude, speed, heading, "AmigoCloud");
        ...
        logger.error("Exception {} parsing from amigocloud AVL feed {}", e.getMessage(), e);
      }
    }
  }
