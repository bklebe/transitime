 ...
 *
 * <p>Documentation on options header available at
...
public class OptionsHeader {
  ...
  private static final byte OPTIONS_BYTE = (byte) 0x83;
  ...
  public static OptionsHeader getOptionsHeader(byte[] bytes) {
    ...
    if (bytes[i++] == OPTIONS_BYTE) {
      ...
      int start = i;
      while (i < start + mobileIdFieldLength) mobileId.append(String.format("%02X", bytes[i++]));
      ...
      return new OptionsHeader(mobileId.toString(), mobileIdType, i);
    } else return null;
  }
