...
public abstract class Report {
  ...
  /**
   * The type of Mobile ID being used by the LMU: 0 – OFF 1 – Electronic Serial Number (ESN) of the
   * LMU 2 – International Mobile Equipment Identifier (IMEI) or Electronic Identifier (EID) of the
   * wireless modem 3 – International Mobile Subscriber Identifier (IMSI) of the SIM card (GSM/GPRS
   * devices only) 4 – User Defined Mobile ID 5 – Phone Number of the mobile (if available) 6 – The
   * current IP Address of the LMU 7 - CDMA Mobile Equipment ID (MEID) or International Mobile
   * Equipment Identifier (IMEI) of the wireless modem
   *
   ...
   *
   * @param packet Contains the data
  ...
  public static Report parseReport(DatagramPacket packet) {
    ...
    if (logger.isDebugEnabled()) {
      ...
      logger.debug(
          "Message data is {} bytes long. Including IP Header "
    ...
    try {
      ...
      int messageStartIdx = optionsHeader != null ? optionsHeader.getNextPart() : 0;
      logger.debug("Options header {}", optionsHeader);

      // Read message header, which specifies type of report
      MessageHeader messageHeader = MessageHeader.getMessageHeader(bytes, messageStartIdx);
      logger.debug("Message header {}", messageHeader);

      if (messageHeader.isMiniEventReport()) {
        MiniEventReport miniEventReport =
            MiniEventReport.getMiniEventReport(
                optionsHeader, messageHeader, bytes, messageHeader.getNextPart());
        return miniEventReport;
      } else {
        logger.info("Not a Mini Event Report so ignoring.");
      }
    } catch (Exception e) {
      logger.error("Exception while parsing CalAmp message. {}", e.getMessage(), e);
    }
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      logger.error(
          "Exception occurred opening DatagramSocket on port {}. {}",
          portNumber,
          e1.getMessage(),
          e1);
      System.exit(-1);
