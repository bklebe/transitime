...
public class CalAmpAvlModule extends AvlModule {
  ...
  private static IntegerConfigValue calAmpFeedPort =
      new IntegerConfigValue(
          "transitclock.avl.calAmpFeedPort",
          20500,
          "The port number for the UDP socket connection for the " + "CalAmp GPS tracker feed.");

  private static final Logger logger = LoggerFactory.getLogger(CalAmpAvlModule.class);
  ...
  public void run() {
    ...
    while (true) {
      logger.info("Starting DatagramSocket on port {}", calAmpFeedPort.getValue());
