...
/**
 * Contains info for a CalAmp mini event report which is a simple GPS report. Documentation is at
...
public class MiniEventReport extends Report {
  ...
  private MiniEventReport(
      ...
      short speedKph,
      byte fixStatus,
      short numberSatellites,
      byte communicationState,
      byte inputs,
      byte eventCode) {
  ...
  /**
   * Reads MiniEventReport from byte stream starting at the offset, which should be just past the
   * message header.
   *
   * @param optionsHeader
   * @param messageHeader
  ...
  public static MiniEventReport getMiniEventReport(
      OptionsHeader optionsHeader, MessageHeader messageHeader, byte[] bytes, int offset) {
  ...
  public String toString() {
    return "MiniEventReport ["
        ...
        + " m/s"
        + ", fixStatus=0x"
        + String.format("%02X", fixStatus)
        + ", numberSatellites="
        + numberSatellites
        + ", communicationState=0x"
        + String.format("%02X", communicationState)
        + ", inputs="
        + String.format("%02X", inputs)
        + ", eventCode=0x"
        + String.format("%02X", eventCode)
        + ", optionsHeader="
        + optionsHeader
        + ", messageHeader="
        + messageHeader
  ...
  /**
   * Actually the message time, not the true GPS fix time, but it should be the same. Only message
   * time is available for mini event reports.
   *
  ...
  /**
   * Returns string representing the error status of the fix. If not an error status then null is
   * returned.
   *
  ...
  /**
   * Whether GPS fix is actually valid. Looks at top 4 bits of fixStatus to determine if Invalid
   * Time, Invalid Fix, Last Known, or Historic are set. If any of those bits are set then the fix
  ...
  public void process() {
    if (isValidGps()) {
      ...
      VehicleConfig vehicleConfig =
          VehicleDataCache.getInstance().getVehicleConfigByTrackerId(mobileId);
      String vehicleId = vehicleConfig != null ? vehicleConfig.getId() : mobileId;

      AvlReport avlReport =
          new AvlReport(
              vehicleId, getEpochTime(), getLat(), getLon(), getSpeed(), getHeading(), "CalAmp");
      ...
      logger.error(
          "GPS fix mini event report is not valid. Fix status " + "is \"{}\". {}",
          getFixStatusStr(),
          this);
    }
  }
