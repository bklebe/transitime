...
public class NextBusAvlModule extends XmlPollingAvlModule {
  ...
  private static StringConfigValue agencyNameForFeed =
      new StringConfigValue(
          "transitclock.avl.nextbus.agencyNameForFeed",
  ...
  // select max(time - timeprocessed)	from avlreports	where time between '10-16-2015' and
  // '10-17-2015';
  ...
  protected Collection<AvlReport> extractAvlData(Document doc) throws NumberFormatException {
    ...
    for (Element vehicle : vehicles) {
      ...
      long gpsEpochTime = previousTime - secsSinceReport * 1000 - apiClockSkewMsecs.getValue();

      // Handle the speed
      float speed = Float.NaN;
      String speedStr = vehicle.getAttributeValue("speedKmHr");
      if (speedStr != null) speed = Geo.converKmPerHrToMetersPerSecond(Float.parseFloat(speedStr));
      ...
      if (headingStr != null) {
        ...
        if (heading < 0) heading = Float.NaN;
      }
      ...
      logger.debug(
          "vehicleId={} time={} lat={} lon={} spd={} head={} "
              + "blk={} leadVeh={} drvr={} psngCnt={}",
          vehicleId,
          Time.timeStrMsec(gpsEpochTime),
          lat,
          lon,
          speed,
          heading,
      ...
      else avlReport.setAssignment(null, AssignmentType.UNSET);
  ...
  /**
   * Sometimes the raw block ID from the feed does not match the GTFS data. For such cases need to
