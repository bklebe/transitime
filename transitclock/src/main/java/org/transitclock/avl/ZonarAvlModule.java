...
/**
 * Reads AVL data from Zonar XML AVL feed. Only GPS data is available. There is no assignment info
...
public class ZonarAvlModule extends XmlPollingAvlModule {
  ...
  private static StringConfigValue zonarUserName =
      new StringConfigValue("transitclock.avl.zonarUserName", "The Zonar user name for API calls.");

  private static StringConfigValue zonarPassword =
      new StringConfigValue("transitclock.avl.zonarPassword", "The Zonar password for API calls.");

  private static final double KM_PER_HOUR_TO_METERS_PER_SEC = 0.27777777777778;

  private static final Logger logger = LoggerFactory.getLogger(ZonarAvlModule.class);
  ...
  protected String getUrl() {
    return "https://"
        + zonarAccount
        + ".zonarsystems.net/interface.php?username="
        + zonarUserName
        + "&password="
        + zonarPassword
