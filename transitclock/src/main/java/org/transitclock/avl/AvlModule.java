...
public abstract class AvlModule extends Module {
  ...
  /**
   * Initializes JMS if need be. Needs to be done from same thread that JMS is written to. Otherwise
  ...
  /**
   * Processes entire collection of AVL reports read from feed by calling processAvlReport() on each
   * one.
   *
   * @param avlReports
   */
  ...
  private void processAvlReportUsingJms(AvlReport avlReport) {
    ...
    if (jmsMsgProducer == null) {
      logger.error(
          "Cannot write AvlReport to JMS because JMS tools "
   ...
   * an AVL feed hitting the Transitime web server and the AVL data getting then pushed to the core
   * system in batches.
   *
