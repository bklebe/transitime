 ...
 * GTFS zip file and storing it on local machine for further processing.
 *
 * @author SkiBu Smith
 */
public class HttpGetFile {
   ...
   * Constructor
   *
   * @param urlStr URL of file to get
   * @param dirName Directory where gotten file is to be written
   */
  ...
  /**
   * Adds specified header key/value to the request. Useful if need to set If-Modified-Since or
   * such.
   *
   * @param key
   * @param value
  ...
  /**
   * Simply a getter. Returns the full name where the file is stored on the local file system.
   *
  ...
  /**
   * Gets the file name from the URL so it can be used as part of the file name for storing the
   * results.
   *
   * @param urlStr
  ...
  private static String getFileNameFromUrl(String urlStr) {
    ...
    if (lastSlashPos == -1) {
      logger.error("Couldn't determine file name so using {}", DEFAULT_FILE_NAME);
  ...
  /**
   * Actually gets and stores the file. The User-Agency property is always set to USER_AGENT.
   *
  ...
  public int getFile() throws IOException {
    ...
    if (file.lastModified() > 0 && remoteFileLastModified < file.lastModified()) {
      logger.warn(
          "Response code was {} but the local file was modified "
    ...
    while ((length = in.read(buffer)) > 0) {
      ...
      if (lengthSinceLoggingMsg > 1024 * 1024) {
        ...
        if (loopTimer.elapsedMsec() > 10 * Time.MS_PER_SEC) {
          ...
          logger.debug(
              "Read in {} bytes or {}% of file {}",
              totalLength,
              StringUtils.oneDigitFormat(100.0 * totalLength / expectedContentLength),
              urlStr);
        }
      }
    }
    ...
    if (totalLength == expectedContentLength)
      logger.debug(
          "Successfully copied {} to file {}. Length was {} " + "bytes. Took {} msec.",
          urlStr,
          fullFileNameForResult,
          totalLength,
          timer.elapsedMsec());
    else
      logger.error(
          "When copying {} to file {} the expected length was " + "{} but only copied {} bytes",
          urlStr,
          fullFileNameForResult,
          expectedContentLength,
          totalLength);
