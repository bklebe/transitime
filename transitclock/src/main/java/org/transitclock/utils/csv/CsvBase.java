...
public class CsvBase {
  ...
  /**
   * For reading values from a CSVRecord. If the column was not defined then CSVRecord.get() throws
   * an exception. Therefore for optional CSV columns better to use this function so don't get
   * exception. This way can continue processing and all errors for the data will be logged. Better
   * than just logging first error and then quitting.
   *
   * <p>Also, if the value is empty string then it is converted to null for consistency. Also trims
   * the resulting string since some agencies leave in spaces.
   *
   * @param record The data for the row in the CSV file
   * @param name The name of the column in the CSV file
  ...
  private String getValue(CSVRecord record, String name, boolean required) {
    ...
    if (!record.isSet(name)) {
      if (required) {
        logger.error(
            "Column {} not defined in file \"{}\" yet it is required", name, getFileName());
      }
      return null;
    }
    ...
    if (value.isEmpty()) {
      if (required) {
        logger.error(
            "For file \"{}\" line number {} for column {} value was not set "
                + "yet it is required",
            getFileName(),
            lineNumber,
            name);
