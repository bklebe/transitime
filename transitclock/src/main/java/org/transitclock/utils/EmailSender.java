 ...
 * transitclock.utils.emailConfigFile to configure parameters.
 *
 * @author SkiBu Smith
 */
public class EmailSender {
  ...
  private static StringConfigValue emailConfigFile =
      new StringConfigValue(
          "transitclock.utils.emailConfigFile",
          "/usr/local/transitclock/config/emailconfig.properties",
          "Specifies name of configuration file used for sending " + "out e-mails.");
  ...
  /**
   * Reads config properties from config file and creates a session that can be used to send
   * e-mails.
   */
  public EmailSender() {
   ...
   *
   * <p>Unfortunately with Gmail can't seem to set the from address. It always ends up being the
   * address of the Gmail account.
   *
  ...
  private static synchronized Properties getProperties() {
    ...
    try (InputStream input = Files.newInputStream(path)) {
      ...
      logger.error(
          "Cannot open and load e-mail server properties " + "file \"{}\".",
          emailConfigFile.getValue());
    }
   ...
   *
   * @param recipients Comma separated list of recipient e-mail addresses
   * @param subject Subject for the e-mail
   * @param messageBody The body for the e-mail
   */
  public void send(String recipients, String subject, String messageBody) {
    ...
    try {
      ...
      logger.info(
          "Successfully sent e-mail to {} . The e-mail "
