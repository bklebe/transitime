...
public class StringUtils {
  ...
  private static final DecimalFormat threeDigitFormat = new DecimalFormat("0.000");
  private static final DecimalFormat sixDigitFormat = new DecimalFormat("0.000000");
  private static final DecimalFormat commasFormat = new DecimalFormat("###,###");

  /**
   * Returns formatted string indicating number of bytes. Uses highest possible units, up to 4
   * digits. will get a result such as 4,231MB
   *
   * @param arg
   * @return
   */
  ...
  /**
   * Outputs distance string along with units. If value below 3,000 then uses meters as in 392.2m .
   * If greater than 3,000 then uses km as in 3.1km .
   *
   * @param distance
  ...
  /**
   * Returns original string padded with spaces so that it is desiredCharacters long. If the
   * original string is already at least as long as desired then it is returned.
   *
   * @param original
  ...
  public static String padWithBlanks(String original, int desiredCharacters) {
    ...
    if (original.length() >= desiredCharacters) return original;
  ...
  /**
   * Returns the name passed in, but the first number in the name is padded with zeros so that the
   * numbers have the same number of digits. This way can sort the names properly even if have names
   * such as Y2 and Y101 (Y2 should be before Y101 even though if used regular string comparison
   * Y101 would be before Y2).
   *
   * @param name The alpha numeric name to be padded.
   * @return a modified version of the name that can be used for proper ordering of a collection of
   *     names. for ordering
   */
  public static String paddedName(String name) {
    ...
    int startOfNumber = 0;
    while (startOfNumber < name.length() && !Character.isDigit(name.charAt(startOfNumber))) {
      ++startOfNumber;
    }

    // If no digits then simply return the route's short name
    if (startOfNumber >= name.length()) return name;
  ...
  /**
   * Sorts the list of IDs. Pads the first numeric part of the identifiers when doing the sorting so
   * that numbers will end up in the proper order. This is 10 times slower than sortIds(), but the
   * ordering is much nicer.
   *
   * @param ids
   */
  ...
  /**
   * Sorts the list of IDs. Uses a regular string sort. Much faster than sorting IDs numerically
   * because don't need to pad the IDs in order for them to be ordered properly. But then number
   * won't be in the proper order. For example, "10" will end up before "9".
   *
