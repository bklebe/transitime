...
public class BoundedExecutor {
   ...
   *
   * @param exec The executor. Recommend using Executors.newFixedThreadPool() to get a fixed thread
   *     pool.
   * @param allowableNumberBeforeBlocking How many items that can be queued before execute() will
   *     block if another Runnable is added. Includes both Runnables that are currently running plus
   *     ones that have already been submitted but are still queued up. The value needs to be at
   *     least as large as the number of threads allowed by the Executor in order for all threads to
   ...
   * queue includes tasks already being executed so if Executor can use 5 threads and all 5 threads
   * are executing a task and allowableNumberBeforeBlocking is 8 then can only have 3 items queued
   * up and not yet started execution before this method will block.
   *
