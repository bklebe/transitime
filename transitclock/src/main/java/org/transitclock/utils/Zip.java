...
public class Zip {
   ...
   *
   * @param zipFileName Name of the file to be unzipped
   * @param subDirectory If not null then will put the resulting unzipped files into the
   *     subdirectory. If null then resulting files end up in same directory as the zip file.
   * @return If successful, directory name where files extracted to. If problem then null.
   */
  public static String unzip(String zipFileName, String subDirectory) {
    ...
    try {
      ...
      if (subDirectory != null) {
        ...
        else directoryName += "/" + subDirectory;
      }
      ...
      logger.info(
          "Successfully extracted files from {} into directory {}. Took {} msec.",
          zipFileName,
          directoryName,
          timer.elapsedMsec());
   ...
   *
   * @param inputDirectory Specifies directory that the inputFileNames are relative to.
   * @param inputFileNames List of files to be put into the zip file. File names should be relative
   *     to the inputDirectory.
   * @param fullZipFileName The full name of the resulting zip file
   * @return Name of the resulting zip file that was created, or null if there was a problem
   */
  public static String zip(
      String inputDirectory, List<String> inputFileNames, String fullZipFileName) {
    logger.info(
        "Writing specified files in directory {} into zip file {}",
        inputDirectory,
        fullZipFileName);
    ...
    try {
      ...
      for (String fileName : inputFileNames) {
        ...
        if (fullInputFileName.equals(fullZipFileName)) {
          logger.info(
              "Not writing file {} to zip file {} because "
      ...
      logger.error("Exception occurred when zipping files. {}", e.getMessage(), e);
  ...
  /**
   * A recursive method for adding all files in a directory to a list of fileNames.
   *
   * @param inputDirectory Where to find the files. This part of the name is not included as part of
   *     the file names in the zip file.
   * @param fileOrDirName Name, relative to the inputDirectory, of file or directory to be added to
   *     the fileNames array.
   * @param fileNames The resulting list of file names.
   * @param outputFileName Name of resulting zip file. The file will be put into the directory
   *     inputDirectory + "/" subDirectory.
   */
  private static void addFileOrDir(
    ...
    if (file.isDirectory()) {
      ...
      for (String subDirFileName : fileNamesForDir) {
        ...
        addFileOrDir(inputDirectory, fileNameRelativeToInputDir, fileNames, outputFileName);
      }
    } else {
      // It is a file, not a directory, so add it to list
      logger.debug(
          "Adding file {} to list of files to store in " + "zip file {}",
          fileOrDirName,
          outputFileName);
  ...
  /**
   * Recursively goes through directory and zips all the files into a zip file.
   *
   * @param inputDirectory The name of the directory where to start. This name is not included in
   *     the file names in the zip file
   * @param subDirectory Appended to inputDirectory to specify what directory to be zipped. This
   *     part of the file name is included as part of the file names in the zip file.
   * @param outputFileName Name of resulting zip file. The file will be put into the directory
   *     inputDirectory + "/" subDirectory.
   * @return Name of the resulting zip file that was created, or null if there was a problem
   */
  public static String zip(String inputDirectory, String subDirectory, String outputFileName) {
