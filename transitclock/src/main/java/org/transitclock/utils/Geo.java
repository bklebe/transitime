...
public class Geo {
  ...
  /**
   * Outputs speed in m/s with just a single digit past the decimal point. Appends "m/s" to indicate
   * units.
   *
   * @param arg
   * @return
   ...
   * Difference in speed is approximately a factor of 3-4, but it should be noted that it still runs
   * very fast. On an old laptop running in Eclipse took 400msec for calculating 1 million
   ...
   * because it just uses Pythagoras theorem and assumes a equirectangular projection. But it should
   * be fine for relatively small distances, such as would get with a city.
   *
   * @param l1
   * @param l2
   * @return
  ...
  public static Location offset(Location l, double deltaX, double deltaY) {
    ...
    Location result =
        new Location(
            l.getLat() + Math.toDegrees(deltaLatInRads),
  ...
  /**
   * Determines the distance between a location and a vector. Looks for a line between the location
   * that is orthogonal to the line representing the vector. If the intersection with the line would
   * actually be before or after the vector then the distance from the location to the corresponding
   * end point of the vector is returned. But if the intersection of the orthogonal line to the
   * vector line is within the vector then the length of the orthogonal line is returned.
   *
   * <p>There are other algorithms that can be more efficient, but they are for Cartesian space. See
   * http://geomalgorithms.com/a02-_lines.html for an example. But since using latitudes &
  ...
  /**
   * Same as distance() but returns NaN if the location is not along the vector. In other words, if
   * closest match to the vector is beyond the beginning or end of the vector then it is not
  ...
  /**
   * Determines best match of location to the vector. Returns the distance along the vector to that
   * match. Uses same basic algorithm as distance().
   *
   * @param loc
   * @param vector
   * @return
  ...
  /**
   * For shifting a path to the right. This is useful when the path data is simply centerline street
   * data, where the stopPaths for the different directions overlap each other. By displacing the
   * stopPaths to the right a bit they don't overlap when zoomed in.
   *
   * <p>This method takes in three locations that define two vectors and hence the vertex that needs
   * to be relocated.
   *
   ...
   * exactly the distance away from the original vertex because if the angle between the vectors is
   * greater than 180 degrees than the stub would need to be longer than the distance specified in
   * order for the new stopPaths to be the distance away from the old stopPaths. But this would be
   * problematic because really sharp turns could cause the stub to be incredibly long. Therefore it
   * is limited to 1.5 of distance.
   *
   * @param l1
   * @param vertex
   * @param l2
   ...
   */
  public static Location rightOffsetVertex(
      Location l1, Location vertex, Location l2, double distance) {
   ...
   * when centerline data used for shapes. This allows the stopPaths in the different directions to
   * not completely overlap. This method is for determining the offset for a first or last point of
   * a set of points that define a shape/path.
   *
  ...
  public static void main(String[] args) {
    ...
    System.err.println(
        "offsetVertex, green, " + offsetVertex.getLat() + ", " + offsetVertex.getLon());
    //
   ...
   * @return
   */
  public static boolean headingOK(
      float vehicleHeading, float segmentheading, float allowableDelta) {
