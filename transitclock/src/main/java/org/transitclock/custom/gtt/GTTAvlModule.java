...
public class GTTAvlModule extends PollUrlAvlModule {
  ...
  protected static StringListConfigValue vehiclesonredroute =
      new StringListConfigValue(
          "transitclock.gtt.vehiclesonredroute",
          null,
          "List of vehicles on read route for HoldingTime trial.");
      ...
      if ((entry.has("route") && !entry.isNull("route") && entry.getString("route").equals("red"))
        ...
        if (avlreports.containsKey(vehicleId)) {
          if (!avlreports.get(vehicleId).equals(timestamp)) {
            avlReport =
                new AvlReport(
                    vehicleId, timestamp.getTime(), latitude, longitude, heading, speed, "GTT");
          }
        } else {
