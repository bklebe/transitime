...
/**
 * Handles generation of Arrivals and Departures by extending the class
 * ArrivalDepartureGeneratorDefaultImpl but also logs arrival/departure information to the SFMTA API
 * for tracking when tech shuttles are stopped at SFMTA stops.
 *
 * @author SkiBu Smith
 */
public class ArrDepGeneratorMissionBayImpl extends ArrivalDepartureGeneratorDefaultImpl {
  ...
  private static Map<String, Integer> sfmtaStopMap = new HashMap<String, Integer>();

  // For keeping track of corresponding arrivals
  private List<Arrival> arrivals = new ArrayList<Arrival>();

  private static final Logger logger = LoggerFactory.getLogger(ArrDepGeneratorMissionBayImpl.class);

  /********************** Member Functions **************************/

  /** Constructor. Initializes sfmtaStopMap to have all the stops that need to be tracked. */
  public ArrDepGeneratorMissionBayImpl() {
    logger.info(
        "Initializing class ArrDepGeneratorMissionBayImpl for "
  ...
  /**
   * Calls the superclass createArrivalTime() but also stores arrival time at SFMTA stops so that
   * when corresponding departure is determined both the arrival and the departure time for that
   * stop can be written to the SFMTA API.
   */
  protected Arrival createArrivalTime(
    ...
    Arrival arrival =
        super.createArrivalTime(vehicleState, arrivalTime, block, tripIndex, stopPathIndex);
  ...
  /**
   * Calls the superclass createDepartureTime() but for SFMTA stops also writes both the arrival and
   * the departure time for that stop can be written to the SFMTA API.
   */
  protected Departure createDepartureTime(
    ...
    Departure departure =
        super.createDepartureTime(vehicleState, departureTime, block, tripIndex, stopPathIndex);
    ...
    if (sfmtaStopMap.containsKey(departure.getStopId())) {
      ...
      while (it.hasNext()) {
        ...
        if (arrival.getTime() < departure.getTime() - 10 * Time.MS_PER_MIN) {
          it.remove();
          logger.error(
              "Arrival was more than 10 minutes old without "
                  + "finding corresponding departure. Therefore "
                  + "dropping the arrival. {}",
              arrival);
        }
      }
      ...
      for (Arrival arrival : arrivals) {
        if (arrival.getVehicleId().equals(departure.getVehicleId())
          ...
          logger.info(
              "Found arrival/departure pair for SFMTA stop. " + "{} {} ", arrival, departure);
