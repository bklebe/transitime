...
/**
 * Contains GTFS config information of stop and paths for the directions. Wanted this info to be
 * automatically generated from the NextBus API but there simply is not sufficient information in
 * the API. This is especially true given the many different trip patterns there are for Mission
 * Bay. For example, the caltrain route has many variations.
 *
 * @author SkiBu Smith
 */
public class MissionBayConfig {
  ...
  /**
   * Provides list of stops for each possible direction/trip pattern. Turns out this is rather
   * complicated because of the variations in the trips so resorted to hard coding the stops.
   *
   * <p>Good way to determine which stops are part of a direction/trip is to look at the schedule
   * API such as http://webservices.nextbus.com/s/xmlFeed?command=schedule&a=sf-mission-bay&r=west
  ...
  public static List<Dir> getSpecialCaseDirections(String routeId) {
    ...
    if (routeId.equals("east")) {
      ...
      d1.gtfsDirection = "0";
      String stops1[] = {"powell", "library", "chinbas", "nektar", "409illi_e", "powell"};
      ...
      d1.gtfsDirection = "0";
      String stops1[] = {
        "1650owen", "calttown", "library", "chinbas", "nektar", "409illi_e", "1650owen"
      };
      d1.stopIds = Arrays.asList(stops1);
      dirList.add(d1);
      ...
      d3.gtfsDirection = "0";
      String stops3[] = {
        "nektar", "409illi_e", "1650owen", "calttown", "library", "chinbas", "nektar"
      };
  ...
  /**
   * Defines the path data for all routes and puts it into the shapeIdsMap. Of course would have
   * preferred to automatically determine the paths for each trip pattern using only the NextBus API
   * but the naming conventions were far too haphazard. Only way to really make it work is to list
   * the path IDs here manually by looking at data in (for route west for example)
   * http://webservices .nextbus.com/service/publicXMLFeed?command=routeConfig&a
   * =sf-mission-bay&verbose=true&r=west From the URL you can see all of the paths. Then for each
   * trip pattern defined in the above method getSpecialCaseDirections() you need to include all the
   * paths between the stops, including the "_d" departure paths, to get the full shape.
   *
  ...
  public static Map<String, List<String>> getPathData() {
    ...
    String pathIds_array_east_loop[] = {
      "east_powell_d",
      "east_powell_library",
      "east_library_chinbas",
      "east_chinbas_nektar",
      "east_nektar_d",
      "east_nektar_409illi_e",
      "east_409illi_e_d",
    ...
    List<String> pathIds_west_loop = Arrays.asList(pathIds_array_west_loop);
    shapeIdsMap.put("west_loop", pathIds_west_loop);

    // Route loop, direction loop
    String pathIds_array_loop_loop[] = {
      "loop_1650owen_d",
      "loop_1650owen_berr5th_s",
      "loop_berr5th_s_calcream",
      "loop_calcream_powell",
      "loop_powell_d",
      "loop_powell_calttown",
      "loop_calttown_library",
      "loop_library_nektar",
      "loop_nektar_409illi_e",
      "loop_409illi_e_d",
    ...
    String pathIds_array_caltrain_am_loop[] = {
      "caltrain_1650owen_d",
      "caltrain_1650owen_calttown",
      "caltrain_calttown_d",
      "caltrain_calttown_library",
      "caltrain_library_d",
      "caltrain_library_chinbas",
      "caltrain_chinbas_nektar",
      "caltrain_nektar_d",
      "caltrain_nektar_409illi_e",
      "caltrain_409illi_e_1650owen",
    };
    List<String> pathIds_caltrain_am_loop = Arrays.asList(pathIds_array_caltrain_am_loop);
    ...
    List<String> pathIds_caltrain_pm_loop = Arrays.asList(pathIds_array_caltrain_pm_loop);
