...
/**
 * For generating some of the GTFS files for an agency by reading data from the NextBus API.
 *
 * @author SkiBu Smith
 */
public class GtfsFromNextBus {
  ...
  private static StringListConfigValue validBlockIds =
      new StringListConfigValue(
          "transitclock.gtfs.validBlockIds",
   ...
   *
   * @param command The NextBus API command
   * @param options Query string options to be appended to request to NextBus API
  ...
  /**
   * Processes XML data for route and extracts stop information. Adds the resulting GTFS stop to the
   * gtfsStopsMap.
   *
   * @param doc Document for routeConfig data for a route
   */
  ...
  /**
   * For each route gets routeConfig input stream from NextBus API and creates and processes an XML
   * Document object to read the data for the stops. Then writes the stops.txt GTFS file.
   *
   * @param routeIds
   */
  ...
  /**
   * Reads from NextBus API all the routes configured and returns list of their identifiers. Also
   * writes the routes.txt file.
   *
   * @return
   */
  ...
  /**
   * Returns the trip ID to used. A concatenation of routeId, blockId, and timeStr. Could use
   * something shorter, such as just the blockId and time, but it is nice to see the full context of
   * the trip in the trips.txt file.
   *
   * @param routeId
   * @param blockId
   * @param timeStr
  ...
  /**
   * Generates headsign info. Previously used "To " + name of last stop but the last stop for trip
   * is not available via the NextBus API.
   *
  ...
  private static List<String> getPathIds(String shapeId) {
    if (shapeIdsMap == null) shapeIdsMap = MissionBayConfig.getPathData();
  ...
  private static void processShapesForRoute(GtfsShapesWriter shapesWriter, Document routeDoc) {
    ...
    for (Dir dir : directions) {
      ...
      // direction
      if (shapesProcessed.contains(shapeId)) continue;
      ...
      for (String nextBusPathId : nextBusPathIds) {
        ...
        if (path == null) {
          logger.error(
              "The nextBusPathId={} was not found for "
        ...
        for (int i = 0; i < path.lats.size(); ++i) {
          ...
          GtfsShape gtfsShape =
              new GtfsShape(shapeId, shapePtLat, shapePtLon, shapePtSequence++, shapeDistTraveled);
  ...
  private static void processShapes(List<String> routeIds) {
    ...
    for (String routeId : routeIds) {
      ...
      try {
        ...
        // that get path names.
        Document routeDoc =
            getNextBusApiInputStream("routeConfig", "r=" + routeId + "&verbose=true");
  ...
  /**
   * Determines the direction for the route that best matches the stops specified in the schedule
   * for the trip. All stops from schedule must be listed in the direction. If there are multiple
   * matches uses the direction with the fewest stops so that can handle case where a schedule trip
   * could match both a direction and a direction that contains additional stops that are not in the
   * schedule.
   *
   * @param stopIdsInSchedForTrip So can determine if trip goes to calt4th stop
   * @param directionsForRoute
   * @param routeId
   * @param tripStartTimeStr Needed so can differentiate between morning and afternoon trips, which
   *     an be different for Mission Bay west route.
   * @return
   */
  private static Dir determineDirection(
      List<String> stopIdsInSchedForTrip,
      List<Dir> directionsForRoute,
      String routeId,
      ...
      if (allStopsAreInDirection) {
        ...
        if (routeId.equals("caltrain")) {
          ...
          if (dir.stopIds.get(0).equals(stopIdsInSchedForTrip.get(0))
              || stopIdsInSchedForTrip.get(0).equals("calttown")) return dir;
        } else return dir;
      }
   ...
   */
  private static void writeGtfsTrip(
      GtfsTripsWriter tripsWriter,
      String tripId,
      String routeId,
      String blockId,
      String tripStartTimeStr,
    ...
    Dir dir =
        determineDirection(stopIdsInSchedForTrip, directionsForRoute, routeId, tripStartTimeStr);
  ...
  /**
   * For the route that the scheduleDoc is for reads in all stop times and puts them into map so
   * that can determine trips.
   *
   * @param scheduleDoc
  ...
  private static Map<String, List<StopTime>> getStopTimesMap(Document scheduleDoc) {
    ...
    for (Element scheduleRow : scheduleRows) {
      ...
      // in the config.
      if (!validBlockIdsList.contains(blockId)) continue;
      ...
      for (Element stop : stopsForScheduleRow) {
        ...
        if (stopTag.endsWith("_a")) stopTag = stopTag.substring(0, stopTag.length() - 2);
   ...
   *
   * @param i Index into stopTimesForBlock
   * @param routeId
   * @param beginIndex So can determine first stop of this trip so can determine when wrap around
   * @param stopTimesForBlock The stop times for the block
   * @param dirsForRoute All of the Dirs for the route
   * @return True if first stop of trip
   */
  private static boolean firstStopInNextTrip(
      int i,
      String routeId,
      int beginIdx,
      List<StopTime> stopTimesForBlock,
    ...
    if (routeId.equals("caltrain")) {
      ...
      if (tripStartTimeSecs < 12 * Time.SEC_PER_HOUR) {
        // morning trip
        if (currentStopId.equals("1650owen")) return true;
      } else {
        // Afternoon trip
        if (currentStopId.equals("nektar")) return true;
      }
    }
    ...
    if (routeId.equals("east")) {
      if (currentStopId.equals("powell")) return true;
    }
    ...
    if (dirsForRoute.size() == 1) {
      ...
      if (currentStopId.equals(firstStopId)) return true;
    }
    ...
    if (stopTimesForBlock.get(i).stopTime
        > stopTimesForBlock.get(i - 1).stopTime + 2 * Time.SEC_PER_HOUR) return true;
  ...
  /**
   * Does really complicated merger of directions and schedule times and determines both the GTFS
   * trips and the GTFS stop times.
   *
   * @param tripsWriter
  ...
  private static void processTripsAndStopTimesForRoute(
      GtfsTripsWriter tripsWriter,
      GtfsStopTimesWriter stopTimesWriter,
      String routeId,
      ...
      do {
        ...
        for (endIdx = beginIdx + 1;
            endIdx < stopTimesForBlock.size()
                && !firstStopInNextTrip(endIdx, routeId, beginIdx, stopTimesForBlock, dirsForRoute);
            ++endIdx) {}

        // Handle end of block properly
        if (endIdx >= stopTimesForBlock.size()) endIdx = stopTimesForBlock.size() - 1;
        ...
        boolean tripWithOnlySingleStop =
            scheduledStopIdsForTrip.size() <= 2
                && scheduledStopIdsForTrip
                    .get(0)
                    .equals(scheduledStopIdsForTrip.get(scheduledStopIdsForTrip.size() - 1));
        if (!tripWithOnlySingleStop) {
          ...
          Dir matchingDir =
              determineDirection(scheduledStopIdsForTrip, dirsForRoute, routeId, tripStartTimeStr);
          ...
          while (dirIdx < matchingDir.stopIds.size() && prevStopTimeIdx < endIdx) {
            ...
            GtfsStopTime gtfsStopTime =
                new GtfsStopTime(tripId, timeStr, timeStr, stopId, dirIdx, timepointStop);
        ...
        beginIdx = endIdx;
        if (deadheading) beginIdx++;
      } while (beginIdx < stopTimesForBlock.size() - 1);
    }
  }

  /**
   * Uses NextBus API schedule command to determine trips and stop times and create the
   * corresponding GTFS files.
   *
   * @param routeIds
   */
