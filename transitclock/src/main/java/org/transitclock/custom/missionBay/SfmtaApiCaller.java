...
public class SfmtaApiCaller {
  ...
  private static StringConfigValue telemetryUrl =
      new StringConfigValue(
          "transitclock.sfmta.telemetryUrl",
  ...
  private static StringConfigValue login =
      new StringConfigValue(
          "transitclock.sfmta.login", "TransitTimeApiUser", "Login for basic authentication.");

  private static StringConfigValue password =
      new StringConfigValue(
          "transitclock.sfmta.password", "SFMTA$hutt1e$3G", "Password for basic authentication.");

  private static StringConfigValue techProviderId =
      new StringConfigValue(
          "transitclock.sfmta.techProviderId", "248", "The TechProviderId for the SFMTA API");

  private static StringConfigValue shuttleCompanyId =
      new StringConfigValue(
          "transitclock.sfmta.shuttleCompanyId", "11", "The ShuttleCompanyId for the SFMTA API");

  private static IntegerConfigValue timeout =
      new IntegerConfigValue(
          "transitclock.sfmta.timeout",
          10000,
  ...
  private static IntegerConfigValue avlReportsBatchSize =
      new IntegerConfigValue(
          "transitclock.sfmta.avlReportsBatchSize",
          10,
          "Won't actually post AVL reports to SFMTA API until this " + "many have been received.");
  ...
  private static SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss-08:00");

  // For batching the AVL reports
  private static List<AvlReport> avlReports = new ArrayList<AvlReport>();

  private static final Logger logger = LoggerFactory.getLogger(SfmtaApiCaller.class);
  ...
  /**
   * Posts the JSON string to the URL. For either the telemetry or the stop command.
   *
   * @param baseUrl
   * @param jsonStr
  ...
  private static boolean post(String baseUrl, String jsonStr) {
    try {
      ...
      logger.error(
          "Exception when writing data to SFMTA API: \"{}\" " + "URL={} json=\n{}",
          e.getMessage(),
          baseUrl,
          jsonStr);
  ...
  private static void postAvlReports(List<AvlReport> avlReports) {
    ...
    if (post(telemetryUrl.getValue(), jsonStr)) {
      logger.info(
          "Successfully pushed out {} telemetry data points to " + "SFMTA API", avlReports.size());
   ...
   * @param departureTime
   */
  public static void postStopReport(
      String vehicleId, int stopId, double lat, double lon, Date arrivalTime, Date departureTime) {
    init();

    StopInfo stopInfo = new StopInfo(vehicleId, stopId, lat, lon, arrivalTime, departureTime);
  ...
  /**
   * To be called when system has a new AVL report. Batches the reports together. If have enough
   * reports (as specified by avlReportsBatchSize) then actually writes them to the API.
   *
   * <p>Synchronized in case multiple threads used when accessing the avlReports cache
   *
   * @param avlReport
   */
  public static synchronized void postAvlReportWhenAppropriate(AvlReport avlReport) {
  ...
  public static void main(String args[]) {
    AvlReport avlReport1 =
        new AvlReport("693", System.currentTimeMillis(), 37.12345, -122.4567, null);
    AvlReport avlReport2 =
        new AvlReport("694", System.currentTimeMillis(), 37.12345, -122.4567, null);
    ...
    StopInfo s1 = new StopInfo("693", 3124, 37.12345, -122.4567, new Date(), new Date());
    StopInfo s2 = new StopInfo("694", 4321, 37.12346, -122.4667, new Date(), new Date());
