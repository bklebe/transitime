...
public class VtaAcsAvlModule extends PollUrlAvlModule {
  ...
  private static StringConfigValue feedUrl =
      new StringConfigValue("transitclock.avl.vta.url", "The URL of the ACS feed to use.");

  private static final Logger logger = LoggerFactory.getLogger(VtaAcsAvlModule.class);
  ...
  protected Collection<AvlReport> processData(InputStream in) throws Exception {
    ...
    while ((line = buf.readLine()) != null) {
      ...
      AvlReport avlReport =
          new AvlReport(vehicleId, epochSecs * Time.MS_PER_SEC, lat, lon, "VTA_ACS");
