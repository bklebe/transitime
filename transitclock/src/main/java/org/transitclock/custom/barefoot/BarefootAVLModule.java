 ...
 *     <p>This module subscribes to a barefoot server to get map matched GPS data.
 */
public class BarefootAVLModule extends PollUrlAvlModule {

  private static IntegerConfigValue barefootPort =
      new IntegerConfigValue(
          "transitclock.avl.barefoot.subscribe.port",
          1235,
          "This is the port of the barefoot server to subscribe to.");

  private static StringConfigValue barefootServer =
      new StringConfigValue(
          "transitclock.avl.barefoot.subscribe.server",
          "127.0.0.1",
          "This is the server that is running the barefoot map matching service.");

  private static DoubleConfigValue minProbability =
      new DoubleConfigValue(
          "transitclock.avl.barefoot.subscribe.minprobability",
          0.1,
  ...
  protected void getAndProcessData() throws Exception {
    ...
    while (!Thread.currentThread().isInterrupted()) {
      ...
      if (location != null) {
        avlReport =
            new AvlReport(vehicleId, time, location.getLat(), location.getLon(), "Barefoot");
        ...
        Point point =
            (Point)
                GeometryEngine.geometryFromWkt(
                    geoPoint, WktImportFlags.wktImportDefaults, Type.Point);
