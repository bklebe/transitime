...
/**
 * Reads AVL data from the NYC MTA BusTime Siri AVL feed and publishes it to appropriate JMS topic
 * so that it can be read by AVL clients.
 *
 * @author SkiBu Smith
 */
public class BusTimeSiriAvlModule extends XmlPollingAvlModule {
  ...
  private static final Logger logger = LoggerFactory.getLogger(BusTimeSiriAvlModule.class);
  ...
  protected Collection<AvlReport> extractAvlData(Document doc) throws NumberFormatException {
    ...
    for (Element vehicle : vehicles) {
      ...
      if (monitoredVehicleJourney != null) {
        ...
        logger.debug(
            "vehicle={} time={} lat={} lon={} spd={} head={} blk={}",
            vehicleId,
            Time.timeStr(gpsEpochTime),
            lat,
            lon,
            speed,
            heading,
        ...
        AvlReport avlReport =
            new AvlReport(vehicleId, gpsEpochTime, lat, lon, speed, heading, "SIRI");
