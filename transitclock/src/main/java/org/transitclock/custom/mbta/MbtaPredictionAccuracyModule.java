...
public class MbtaPredictionAccuracyModule extends PredictionAccuracyModule {
  ...
  private static final StringConfigValue apiKey =
      new StringConfigValue(
          "transitclock.predAccuracy.apiKey",
  ...
  /**
   * Gets XML data for route from API an returns an XML Document object containing the resulting
   * data.
   *
  ...
  private Document getExternalPredictionsForRoute(RouteAndStops routeAndStops) {
    ...
    logger.info(
        "Getting predictions from API for route={} using URL={}", routeAndStops.routeId, fullUrl);
    ...
    try {
      ...
      if (httpResponseCode != HttpURLConnection.HTTP_OK) {
        if (httpResponseCode == HttpURLConnection.HTTP_NOT_FOUND) {
          ...
          logger.info(
              "Response code was {} indicating that no data "
  ...
  /**
   * Returns true if the specified stop is not a wait stop, indicating that the prediction is an
   * arrival prediction (wait stops are terminal departures and are therefore departure predictions.
   *
   * @param tripId
   * @param stopId
   * @return
  ...
  private boolean isArrival(String tripId, String stopId) {
    ...
    if (stopPath == null) {
      logger.error("stopId={} doesn't exist for tripId={}", stopId, tripId);
      return false;
    }
  ...
  private void processExternalPredictionsForRoute(
    ...
    for (Element direction : directions) {
      ...
      if (!routeAndStops.stopIds.keySet().contains(directionId)) continue;
      ...
      for (Element trip : trips) {
        ...
        for (Element stop : stops) {
          ...
          String predictionEpochTimeStr = stop.getAttributeValue("pre_dt");
          Date predictedTime = new Date(Long.parseLong(predictionEpochTimeStr + "000"));
