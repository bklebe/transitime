...
/**
 * Processes MBTA AVL feed data one day at a time to determine the which block a trip is associated
 * with. Writes out results to a supplemental trips.txt GTFS file so that blocks can be used as part
 * of the MBTA configuration.
 *
 * @author SkiBu Smith
 */
public class GenerateMbtaBlockInfo {
  ...
  private static Map<String, String> tripToBlockMap = new HashMap<String, String>();
  ...
  private static Map<String, GtfsTrip> gtfsTripsMap = new HashMap<String, GtfsTrip>();
  ...
  private static Map<String, String> tripToModifiedServiceIdMap = new HashMap<String, String>();

  private static final Logger logger = LoggerFactory.getLogger(GenerateMbtaBlockInfo.class);
   ...
   */
  private static void writeSupplementalTripsFile(String supplementTripsFileName) {
    ...
    for (String tripShortName : sortedTripNames) {
      ...
      if (mismatchedAssignments.get(tripShortName) != null) continue;
  ...
  /**
   * Didn't realize at beginning that assignments from the feed in the form "dd00" need to be
   * modified to be "dd" so that they match the trip short names in the GTFS trips.txt file. This
   * means that some early data had the wrong trip identifier. To make this class work with the old
   * problematic names stored in the AVL database need to adjust them to remove the trailing zeros.
   *
   * @param assignment
   * @return The assignment adjust so that "dd00" becomes "dd"
   */
  private static String adjustAssignment(String assignment) {
    if (assignment.length() == 4 && assignment.endsWith("00")) return assignment.substring(0, 2);
  ...
  private static String pad(String s, int desiredLength) {
    while (s.length() < desiredLength) s += " ";
    return s;
  }

  /**
   * Processes days worth of AVL data to determine the block assignments for each trip. Updates the
   * static tripToBlockMap with the block assignment for each trip.
   *
   * @param beginDate
   */
  private static void processDayOfData(Date beginDate) {
    ...
    List<AvlReport> avlReports =
        AvlReport.getAvlReportsFromDb(beginDate, endDate, null, "ORDER BY time");
    System.out.println(
        "Read in "
            + avlReports.size()
            + " AVL reports for "
    ...
    for (String vehicleId : reportsByVehicle.keySet()) {
      ...
      for (AvlReport avlReport : avlReportsForVehicle) {
        ...
        if (start != null) {
          ...
          while (s.length() < 5) s += " ";
          earlinessStr = earlinessOfAssignment >= 0 ? s + " early" : s + " LATE!";
        } else {
          earlinessStr = "-----------";
        }
        ...
        if (tripShortName.equals("9999") || blockId.equals("000")) continue;
        ...
        if (earlinessOfAssignment < 15 * Time.SEC_PER_MIN && !previousBlockId.equals("000"))
          continue;
        ...
        if (previousBlockForTrip != null && !previousBlockForTrip.equals(blockId)) {
          ...
          System.out.println(
              "Got missmatch between block assignment "
                  + "for tripId="
                  + tripShortName
                  + ". Block for that trip was "
                  + previousBlockForTrip
                  + " but for begin date "
                  + beginDate
                  + " it is "
                  + blockId);
        ...
        String serviceIdForBlock =
            gtfsTripForBlock == null ? serviceIdForTrip : gtfsTripForBlock.getServiceId();
        ...
        if (!serviceIdForBlock.equals(serviceIdForTrip)) {
          tripToModifiedServiceIdMap.put(gtfsTrip.getTripShortName(), serviceIdForBlock);
        }
  ...
  /**
   * Determines start and end times of trips using GTFS stop_times.txt data and puts them into
   * tripStartTimeMap and tripEndTimeMap.
   *
   * @param gtfsDir
   */
  ...
  /**
   * @param args args[0] is the date to run program for, args[1] is number of days of data to use,
   *     args[2] is the directory name where the GTFS files reside and where the
   *     supplement/trips.txt file is written.
   */
