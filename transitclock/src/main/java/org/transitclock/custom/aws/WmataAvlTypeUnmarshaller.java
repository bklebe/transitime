...
public class WmataAvlTypeUnmarshaller implements SqsMessageUnmarshaller {
  ...
  private static final Logger logger = LoggerFactory.getLogger(WmataAvlTypeUnmarshaller.class);
  ...
  private AvlReportWrapper toAvlReport(JSONObject jsonObj) {
    ...
    if (msgObj.has("vehicleid")) vehicleId = String.valueOf(msgObj.getLong("vehicleid"));
    Double lat = null;
    if (msgObj.has("latitude")) lat = msgObj.getDouble("latitude");

    Double lon = null;
    if (msgObj.has("longitude")) lon = msgObj.getDouble("longitude");

    Long time = null;
    if (msgObj.has("avlDate")) time = msgObj.getLong("avlDate");
    ...
    if (vehicleId != null && lat != null && lon != null && time != null) {
      ...
      return new AvlReportWrapper(
          ar, avlLatency, forwarderProcessingLatency, sqsLatency, totalLatency);
    }
