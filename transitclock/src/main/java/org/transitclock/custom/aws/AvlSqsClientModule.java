...
/**
 * Reads AVL data from AWS SQS topic, deserializes it, and process it following the patter
 * established by AvlJmsClientModule.
 */
public class AvlSqsClientModule extends Module {
  ...
  private static IntegerConfigValue numAvlThreads =
      new IntegerConfigValue(
          "transitclock.avl.jmsNumThreads",
          1,
  ...
  private static IntegerConfigValue messageLogFrequency =
      new IntegerConfigValue(
          "transitclock.avl.messageLogFrequency",
  ...
  private static StringConfigValue sqsSecret =
      new StringConfigValue(
          "transitclock.avl.sqsSecret", null, "The AWS Secret with SQS read access");
  ...
  private static StringConfigValue snsSecret =
      new StringConfigValue(
          "transitclock.avl.snsSecret", null, "The AWS Secret with SNS write access");
  ...
  private static ClassConfigValue unmarshallerConfig =
      new ClassConfigValue(
          "transitclock.avl.unmarshaller",
          WmataAvlTypeUnmarshaller.class,
  ...
  private static StringConfigValue applicableList =
      new StringConfigValue(
          "transitclock.avl.filterVehicleIdList", null, "List of vehicle Ids to filter on");
  ...
  public AvlSqsClientModule(String agencyId) throws Exception {
    ...
    logger.info(
        "Starting AvlClient for agencyId={} with "
            + "maxAVLQueueSize={}, numberThreads={} and url={}",
        agencyId,
        maxAVLQueueSize,
        numberThreads,
        _url);
    ...
    if (numberThreads < 1) {
      logger.error(
          "Number of threads must be at least 1 but {} was "
              + "specified. Therefore using 1 thread.",
          numberThreads);
      numberThreads = 1;
    }
    ...
    if (!StringUtils.isEmpty(snsKey.getValue())
      ...
      try {
        ...
        _sns =
            new AmazonSNSClient(new BasicAWSCredentials(snsKey.getValue(), snsSecret.getValue()));
      } catch (Exception any) {
    ...
    while (!Thread.interrupted()) {
      try {
        ...
        try {
          ...
          logger.error(
              "dropping message {} as queue is full.  deseralize size={}, archive size={}:  ",
              message,
              ise,
              ...
              for (AvlReportWrapper avlReport : avlReports) {
                ...
                if (avlReport != null) {
                  if (avlReport.getTotalLatency() != null) {
                    monitoring.saveMetric(
                        "PredictionTotalQueueLatencyInMillis",
                        new Double(avlReport.getTotalLatency()),
                        1,
                  ...
                  if (avlReport.getSqsLatency() != null) {
                    monitoring.saveMetric(
                        "PredictionSQSQueueLatencyInMillis",
                        new Double(avlReport.getSqsLatency()),
                        1,
                        CloudwatchService.MetricType.AVERAGE,
                        CloudwatchService.ReportingIntervalTimeUnit.MINUTE,
                        false);
                  ...
                  if (avlReport.getAvlLatency() != null) {
                    monitoring.saveMetric(
                        "PredictionAvlQueueLatencyInMillis",
                        new Double(avlReport.getAvlLatency()),
                        1,
                        CloudwatchService.MetricType.AVERAGE,
                        CloudwatchService.ReportingIntervalTimeUnit.MINUTE,
                        false);
                  ...
                  if (avlReport.getForwarderProcessingLatency() != null) {
                    monitoring.saveMetric(
                        "PredictionForwarderProcessingLatencyInMillis",
                  ...
                  if (avlReport.getForwarderSendLatency() != null) {
                    monitoring.saveMetric(
                        "PredictionForwarderSendLatencyInMillis",
                        new Double(avlReport.getForwarderSendLatency()),
                        1,
                        CloudwatchService.MetricType.AVERAGE,
                        CloudwatchService.ReportingIntervalTimeUnit.MINUTE,
                        false);
  ...
  private class StatusTask implements Runnable {
    ...
    public void run() {
      while (!Thread.interrupted()) {
        try {
          long lastAvlReportTime =
              (System.currentTimeMillis() - AvlProcessor.getInstance().lastAvlReportTime()) / 1000;
          logger.info(
              "Queue Size Report:  AVL last report {}s, recieve={}, deserialize={}, ack={}, archive={}",
              lastAvlReportTime,
              _receiveQueue.size(),
              _deserializeQueue.size(),
              _acknowledgeQueue.size(),
          ...
          monitoring.saveMetric(
              "PredictionReceiveQueueSize",
              new Double(_receiveQueue.size()),
              1,
          ...
          monitoring.saveMetric(
              ...
              new Double(_deserializeQueue.size()),
              1,
              CloudwatchService.MetricType.SCALAR,
              CloudwatchService.ReportingIntervalTimeUnit.IMMEDIATE,
              false);
          monitoring.saveMetric(
              "PredictionAckQueueSize",
              new Double(_acknowledgeQueue.size()),
              1,
              CloudwatchService.MetricType.SCALAR,
              CloudwatchService.ReportingIntervalTimeUnit.IMMEDIATE,
              false);
          monitoring.saveMetric(
              "PredictionArchiveQueueSize",
              new Double(_archiveQueue.size()),
              1,
              CloudwatchService.MetricType.SCALAR,
              CloudwatchService.ReportingIntervalTimeUnit.IMMEDIATE,
              false);
