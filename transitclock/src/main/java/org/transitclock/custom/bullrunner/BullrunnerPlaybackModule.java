...
/**
 * @author scrudden Throw away module. Just used to read in archived Bullrunner data to work on
 *     Kalman predictions for frequency based services.
 */
public class BullrunnerPlaybackModule extends PollUrlAvlModule {
  ...
  private static StringConfigValue bullrunnerbasePlaybackUrl =
      new StringConfigValue(
          "transitclock.avl.bullrunnerbasePlaybackUrl",
          "http://transit.jadorno.com/bullrunner/gtfsrt/vehicle_positions",
  ...
  private static final Logger logger = LoggerFactory.getLogger(BullrunnerPlaybackModule.class);

  public BullrunnerPlaybackModule(String agencyId) {
    super(agencyId);

    if (latesttime == null) latesttime = parsePlaybackStartTime(playbackStartTimeStr.getValue());
  }
  ...
  protected Collection<AvlReport> processData(InputStream in) throws Exception {
    ...
    try {
      ...
      for (int i = 0; i < entities.length(); i++) {
        ...
        if (vehicleTrip.has("routeId")
            && (runnerTestRoute.getValue() == null
                || vehicleTrip.getString("routeId").equals(runnerTestRoute.getValue()))) {
          ...
          avlReport.setAssignment(vehicleTrip.getString("routeId"), AssignmentType.ROUTE_ID);
