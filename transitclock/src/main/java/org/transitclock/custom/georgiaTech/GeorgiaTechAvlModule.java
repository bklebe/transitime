...
/**
 * Gets GPS data from the Georgia Tech AVL feed. This feed is different from other AvlModules in
 * that it reads data from a socket instead of polling from a URL (such as an XML feed).
 *
 * @author SkiBu Smith
 */
public class GeorgiaTechAvlModule extends AvlModule {
  ...
  private static StringConfigValue georgiaTechFeedDomainName =
      new StringConfigValue(
          "transitclock.avl.georgiaTechFeedDomainName",
          "The domain name for the socket connection for the " + "Georgia Tech AVLfeed.");

  private static IntegerConfigValue georgiaTechFeedPort =
      new IntegerConfigValue(
          "transitclock.avl.georgiaTechFeedPort",
          "The port number for the socket connection for the " + "Georgia Tech AVLfeed.");
  ...
  private static final Logger logger = LoggerFactory.getLogger(GeorgiaTechAvlModule.class);
  ...
  /**
   * Process AVL report for a vehicle by reading data from the socket. The data is in NMEA format
   * and also includes special commands for specifying the vehicleId and the assignment. Calls
   * superclass processAvlReport() for each AVL report in order to actually process the report and
   * generate predictions.
   *
   * @param reader
   * @throws IOException
   */
  private void readVehicleDataFromSocket(BufferedReader reader) throws IOException {
    ...
    while (!(input = readLine(reader)).isEmpty()) {
      ...
      if (paramterName.equals("VID")) vehicleId = value;

      // If assignment ("job"?) then keep track of it
      if (paramterName.equals("J")) assignmentId = value;
    }
    ...
    if (shouldProcessAvl) processAvlReport(avlReport);
  }

  /**
   * Appends data for a vehicle to a CSV file. Just for testing. Can be useful for sketching out
   * vehicle locations using Google Map Engine.
   *
   * @param vehicleId
   * @param gpsLoc
   * @throws IOException
  ...
  private static void writeToFile(String vehicleId, NmeaGpsLocation gpsLoc) throws IOException {
  ...
  /**
   * Reads data from socket forever and processes it. Catches exceptions so really will run forever.
   */
  private void readLoop() {
    ...
    while (true) {
      ...
      try {
        ...
        soc = new Socket(georgiaTechFeedDomainName.getValue(), georgiaTechFeedPort.getValue());
        ...
        logger.error("Exception occurred in readLoop(). Will try again " + "in a few seconds", e);
