...
/**
 * Defines the two vectors that define an intersection. The vector segment should be the block
 * before the intersection and the second one should be the block from the intersection to the end
 * of the block after the intersection.
 *
 * @author SkiBu Smith
 */
public class Intersection {
  ...
  private static final double ALLOWABLE_DISTANCE = 40.0;
  ...
  /**
   * Returns distance from lat,lon to the first path segment. Returns NaN if the match is to before
   * or after the vector or if the distance from the vector is greater than allowableDistance.
   *
   * @param loc
   * @return
   */
  ...
  /**
   * Returns distance from lat,lon to the second path segment. Returns NaN if the match is to before
   * or after the vector or if the distance from the vector is greater than allowableDistance.
   *
   * @param loc
   * @return
