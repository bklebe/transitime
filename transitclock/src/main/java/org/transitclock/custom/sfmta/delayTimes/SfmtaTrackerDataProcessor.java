...
/**
 * An experiment for determining using GPS how much time is lost when a bus has to stop at 4-way
 * stop. Used to better understand planned changes for the 71-Haight.
 *
 * @author SkiBu Smith
 */
public class SfmtaTrackerDataProcessor {
  ...
  // be stopped.
  private static final double MIN_RADIUS_FROM_INTERSECTION =
      35.0; // FIXME changed to 15 to get Shrader data // 35.0;

  private static final Logger logger = LoggerFactory.getLogger(SfmtaTrackerDataProcessor.class);
  ...
  private static double speed(Loc loc1, Loc loc2) {
    double distanceBetweenLocs = Geo.distanceHaversine(loc1.getLocation(), loc2.getLocation());
  ...
  /**
   * Determines top speed for the locations. Since speed usually not available directly on iPhone
   * when using Javascript this method looks at two subsequent locations in the array.
   *
   * @param locs
   * @param intersectionLoc Location of intersection that matches must be away from
   * @param minRadiusFromIntersection How far matches need to be away from intersection to be used
   *     as top speed index. Prevents using noisy value.
   * @return
   */
  private static TopSpeedLoc topSpeed(
      List<Loc> locs, Location intersectionLoc, double minRadiusFromIntersection) {
    ...
    for (int i = 1; i < locs.size() - 2; ++i) {
      ...
      // noisy.
      double distanceFromIntersection = Geo.distance(intersectionLoc, loc1.getLocation());
      if (distanceFromIntersection < minRadiusFromIntersection) {
        logger.debug(
            "For i={} was {}m from intersection which is too "
                + "close, so not using for finding top speed index. epoch={} {} {}",
            i,
            StringUtils.oneDigitFormat(distanceFromIntersection),
            Time.timeStrMsec(loc1.epochTime),
            loc1.epochTime,
            loc1.getLocation());
        continue;
      }
      ...
      double distance =
          Geo.distance(new Location(loc1.lat, loc1.lon), new Location(loc2.lat, loc2.lon));
      double elapsedTime = (loc2.epochTime - loc1.epochTime) / 1000.0;
      logger.debug(
          "speed for i={} is {}m/s or {}mph latLon={},{} " + "distance={}m time={}s epoch={} {}",
          i,
    ...
    if (speedMetersPerSecond > 24.0 * Geo.MPH_TO_MPS) speedMetersPerSecond = 24.0 * Geo.MPH_TO_MPS;
  ...
  private static void processIntersectionMatch(
      Intersection intersection, List<Loc> matchesToV1, List<Loc> matchesToV2) {
    if (matchesToV1.size() < 2 || matchesToV2.size() < 2) {
      logger.debug(
          "Didn't get at least 2 matches for both matchesToV1 "
    ...
    TopSpeedLoc v1TopSpeedLoc =
        topSpeed(matchesToV1, intersectionLoc, MIN_RADIUS_FROM_INTERSECTION);
    if (v1TopSpeedLoc == null) {
      logger.debug("No valid speed points for v1");
      return;
    }
    logger.debug(
        "Filtered top speed for v1={}m/s or {}mph at topSpeedIdx={} for {}",
    ...
    TopSpeedLoc v2TopSpeedLoc =
        topSpeed(matchesToV2, intersectionLoc, MIN_RADIUS_FROM_INTERSECTION);
    if (v2TopSpeedLoc == null) {
      logger.debug("No valid speed points for v2");
      return;
    }
    logger.debug(
        "Filtered top speed for v2={}m/s or {}mph at topSpeedIdx={} for {}",
    ...
    double actualTimeBetweenLocs =
        (v2TopSpeedLoc.loc.epochTime - v1TopSpeedLoc.loc.epochTime) / 1000.0;

    double distanceBetweenLocs =
        Geo.distanceHaversine(v1TopSpeedLoc.getLocation(), v2TopSpeedLoc.getLocation());
    ...
    logger.info(
        "RESULT: difference={}s actualTimeBetweenLocs={}s "
        ...
        v1TopSpeedLoc.getLocation(),
        v2TopSpeedLoc.getLocation());
    logger.debug("");
  }
  ...
  public static void main(String[] args) {
    ...
    List<Intersection> intersections = Intersection.readIntersections(intersectionsFileName);
    ...
    for (int locCnt = 0; locCnt < locs.size(); ++locCnt) {
      ...
      for (Intersection intersection : intersections) {
        ...
        if (!Double.isNaN(distanceToV1)) {
          ...
          logger.debug(
              "Calling processIntersectionMatch() for " + "\"{}\" for locCnt={}",
              intersection.name,
              locCnt);
