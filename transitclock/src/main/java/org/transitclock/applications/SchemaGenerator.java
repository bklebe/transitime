...
/**
 * For generating SQL schema files based on classes to be stored in database that were annotated for
 * Hibernate. This is much nicer than trying to figure out what the schema really should be by hand.
 * This code was copied from http:/
 * /jandrewthompson.blogspot.nl/2009/10/how-to-generate-ddl-scripts-from.html
 *
 * <p>Note that unfortunately there does not appear to be a way to specify the order of the columns
 * in the resulting create table SQL statements. Though this has been asked for, it appears to still
 * be a limitation of Hibernate. The default ordering appears to be first the @Id columns in reverse
 * alphabetical order, and then the non @Id columns in alphabetical order. Yes, quite peculiar.
 *
 * <p>Since the resulting automatically generated files have unneeded drop commands these are
 * filtered out. This way the resulting sql is smaller and easier to understand.
 *
...
public class SchemaGenerator {
  ...
  /**
   * MySQL handles fractional seconds differently from PostGRES and other DBs. Need to use
   * "datetime(3)" for fractional seconds whereas with PostGRES can use the default "timestamp"
   * type. In order to handle this properly in the generated ddl schema files need to not
   * use @Column(columnDefinition="datetime(3)") in the Java class that defines the db object.
   * Instead need to use this special ImprovedMySQLDialect as the Dialect.
   */
  ...
  /**
   * Gets rid of the unwanted drop table commands. These aren't needed because the resulting script
   * is intended only for creating a database, not for deleting all the data and recreating the
   * tables.
   *
  ...
  private void trimCruftFromFile(String outputFilename) {
    ...
    try {
      ...
      System.err.println(
          "Could not trim cruft from file " + outputFilename + " . " + e.getMessage());
    } finally {
      try {
  ...
  /**
   * Param args args[0] is the package name for the Hibernate annotated classes whose schema is to
   * be exported such as "org.transitclock.db.structs". args[1] is optional output directory where
   * the resulting files are to go. If the optional output directory is not specified then schema
   * files written to local directory.
   *
   * <p>The resulting files have the name "ddl_" plus dialect name such as mysql or oracle plus the
   * first two components of the package name such as org_transitclock.
   */
