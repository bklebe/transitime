 ...
 * expected travel and stop times.
 *
 * <p>NOTE: This could probably be made less resource/memory intensive by processing a days worth of
 * data at a time. Another possibility would be to try to process the data while it is being read in
 * instead of reading it all in at the beginning. But that would likely be quite difficult to
 * implement. Processing one day of data at a time would likely be far simpler and therefore a
 * better choice.
 *
 * @author SkiBu Smith
 */
public class UpdateTravelTimes {
   ...
   * db and the corresponding travel times will also be stored.
   *
   * <p>Also updates ActiveRevisions so that new value will be written to db when the session is
   * closed.
   *
   * @param session
   * @param tripMap Map of all of the trips. Keyed on tripId.
   * @param travelTimeInfoMap Contains travel times that are available by trip pattern ID
   * @return the newly created travelTimesRev
   */
  private static int setTravelTimesForAllTrips(
      Session session, Map<String, Trip> tripMap, TravelTimeInfoMap travelTimeInfoMap) {
    ...
    for (Trip trip : tripMap.values()) {
      ...
      for (int stopIdx = 0; stopIdx < numStopsInTrip; ++stopIdx) {
        ...
        TravelTimeInfoWithHowSet travelTimeInfo = travelTimeInfoMap.getBestMatch(trip, stopIdx);
        ...
        if (travelTimeInfo != null) {
          ...
          if (travelTimeInfo.areTravelTimesValid()) {
            ...
            TravelTimesForStopPath originalTravelTimes = trip.getTravelTimesForStopPath(stopIdx);
            travelTimes = originalTravelTimes.getTravelTimesMsec();
            logger.error(
                "For trip={} stop={} invalid travel times from {} so falling back "
                    + "on old travel times from {}",
                trip,
                stopIdx,
                travelTimeInfo,
                originalTravelTimes);
          ...
          if (travelTimeInfo.isStopTimeValid()) {
            ...
            logger.error(
                "For trip={} stop={} invalid stop times from {} so falling back "
                    + "on old stop time from {}",
                trip,
                stopIdx,
                travelTimeInfo,
                originalTravelTimes);
          ...
          // travel time rev.
          ttForStopPathToUse = originalTravelTimes.clone(newTravelTimesRev);
        }

        // If already have created the exact same TravelTimesForStopPath
        // then use the existing one so don't generate too many db
        // objects.
        TravelTimesForStopPath cachedTTForStopPath = ttForStopPathCache.get(ttForStopPathToUse);
        if (cachedTTForStopPath == null) {
          ...
          ttForStopPathCache.put(ttForStopPathToUse, ttForStopPathToUse);
        } else {
  ...
  /**
   * Read in the current Trips that are configured for the active rev of the configuration. This
   * should be done after the historical data is read in so that less memory is used at once.
   *
   * @param agencyId
   * @param session
   * @return
  ...
  /**
   * Reads historic data from db and processes it, putting it all into a TravelTimeInfoMap. Then
   * stores the travel times for all of the trips.
   *
   * <p>Also updates ActiveRevisions so that new value will be written to db when the session is
   * closed.
   *
   * @param session
   * @param agencyId
   * @param specialDaysOfWeek Not fully implemented. Should therefore be null for now.
   * @param beginTime
   * @param endTime
   * @return the newly created travelTimeRev
   */
  private static int processTravelTimes(
      Session session,
      String agencyId,
      List<Integer> specialDaysOfWeek,
      Date beginTime,
      Date endTime) {
    ...
    TravelTimeInfoMap travelTimeInfoMap = processor.createTravelTimesFromMaps(tripMap);
  ...
  /**
   * Creates a session and reads historic data from db and processes it, putting it all into a
   ...
   * @param endTime
   */
  public static void manageSessionAndProcessTravelTimes(
      String agencyId, List<Integer> specialDaysOfWeek, Date beginTime, Date endTime) {
    ...
    try {
      ...
      newTravelTimesRev =
          processTravelTimes(session, agencyId, specialDaysOfWeek, beginTime, endTime);
    ...
    logger.info(
        "Done processing travel times. Changes successfully "
  ...
  /**
   * arg[0] specifies both the start date and end date. If an addition argument is specified it is
   * used as the end date. Otherwise the data is processed for just a single day.
   *
   * @param args
   */
  public static void main(String[] args) {
    ...
    logger.info("Processing travel times for beginTime={} endTime={}", startDateStr, endDateStr);

    // Do all the work...
    manageSessionAndProcessTravelTimes(agencyId, specialDaysOfWeek, beginTime, endTime);
