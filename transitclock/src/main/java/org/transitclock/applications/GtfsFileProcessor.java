...
public class GtfsFileProcessor {
   ...
   * @param configRev If not -1 then will use this config rev instead of incrementing from the
   *     current config rev from the db
   * @param shouldStoreNewRevs If true then will store the new config and travel times revs into
   *     ActiveRevisions table in db
  ...
  public GtfsFileProcessor(
    ...
    if (configFile != null) {
      try {
        ...
        logger.error("Error reading in config file \"" + configFile + "\". Exiting program.", e);
        System.exit(-1);
      }
    }
  ...
  private void obtainGtfsFiles() throws IllegalArgumentException {
    ...
    if (gtfsDirectoryName != null && (gtfsUrl != null || gtfsZipFileName != null)) {
      throw new IllegalArgumentException(
    ...
    if (gtfsUrl != null) {
      gtfsZipFileName =
          HttpGetGtfsFile.getFile(AgencyConfig.getAgencyId(), gtfsUrl, unzipSubdirectory);
    }
   ...
   * database. This is especially important for calendar dates. Needs to be called before the db is
   * first accessed in order to have an effect with postgres (with mysql can do so afterwards, which
   * is strange). So needs to be done before GtfsData() object is constructed.
   *
  ...
  /**
   * Handles command line option for doubles. If the option is not set or cannot be parsed then the
   ...
   * @return
   */
  private static double getDoubleCommandLineOption(
      String commandLineOption, double defaultValue, CommandLine commandLineArgs) {
  ...
  /**
   * Handles command line option for an int. If the option is not set or cannot be parsed then the
   * default value is used.
   *
   * @param commandLineOption
   * @param defaultValue
   ...
   * @return
   */
  private static int getIntegerCommandLineOption(
      String commandLineOption, int defaultValue, CommandLine commandLineArgs) {
    String valueStr = commandLineArgs.getOptionValue(commandLineOption);
    if (valueStr != null) {
      try {
  ...
  private static GtfsFileProcessor createGtfsFileProcessor(CommandLine commandLineArgs) {
    ...
    double pathOffsetDistance =
        getDoubleCommandLineOption("pathOffsetDistance", 0.0, commandLineArgs);
    double maxStopToPathDistance =
        getDoubleCommandLineOption("maxStopToPathDistance", 60.0, commandLineArgs);
    double maxDistanceBetweenStops =
        getDoubleCommandLineOption("maxDistanceBetweenStops", 6000.0, commandLineArgs);
    boolean disableSpecialLoopBackToBeginningCase =
        commandLineArgs.hasOption("disableSpecialLoopBackToBeginningCase");
    double maxDistanceForEliminatingVertices =
        getDoubleCommandLineOption("maxDistanceForEliminatingVertices", 3.0, commandLineArgs);
    int defaultWaitTimeAtStopMsec =
        getIntegerCommandLineOption(
            "defaultWaitTimeAtStopMsec", 10 * Time.MS_PER_SEC, commandLineArgs);
    double maxSpeedKph = getDoubleCommandLineOption("maxSpeedKph", 97.0, commandLineArgs);
    double maxTravelTimeSegmentLength =
        getDoubleCommandLineOption("maxTravelTimeSegmentLength", 200.0, commandLineArgs);
    int configRev = getIntegerCommandLineOption("configRev", -1, commandLineArgs);
  ...
  private static CommandLine processCommandLineOptions(String[] args) {
    ...
    options.addOption(
        OptionBuilder.withArgName("notes")
            .hasArg()
    ...
    options.addOption(
        "disableSpecialLoopBackToBeginningCase", false, "Should disable loopBackToBeggining");
    options.addOption(
        "storeNewRevs",
        false,
        "Stores the config and travel time revs into ActiveRevisions " + "in database.");

    options.addOption("skipDeleteRevs", false, "Delete the rev to be created first just in case.");
  ...
  /**
   * For actually invoking the GTFS processor. Note that command line options are used to set the
