...
/**
 * For generating more accurate schedule times for GTFS trips.txt file by using departure data
 * obtained via GPS.
 *
 * @author SkiBu Smith
 */
  ...
  private static CommandLine processCommandLineOptions(String[] args) {
    ...
    options.addOption(
        OptionBuilder.hasArg()
            ...
            .isRequired()
            .withDescription(
                "Begin date for reading arrival/departure "
    ...
    options.addOption(
        OptionBuilder.hasArg()
            .withArgName("MM-dd-yyyy")
            .withDescription(
                "Optional end date for reading arrival/departure "
                    + "times from database. Format is MM-dd-yyyy, "
    ...
    options.addOption(
        OptionBuilder.hasArg()
            ...
            .isRequired()
            .withDescription(
                "Specifies fraction of times that should. " + "Good value is probably 0.2")
            .create("f"));
    ...
    options.addOption(
        OptionBuilder.hasArg()
            .withArgName("secs")
            .withDescription(
                "How many seconds arrival/departure must be "
    ...
    options.addOption(
        OptionBuilder.hasArg()
            .withArgName("minutes")
            .withDescription(
                "For providing schedule adherence information. "
    ...
    options.addOption(
        "updateFirstStopOfTrip", false, "Set if should modify time even for first stops of trips.");
    ...
    if (desiredFractionEarly < 0.0 || desiredFractionEarly > 0.5) {
      System.err.println(
          "Paramater -f \"" + desiredFractionEarly + "\" must be between 0.0 and 0.5");
      System.exit(-1);
    }
    ...
    try {
      ...
      if (beginTime.getTime() > System.currentTimeMillis()) {
        System.err.println(
            "Paramater -b \"" + beginDateStr + "\" is in the future and therefore invalid!");
        System.exit(-1);
      }
    ...
    if (cmd.hasOption("allowableFromMean")) {
      ...
      try {
        ...
        System.err.println(
            "Option -allowableFromMean value \""
                + param
    ...
    if (cmd.hasOption("allowableFromOriginal")) {
      ...
      try {
        ...
        System.err.println(
            "Option -allowableFromOriginal value \""
                + param
                + "\" could not be parsed into an integer.");
        System.exit(-1);
      }
