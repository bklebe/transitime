...
public class Core {
  ...
  /**
   * Construct the Core object and read in the config data. This is private so that the createCore()
  ...
  /**
   * Creates the Core object for the application. There can only be one Core object per application.
   * Uses CoreConfig.getAgencyId() to determine the agencyId. This means it typically uses the
   * agency ID specified by the Java property -Dtransitclock.core.agencyId .
   *
  ...
  /**
   * For when need system time but might be in playback mode. If not in playback mode then the time
   * will be the time of the system clock. But if in playback mode then will be using a
  ...
  private static void processCommandLineOptions(String[] args) throws ParseException {
    ...
    options.addOption(
        OptionBuilder.withArgName("configRev")
            .hasArg()
            .withDescription(
                "Specifies optional configuration revision. "
    ...
    if (cacheReloadStartTimeStr.getValue().length() > 0
      ...
      */
      ...
      for (int i = 0; i < CoreConfig.getDaysPopulateHistoricalCache(); i++) {
        ...
        if (TripDataHistoryCacheFactory.getInstance() != null) {
          logger.debug(
              "Populating TripDataHistoryCache cache for period {} to {}", startDate, endDate);
          TripDataHistoryCacheFactory.getInstance()
              .populateCacheFromDb(session, startDate, endDate);
        }
      ...
      for (int i = 0; i < CoreConfig.getDaysPopulateHistoricalCache(); i++) {
        ...
        if (StopArrivalDepartureCacheFactory.getInstance() != null) {
          logger.debug(
              "Populating StopArrivalDepartureCache cache for period {} to {}", startDate, endDate);
          StopArrivalDepartureCacheFactory.getInstance()
              .populateCacheFromDb(session, startDate, endDate);
        }
      ...
      for (int i = 0; i < CoreConfig.getDaysPopulateHistoricalCache(); i++) {
        ...
        if (ScheduleBasedHistoricalAverageCache.getInstance() != null) {
          logger.debug(
              ...
              startDate,
              endDate);
          ScheduleBasedHistoricalAverageCache.getInstance()
              .populateCacheFromDb(session, startDate, endDate);
        }
  ...
  public static void main(String[] args) {
    try {
      ...
      if (optionalModuleNames.size() > 0)
        logger.info(
            "Starting up optional modules specified via "
