...
/**
 * Reads in a GTFS-realtime file for a Vehicle Positions feed and converts it to a CSV file
 * containing the AVL reports.
 *
 * @author SkiBu Smith
 */
public class ConvertGtfsRtToCsvFile {

  private static final Logger logger = LoggerFactory.getLogger(ConvertGtfsRtToCsvFile.class);
  ...
  private static StringConfigValue csvFileOutputDir =
      new StringConfigValue(
          "transitclock.avl.csvFileOutputDir",
          "/Users/Mike/gtfsRealtimeData/csv",
          "Name of directory where output will be written");

  private static String getTimeZoneStr() {
    if (timeZoneStr.getValue().isEmpty()) return null;
    else return timeZoneStr.getValue();
  }

  private static StringConfigValue timeZoneStr =
      new StringConfigValue(
          "transitclock.avl.timeZoneStr",
          "",
          "The timezone for the agency. In the form " + "\"America/New_York\"");
  ...
  private static BooleanConfigValue offsetForMapOfChina =
      new BooleanConfigValue(
          "transitclock.avl.offsetForMapOfChina",
          false,
  ...
  private static void processReport(
      AvlReport avlReport, Map<String, List<AvlReport>> avlReportsByVehicle) {
    String vehicleId = avlReport.getVehicleId();
    List<AvlReport> avlReportsForVehicle = avlReportsByVehicle.get(vehicleId);
   ...
   * @param avlReportsByVehicle
   */
  private static void writeCsvFiles(Map<String, List<AvlReport>> avlReportsByVehicle) {
  ...
  public static void main(String[] args) {
    ...
    for (String uri : uris) {
      ...
      Map<String, List<AvlReport>> avlReportsByVehicle = new HashMap<String, List<AvlReport>>();
