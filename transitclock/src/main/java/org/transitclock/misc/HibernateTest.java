...
/**
 * For testing writing data to database. Compares batch to stateless processing. Makes it easy to
 * see what happens when turn on logging and such.
 *
 * @author SkiBu Smith
 */
public class HibernateTest {
  ...
  /**
   * For comparing timing of doing a large number of inserts. Can either use regular batch mode
   * where there is a specified batch size but the caching and other internal features are still
   * used. Or can use a StatelessSession to bypass all of that internal stuff. This way can easily
  ...
  private static void timeBatchStore(int cnt, boolean batch) {
    ...
    try {
      ...
      try {
        Transaction tx =
            batch ? batchSession.beginTransaction() : statelessSession.beginTransaction();

        // Store some AVLReportss
        long initialTime = System.currentTimeMillis();
        for (int i = 0; i < cnt; ++i) {
          AvlReport report =
              new AvlReport((batch ? "batch" : "stateless") + i, initialTime + i, 1.23, 4.56, null);
          if (batch) batchSession.save(report);
          else statelessSession.insert(report);
