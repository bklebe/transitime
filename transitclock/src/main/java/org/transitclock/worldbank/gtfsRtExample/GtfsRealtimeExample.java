...
/**
 * For processing GPS data and outputting it into a GTFS-realtime format file. The data can come
 * from a database or can use test data. The data can be written to a file or output to stdout in
 * human readable format. Data can be obtained and written once or can continuously poll for data.
 * Writing data once is for when want to create a large data set for improving the accuracy of the
 * GTFS schedule times. The polling is for when want to provide a live realtime feed of
 * GTFS-realtime data.
 */
public class GtfsRealtimeExample {
  ...
  /**
   * Takes in ApiVehicle and puts it into a GTFS-realtime VehiclePosition object.
   *
   * @param vehicleData
  ...
  private static VehiclePosition createVehiclePosition(VehicleData vehicleData)
    ...
    VehicleDescriptor.Builder vehicle =
        VehicleDescriptor.newBuilder().setId(vehicleData.getVehicleId());
    ...
    if (vehicleData.getSpeed() != null) position.setSpeed(Float.parseFloat(vehicleData.getSpeed()));
    ...
    if (trip != null) vehiclePosition.setTrip(trip);
  ...
  private static FeedMessage createMessage(List<VehicleData> vehicleDataList) {
    ...
    for (VehicleData vehicleData : vehicleDataList) {
      FeedEntity.Builder vehiclePositionEntity =
          FeedEntity.newBuilder().setId(vehicleData.getVehicleId());
  ...
  /**
   * This method is from http://mindprod.com/jgloss/utf.html Copyright: (c) 2009-2014 Roedy Green,
   * Canadian Mind Products, http://mindprod.com
   *
   * <p>decode a String from UTF-8 bytes. We handle only 16-bit chars.
   *
   * <p>UTF-8 is normally decoded simply with new String( byte[], "UTF-8" ) or with an
   * InputStreamReader but this is roughly what goes on under the hood, if you ever need to write
   * your own decoder for some non-Java platform, or you are just curious how it works.
   *
   * <p>This works for 16-bit characters only. It does not handle 32-bit characters encoded with the
   * contortionist use of the low (0xdc00..0xdfff) and high(0xd800..0xdbff) bands of surrogate
   * characters.
   *
   ...
   * @return
   */
  private static String convertOctalEscapedStringToUtf(String octalEscapedString) {
    ...
    for (int i = 0, j = 0; i < originalBytes.length; ++i, ++j) {
      ...
      if (originalByte == '\\') {
        ...
        originalByte = (byte) ((octalDigit1 << 6) + (octalDigit2 << 3) + octalDigit3);
  ...
  private static void writeMessageToFile(FeedMessage message, String fileName) {
    try {
      ...
      if (outputHumanReadable) {
        ...
        String decodedMessage = convertOctalEscapedStringToUtf(message.toString());
  ...
  /**
   * Writes the GTFS-realtime data to a temporary file and then moves it to the proper place. This
   * way the file will always be complete.
   *
   * @param message the GTFS-realtime message
  ...
  /**
   * This main() method does all the work of retrieving the data and writing it to a GTFS-realtime
   * file. Also does the polling if so specified by the command line argument -polling.
   *
   * @param args
   */
  public static void main(String args[]) {
    ...
    do {
      ...
      System.out.println("Read in " + vehicleDataList.size() + " records of GPS data.");
      ...
      if (outputHumanReadable) {
        ...
        System.out.println(
            "Writing GTFS-realtime messsage to file \""
                + ConfigurableParameters.OUTPUT_FILE_NAME
                + "\" ...");
        if (usingPolling())
          overwriteMessageToFile(message, ConfigurableParameters.OUTPUT_FILE_NAME);
        else writeMessageToFile(message, ConfigurableParameters.OUTPUT_FILE_NAME);
      ...
      long waitTime = pollingTimeSeconds * 1000 - (System.currentTimeMillis() - loopStartTime);
      if (waitTime > 0) {
        try {
