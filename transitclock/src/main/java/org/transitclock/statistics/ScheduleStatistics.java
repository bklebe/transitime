...
/**
 * Contains methods for taking arrival/departure info that is collected in maps and processing it to
 ...
 *
 * <p>Note: convenient website for determining standard deviation is at
...
public class ScheduleStatistics {
   ...
   * determining more accurate schedule times.
   */
  ...
  /**
   * For the trip/stop specified by gtfsStopTime determines the Stats object which contains mean,
   * standard deviation, min, max, and number of data points. Sets everything in the Stats structure
   * except for the bestValue member.
   *
   * @param timesFromDb data for particular trip/stop. This list will be modified by filtering out
   *     data that is too far from the schedule time or too far from the mean.
   * @param originalGtfsStopTime The original data from stop_times.txt for this trip/stop. Used for
   *     filtering out outliers. Can be null when using test data instead of data from
   *     stop_times.txt.
   * @param arrivalsOrDepartures specifies whether should handle as arrivals or departures
   * @return Stats object containing the statistics for the trip/stop or null if there is no data.
   */
  private static Stats getStatisticsForTripStop(
    ...
    if (timesFromDb == null) return null;
    ...
    while (iterator.hasNext()) {
      ...
      if (Math.abs(time - mean) > allowableDifferenceFromMeanSecs) {
        ...
        logger.debug(
            ...
            originalGtfsStopTime == null ? "NA" : originalGtfsStopTime.getTripId(),
            originalGtfsStopTime == null ? "NA" : originalGtfsStopTime.getStopId(),
    ...
    if (timeFilteredOut) {
      ...
      results.mean = (float) Statistics.mean(results.filteredTimesArray);
    } else {
    ...
    for (int time : results.filteredTimesArray) {
      ...
      if (time < results.min) results.min = time;
      if (time > results.max) results.max = time;
    }
  ...
  /**
   * Goes through the Collection of Stats objects passed in and determines the fraction (0.0 - 1.0)
   * that are before the number of standardDeviations specified before the mean.
   *
   * @param standardDeviations The number of standard of deviations to subtract from the mean time
   *     for determining fraction of arrivals/departures that are early.
   * @param statistics Contains the times, the mean, and the standard deviation used for the
   *     calculations for all of the routes.
   * @return The fraction of arrival/departure times that are early
   */
  private static double determineFractionEarly(
      double standardDeviations, Collection<Stats> statistics) {
    int totalTimes = 0;
    int totalEarly = 0;
    for (Stats stats : statistics) {
      if (stats.filteredTimesArray.length >= 2) {
        ...
        for (int time : stats.filteredTimesArray) {
          double allowableTime = stats.mean - stats.standardDeviation * standardDeviations;
  ...
  /**
   * For the route specified first determines the basic stats including the mean and standard
   * deviation. Then determines how many standard deviations to use for this route such that get
   * approximately the desired fraction of times being earlier than the mean minus the number of
   * standard deviations for this trip/stop. Puts the resulting Stats objects that now contain the
   * best arrival/departure value to use into statsResultsByTripStopMap. This is done on a per route
   * basis since it is expected that the distribution of the stats will vary between routes.
   *
   ...
   * @param timesFromDbByTripStopForRouteSubMap the arrival/departure times for the route
   * @param statsResultsByTripStopMap for returning the results
   * @param routeId for logging
   * @param allowableDifferenceFromMeanSecs Specifies how consistent arrival/departure times from Db
   *     must be in order to be used.
   * @param allowableDifferenceFromOriginalTimeSecs Specifies how consistent arrival/departure times
   *     from Db must be in order to be used.
   * @param desiredFractionEarly Specifies the desired number of the arrival times that should be
   *     early
   * @param arrivalsOrDepartures specifies whether should handle as arrivals or departures
   */
  public static void determineStatsForRoute(
      ...
      ArrivalsOrDepartures arrivalsOrDepartures) {
    logger.debug("Processing {} data for routeId={}", arrivalsOrDepartures, routeId);

    // For the route get all of the statistics including the mean and
    // standard deviation for each stop
    Set<TripStopKey> tripStopKeysForRouteFromDb = timesFromDbByTripStopForRouteSubMap.keySet();
    Map<TripStopKey, Stats> statsForRoute =
        new HashMap<TripStopKey, Stats>(tripStopKeysForRouteFromDb.size());
    for (TripStopKey tripStopKey : tripStopKeysForRouteFromDb) {
      List<Integer> timesFromDb = timesFromDbByTripStopForRouteSubMap.get(tripStopKey);
      GtfsStopTime originalGtfsStopTime =
          originalGtfsStopTimes == null ? null : originalGtfsStopTimes.get(tripStopKey);
      Stats statsForTripStop =
          ScheduleStatistics.getStatisticsForTripStop(
              timesFromDb,
    ...
    for (int iteration = 0; iteration < NUMBER_ITERATIONS; ++iteration) {
      double fractionEarly = determineFractionEarly(currentStdDevs, statsForRoute.values());
      logger.debug(
          "For iteration={} currentStdDevs={} " + "desiredFractionEarly={} fractionEarly={}",
          iteration,
          currentStdDevs,
  ...
  /**
   * Determines and logs schedule adherence results both using the original schedule and using the
   * new improved schedule.
   *
   * @param gtfsStopTimes The original GtfsStopTimes read from stop_times.txt.
   * @param arrivalStatsResultsByTripStopMap All the arrival time results. Keyed on trip/stop.
   * @param departureStatsResultsByTripStopMap All the departure time results. Keyed on trip/stop.
   * @param allowableEarlySecs For schedule determining adherence
   * @param allowableLateSecs For schedule determining adherence
   */
  public static void processScheduleAdherence(
    ...
    Iterator<GtfsStopTime> gtfsStopTimesIterator = gtfsStopTimes.values().iterator();
    ...
    do {
      ...
      if (gtfsStopTimesIterator.hasNext()) nextGtfsStopTime = gtfsStopTimesIterator.next();
      else nextGtfsStopTime = null;
      ...
      TripStopKey tripStopKey =
          ScheduleDataProcessor.getTripStopKey(
              currentGtfsStopTime.getTripId(), currentGtfsStopTime.getStopId());
      if (nextGtfsStopTime == null
          || !currentGtfsStopTime.getTripId().equals(nextGtfsStopTime.getTripId())) {
      ...
      if (stats != null && originalScheduleTime != null) {
        ...
        for (int time : stats.unfilteredTimesArray) {
          if (time < originalScheduleTime - allowableEarlySecs) ++numberEarlyOrigSchedule;
          else if (time > originalScheduleTime + allowableLateSecs) ++numberLateOrigSchedule;
          if (time < newScheduleTime - allowableEarlySecs) ++numberEarlyNewSchedule;
          else if (time > newScheduleTime + allowableLateSecs) ++numberLateNewSchedule;
        }
      }
    ...
    double percentageOnTimeOrigSched =
        100.0
            * (totalDataPoints - numberEarlyOrigSchedule - numberLateOrigSchedule)
            / totalDataPoints;
    logger.info(
        "For original schedule time numDataPoints={} early={} " + "late={} percentageOnTime={}",
        totalDataPoints,
    ...
    double percentageOnTimeNewSched =
        100.0
            * (totalDataPoints - numberEarlyNewSchedule - numberLateNewSchedule)
            / totalDataPoints;
    logger.info(
        "For new schedule time numDataPoints={} early={} " + "late={} percentageOnTime={}",
        totalDataPoints,
