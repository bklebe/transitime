...
public class VehiclesServer extends AbstractServer implements VehiclesInterface {
  ...
  public Collection<IpcActiveBlock> getActiveBlocks(
    ...
    List<Block> blocks =
        BlocksInfo.getCurrentlyActiveBlocks(routeIds, null, allowableBeforeTimeSecs, -1);
  ...
  public Collection<IpcActiveBlock> getActiveBlocksWithoutVehicles(
    ...
    for (Block block : blocks) {
      try {
        ...
        IpcActiveBlock ipcBlockAndVehicle =
            new IpcActiveBlock(
                ipcBlock, activeTripIndex, new ArrayList<IpcVehicle>(), tripForSorting);
        results.add(ipcBlockAndVehicle);
      } catch (Exception e) {
        logger.warn(
            "Error while fecthing active blocks data (probably hibernate still loading data): "
                + e.getMessage());
      }
    }
    // Sort the results so that ordered by route and then block start time
  ...
  private Collection<IpcActiveBlock> getActiveBlocksAndVehiclesByRouteId(
    ...
    for (Block block : blocks) {
      ...
      if (routeIds != null
          && !routeIds.isEmpty()
          && !routeIds.contains(tripForSorting.getRouteId())) continue;
