...
public class PredictionsServer extends AbstractServer implements PredictionsInterface {
   ...
   * @return the singleton PredictionsServer object. Usually does not need to used since the server
  ...
  public List<IpcPredictionsForRouteStopDest> get(
      String routeIdOrShortName, String stopId, int predictionsPerStop) throws RemoteException {
  ...
  @Override
  public List<IpcPredictionsForRouteStopDest> get(
      List<RouteStop> routeStops, int predictionsPerStop) throws RemoteException {
   ...
   * route order, and then by direction. This way the most likely useful stops are displayed first.
   */
  private static Comparator<IpcPredictionsForRouteStopDest> predsByLocComparator =
      new Comparator<IpcPredictionsForRouteStopDest>() {
        ...
        public int compare(
            IpcPredictionsForRouteStopDest pred1, IpcPredictionsForRouteStopDest pred2) {
          ...
          if (routeOrderDifference < 0) {
            ...
            if (pred1.getDistanceToStop()
                > pred2.getDistanceToStop() + DISTANCE_AT_WHICH_ROUTES_GROUPED)
            ...
            if (pred2.getDistanceToStop()
                > pred1.getDistanceToStop() + DISTANCE_AT_WHICH_ROUTES_GROUPED)
  ...
  @Override
  public List<IpcPredictionsForRouteStopDest> get(
      Location loc, double maxDistance, int predictionsPerStop) throws RemoteException {
    ...
    for (StopInfo stopInfo : stopInfos) {
      ...
      List<IpcPredictionsForRouteStopDest> predictionsForStop =
          predictionDataCache.getPredictions(
