...
public class IpcPredictionsForRouteStopDest implements Serializable {
  ...
  public IpcPredictionsForRouteStopDest(
      TripPattern tripPattern, String stopId, double distanceToStop) {
    ...
    if (route == null) {
      throw new IllegalArgumentException(
          "RouteId=" + tripPattern.getRouteId() + " does not exist.");
    }
   ...
   * @param distanceFromStop For when getting predictions by location
   */
  private IpcPredictionsForRouteStopDest(
    ...
    synchronized (toClone) {
      ...
      for (int i = 0; i < size; ++i) {
        ...
        if (prediction.getPredictionTime() > maxSystemTimeForPrediction) break;
  ...
  /**
   * Constructor used for when deserializing a proxy object. Declared private because only used
   * internally by the proxy class.
   *
   * @param routeId
  ...
  /**
   * SerializationProxy is used so that this class can be immutable and so that can do versioning of
   * objects.
   */
  private static class SerializationProxy implements Serializable {
    ...
    private Object readResolve() {
      return new IpcPredictionsForRouteStopDest(
          routeId,
          routeShortName,
          routeName,
          routeOrder,
  ...
  /**
   * Gets a copy of this object. This is done with the object being copied synchronized so that the
   * predictions remain coherent. Limits number of predictions to maxPredictionsPerStop.
   *
   * @param maxPredictionsPerStop
   * @param distanceFromStop For when getting predictions by location
   * @return
  ...
  public IpcPredictionsForRouteStopDest getClone(int maxPredictionsPerStop, double distanceToStop) {
    ...
    IpcPredictionsForRouteStopDest clone =
        new IpcPredictionsForRouteStopDest(
            this, maxPredictionsPerStop, Long.MAX_VALUE, distanceToStop);
    return clone;
  }
   ...
   * predictions remain coherent. Limits number of predictions to maxPredictionsPerStop.
   *
   * @param maxPredictionsPerStop Won't copy more then this number of predictions
   * @param maxSystemTimeForPrediction Max point in future want predictions for. This way can limit
   *     predictions when requesting a large number of them.
  ...
  public IpcPredictionsForRouteStopDest getClone(
    ...
    IpcPredictionsForRouteStopDest clone =
        getClone(maxPredictionsPerStop, maxSystemTimeForPrediction, Double.NaN);
    return clone;
  }
   ...
   *
   * <p>Not sure if really need to synchronize removal of predictions from list since it is only a
   * single operation. But synching it is certainly the safe thing to do.
   *
   * @param oldPrediction
   */
  ...
  /**
   * Removes predictions that are older than the current time. Useful for when getting predictions.
   * Synchronized because don't want predictions list to be changing by another thread when removing
   * predictions.
   *
    ...
    while (iterator.hasNext()) {
      ...
      if (currentPrediction.getPredictionTime() < currentTime) {
        ...
        VehicleState vehicleState =
            vehicleStateManager.getVehicleState(currentPrediction.getVehicleId());
  ...
  /**
   * Updates the predictions for this object with the new predictions for a vehicle.
   *
   * <p>Synchronized because there are multiple steps in removing old predictions and creating new
   * ones.
   *
   * @param newPredsForRouteStopDest The new predictions for the vehicle
   * @param currentTime So can get rid of predictions that have expired.
   */
  public synchronized void updatePredictionsForVehicle(
      List<IpcPrediction> newPredsForRouteStopDest, long currentTime) {
    // If no predictions then nothing to do so return.
    if (newPredsForRouteStopDest == null || newPredsForRouteStopDest.isEmpty()) return;
  ...
  public String toString() {
    return "IpcPredictionsForRouteStopDest ["
        ...
        + directionId
        + ", distanceToStop="
        + Geo.distanceFormat(distanceToStop)
        + ", predictionsForRouteStop="
        + predictionsForRouteStopDest
        + "]";
  }
  ...
  /**
   * Distance from location to the stop if getting predictions by location. Otherwise will return
   * Double.NaN.
   *
   * @return
   */
