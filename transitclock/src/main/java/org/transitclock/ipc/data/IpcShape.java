...
public class IpcShape implements Serializable {
  ...
  /**
   * Adds list of locations for a stop path to the location member. Tries to reduce number of
   * segments by combining segment for previous stop path with segment of new stop path if they are
   * going in straight line within MAX_VERTEX_DISTANCE.
   *
   * @param locs List of locations for a stop path to be added
   */
  public void add(List<Location> locs) {
    ...
    for (int i = 1; i < locs.size(); ++i) locations.add(locs.get(i));
  }
