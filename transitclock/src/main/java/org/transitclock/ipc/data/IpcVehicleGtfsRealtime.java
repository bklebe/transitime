 ...
 * Made a separate class from IpcVehicle since the expect other interfaces, such as the JSON/XML
 * interface, to be dominant and don't want to bog down all the other IpcVehicle requests with
 * extraneous data.
 *
 * @author SkiBu Smith
 */
public class IpcVehicleGtfsRealtime extends IpcVehicle {
  ...
  public IpcVehicleGtfsRealtime(VehicleState vs) {
    ...
    if (temporalMatch != null) {
      ...
      // transit to a stop.
      StopPath stopPath = atStop ? match.getAtStop().getStopPath() : match.getStopPath();
   ...
   * @param isCanceled
   */
  protected IpcVehicleGtfsRealtime(
      String blockId,
      ...
      boolean isDelayed,
      boolean isLayover,
      long layoverDepartureTime,
      String nextStopId,
      String nextStopName,
      String vehicleType,
      long tripStartEpochTime,
      boolean atStop,
      String atOrNextStopId,
      Integer atOrNextGtfsStopSeq,
      long freqStartTime,
      IpcHoldingTime holdingTime,
   ...
   */
  protected static class GtfsRealtimeVehicleSerializationProxy extends SerializationProxy {
    ...
    private Object readResolve() {
      return new IpcVehicleGtfsRealtime(
          blockId,
          blockAssignmentMethod,
          avl,
          heading,
  ...
  /**
   * Returns stop ID of stop that vehicle is at or for the next stop if vehicle is in between stops.
   *
   * @return Stop ID of stop that vehicle is currently at or is transitioning to
   */
  ...
  /**
   * Returns the GTFS stop sequence of the stop the vehicle is at or is moving to.
   *
