 ...
 *
 * <p>Declared serializable since using RMI to pass back Prediction objects and RMI uses
 * serialization.
 *
 * @author SkiBu Smith
 */
public class IpcPrediction implements Serializable {
   ...
   * @param stopId
   * @param gtfsStopSeq
   * @param trip Can be set to null for testing but usually will be a valid trip
   * @param predictionTime The prediction to present to the user. Can be different from
   *     actualPredictionTime in that for wait stops might want to show user the scheduled time even
   *     if the actual prediction time, including the stop wait time, might be greater.
   * @param actualPredictionTime The prediction time including all factors, including the stop wait
   *     time for wait stops presented to the user is different than the actual prediction time.
   *     Only used on server side when calculating predictions.
   * @param atEndOfTrip True if prediction for last stop of trip, which means likely not useful to
   *     user
  ...
  public IpcPrediction(
    ...
    this.tripStartEpochTime =
        Core.getInstance().getTime().getEpochTime(trip.getStartTime(), currentTime);
    ...
    this.lateAndSubsequentTripSoMarkAsUncertain = lateAndSubsequentTripSoMarkAsUncertain;
  ...
  private static class SerializationProxy implements Serializable {
    ...
    private SerializationProxy(IpcPrediction p) {
      ...
      this.lateAndSubsequentTripSoMarkAsUncertain = p.lateAndSubsequentTripSoMarkAsUncertain;
  ...
  public String toString() {
    return "IpcPrediction ["
        ...
        + (isDelayed ? ", delayed=t" : "")
        + (lateAndSubsequentTripSoMarkAsUncertain
            ? ", lateAndSubsequentTripSoMarkAsUncertain=t"
            : "")
        + ", arrival="
        + (isArrival ? "t" : "f")
        ...
        + (!Float.isNaN(passengerFullness)
            ? ", psngrFullness=" + StringUtils.twoDigitFormat(passengerFullness)
            : "")
  ...
  /**
   * Returns the passenger count as obtained from the AVL feed. The value will not be valid for
   * systems that do not have passenger counting sensors. Therefore should use
   * isPassengerCountValid() to determine if the value is valid.
   *
  ...
  /**
   * Returns the trip associated with the prediction. Only valid on server side since trip is not
   * passed to client.
   *
   * @return
   */
  ...
  /**
   * Returns the short name for the route associated with the prediction. Only valid on server side
   * since uses the trip member and the trip is not passed to client.
   *
   * @return
   */
