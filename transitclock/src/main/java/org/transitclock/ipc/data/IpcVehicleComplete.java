...
/**
 * Extension of IpcVehicle class so that all info describing vehicle is included. All this info is
 * put into the VehicleDataCache so that can get whatever data is needed be the remote clients. In
 * particular, includes additional info for SIRI feed. Made a separate class from IpcVehicle since
 * the SIRI feed is not expected to get significant use (it is a poor interface) and don't want to
 * bog down all the other IpcVehicle requests with extraneous data.
 *
 * @author SkiBu Smith
 */
public class IpcVehicleComplete extends IpcVehicleGtfsRealtime {
  ...
  public IpcVehicleComplete(VehicleState vs) {
    ...
    if (trip != null) {
      List<String> stopIdsForTrip = vs.getTrip().getTripPattern().getStopIds();
  ...
  private IpcVehicleComplete(
      ...
      Double distanceOfNextStopFromTripStart,
