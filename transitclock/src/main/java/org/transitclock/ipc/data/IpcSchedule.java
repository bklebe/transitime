...
public class IpcSchedule implements Serializable {
   ...
   */
  private IpcSchedule(
      Route route, String directionId, String directionName, String serviceId, String serviceName) {
    super();
  ...
  /**
   * Use custom serialization so that only the members that are not transient are actually
   * serialized.
   *
   * @param oos
   * @throws IOException
  ...
  private static class ServiceDirectionKey extends MapKey {
    ...
    public String toString() {
      return "ServiceDirectionKey [" + "serviceId=" + o1 + ", directionId=" + o2 + "]";
    }
  }

  /**
   * Sorts the trips in order of schedule times. This is a bit difficult because won't always have
   * times for the same stops for a trip. Basically need to go through stops for the trips and find
   * the first common stop and compare times for it.
   */
            ...
            for (int i1 = 0; i1 < numberStopPaths1; ++i1) {
              ...
              if (st1 != null && st1.getTime() != null) {
                ...
                for (int i2 = 0; i2 < numberStopPaths2; ++i2) {
                  ...
                  // to one in o1...
                  if (st2 != null && st2.getTime() != null && stopId1.equals(stopId2)) {
                    // Found a match!
                    if (st1.getTime() < st2.getTime()) return -1;
                    else if (st1.getTime() > st2.getTime()) return 1;
                    else return 0;
                  }
                }
              }
  ...
  /**
   * Sorts the list of IpcSchedule objects. Puts the service classes with more trips first. This
   * should put weekday before Saturday before Sunday. Also sorts by direction ID when service class
   * is the same. And if number of trips is the same then tries to put Sunday last by looking at the
   * serviceId.
   *
        ...
        new Comparator<IpcSchedule>() {
          ...
          public int compare(IpcSchedule o1, IpcSchedule o2) {
            if (o1.getServiceId().equals(o2.getServiceId())) {
              ...
              if (o1.getIpcSchedTrips().size() < o2.getIpcSchedTrips().size()) return 1;
              else if (o1.getIpcSchedTrips().size() > o2.getIpcSchedTrips().size()) return -1;
              else {
                ...
                // after "Saturday"
                if (o2.getServiceId().toLowerCase().contains("sun")) return -1;
                else return 0;
              }
            }
          }
  ...
  /**
   * Creates a IpcSchedule for each service ID and each direction for the blocks for the route.
   *
   * @param route
  ...
  public static List<IpcSchedule> createSchedules(Route route, List<Block> blocksForRoute) {
    ...
    for (Block block : blocksForRoute) {
      ...
      for (Trip trip : block.getTrips()) {
        ...
        ServiceDirectionKey key = new ServiceDirectionKey(serviceId, directionId);
        IpcSchedule ipcSchedule = schedulesMap.get(key);
        if (ipcSchedule == null) {
          ipcSchedule = new IpcSchedule(route, directionId, directionName, serviceId, serviceName);
          schedulesMap.put(key, ipcSchedule);
        }

        // Add current trip to the IpcSchedule
        if (!ipcSchedule.trips.contains(trip)) ipcSchedule.trips.add(trip);
      }
    }
