...
public class AgencyMonitorClient {
  ...
  /**
   * Does a simple ping to each agency to make sure that the core for that agency is running and can
   * communicate with it via IPC.
   *
  ...
  /**
   * Uses RMI to invoke monitoring of the agency core to see if everything is operating properly. If
   * there is a problem then and error message is returned. If everything OK then null returned.
   *
  ...
  /**
   * Goes through all agencies in the WebAgency database table that are marked as active and monitor
   * them to see if there are any problems. The monitoring on the server side will automatically
   * send out e-mail notifications if there is a problems. This method sends out e-mails if there is
   * a problem connecting to an agency.
   *
