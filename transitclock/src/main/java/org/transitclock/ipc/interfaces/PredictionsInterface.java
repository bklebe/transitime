...
public interface PredictionsInterface extends Remote {
  ...
  public List<IpcPredictionsForRouteStopDest> get(
      String routeShortName, String stopId, int predictionsPerStop) throws RemoteException;

  /**
   * For each route/stop specified returns a list of predictions for that stop. Since expensive RMI
   * calls are being done this method is much more efficient for obtaining multiple predictions then
   * if a separate get() call is done for each route/stop.
   *
   * @param routeStops List of route/stops to return predictions for. Uses route short name or route
   *     ID
   * @param predictionsPerStop Max number of predictions to return per route/stop
   * @return List of PredictionsForRouteStop objects for the route/stop. There is a separate one for
