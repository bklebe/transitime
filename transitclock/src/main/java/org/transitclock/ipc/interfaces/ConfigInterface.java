...
public interface ConfigInterface extends Remote {
   ...
   *     used instead of routeId since routeIds unfortunately often change when there is a schedule
   *     change.
   * @param directionId optional. If want UI to highlight the remaining stops and paths left in the
   *     trip then can specify directionId along with the stopId. The directionId can be needed for
   *     agencies where only a single stop is used for both directions for a route.
   * @param stopId optional. If want UI to highlight the remaining stops and paths left in trip then
   *     stopId is used to return which stops remain in trip. If this additional info not needed for
   ...
   * @throws RemoteException
   */
  public IpcRoute getRoute(
      String routeIdOrShortName, String directionId, String stopId, String tripPatternId)
      throws RemoteException;
  ...
  /**
   * Returns block info for specified blockId and serviceId. Includes all trip and trip pattern info
   ...
   * looks for a trip with a trip_short_name that matches the tripId. This way can easily find trips
   * using trip_short_name, which for agencies such as MBTA commuter rail is more frequently used.
