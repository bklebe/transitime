...
/**
 * For clients to access RMI based method calls for a remote object simply need to call something
 * like: Hello hello = ClientFactory.getInstance(agencyId, Hello.class);
 *
 * @author SkiBu Smith
 */
public class ClientFactory<T extends Remote> {
  ...
  private static StringConfigValue debugRmiServerHost =
   ...
   * created every time getInstance() is called. This is reasonable because usually will only be
   * creating a single such object and if there are a few that is fine as well since they are not
   * that expensive.
   *
   * @param host Where remote object lives
   * @param agencyId For creating bind name for remote object
   * @param clazz Because this is a static method the only way to get the class name is to pass in
   *     the class.
   * @return Proxy to the RMI object on the server. If there is a problem accessing the object then
   *     null is returned.
   */
  public static <T extends Remote> T getInstance(String agencyId, Class<T> clazz) {
    ...
    try {
      ...
      logger.error(
          "Exception occurred when creating the RMI client "
  ...
  /**
   * Creates an RMI stub based on the project name, host name, and class name. An RMI stub is a
   * remote reference to an object.
   *
   * @param info Species the agency ID and the host name
   * @param updateHostName Indicates whether should update host name instead of just getting old
   *     values from cache. Should update if there is a problem in invoking an RMI call since
   *     perhaps the server for an agency was moved to a new hostname. But when first creating a
   *     client factory don't want to access db so should just use cached value.
   * @return The RMI stub for the remote object
   * @throws RemoteException
   * @throws NotBoundException
   */
  public static <T extends Remote> T getRmiStub(RmiStubInfo info, boolean updateHostName)
    ...
    logger.debug(
        "Getting RMI registry for hostname={} port={} ...", hostName, RmiParams.getRmiPort());
    // Get the registry
    Registry registry = LocateRegistry.getRegistry(hostName, RmiParams.getRmiPort());

    // Get the remote object's bind name
    String bindName = AbstractServer.getBindName(info.getAgencyId(), info.getClassName());

    logger.debug(
        "Got RMI registry. Getting RMI stub from registry for " + "bindName={} ...", bindName);
  ...
  /**
   * Sets the RMI timeout if haven't done so yet. This way RMI calls will not just hang if can't
   * connect.
   */
