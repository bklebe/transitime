...
public class RmiStubInfo {
  ...
  /**
   * Returns the RMI hostname. Will use command line parameter -Dtransitclock.core.rmiHost if it is
   * set. If not set then looks in WebAgencies table in the web database. Returns null if not
   * configured.
   *
   * @param rereadIfOlderThanMsecs If web agencies read from db more than this time ago then they
   *     are read in again. Set to a high value such as Integer.MAX_VALUE if there is no indication
   *     of a problem, such as when a ClientFactory is first setup. But if there is a problem
   *     communicating via RMI then should use a small value such as 30 seconds so that the system
   *     will read in possibly new host name from the database, yet not read db for every
   *     problematic request.
  ...
  /**
   * Returns the RMI hostname that is cached. Will not update the cache even if agency not in cache
   * and cache not updated for a long time. Will use command line parameter
  ...
  /**
   * Intended for when communication was working but now is not in that this method will get updated
   * hostname data from db if cache is somewhat old. Returns the RMI hostname. Will use command line
   * parameter -Dtransitclock.core.rmiHost if it is set. If not set then looks in cached version of
   * WebAgencies table from the web database. If cached data is more than 30 seconds old then will
   * reread WebAgency data from db. Returns null if agency not configured.
   *
   * @return The host name, or null if agency doesn't exist
   */
