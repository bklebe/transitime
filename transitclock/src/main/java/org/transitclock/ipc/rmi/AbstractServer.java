 ...
 * an e-mail alert when rebinding if the rmiregistry is not available.
 *
 * <p>Once a AbstractServer superclass object is created it will continue to exist.
 *
 * <p>AbstractServer configures RMI to not use port 0, basically a random port, when establishing
 * secondary communication. Instead, RMI is configured to use port 2098. In this way the firewalls
 * for the servers can be configured to only allow traffic on the two ports 2099 & 2098 for RMI. If
 * there are multiple systems running on a single server then need to use a separate secondary port
 * for each system (such as 2097, 2096, etc). Using a specific port for secondary communication is
 * much safer than having to completely open up network traffic.
 *
 * <p>If another agency is already using the secondary RMI port on the server then will get an
 * exception and e-mail notification is sent out.
 *
 * @author SkiBu Smith
 */
public abstract class AbstractServer {
  ...
  /**
   * Binds the RMI implementation object to the registry so that it can be accessed by a client. It
   * also periodically rebinds the object every 30 seconds so that if the rmiregistry gets restarted
   * the RMI object will then automatically become available again. Sends an e-mail alert when
   * rebinding if the rmiregistry is not available.
   *
   * <p>Protected because should only be instantiated by a superclass factory.
   *
   * <p>This constructor "leaks" a reference to the object before the constructor has completed via
   * UnicastRemoteObject.exportObject(). This normally should not be done because the function could
   * cause the object to be used before it has been fully constructed. But by leaking the object we
   * can guarantee that any Server class that inherits from this class will be properly initialized
   * when the subclass constructor is called. This is fine as long as the Server object constructor
   * doesn't do any further initialization.
   *
   * @param agencyId For registering object with rmiregistry. Can have several different projects
   *     running on a server so need to specify the project name.
   * @param objectName For registering object with rmiregistry. Will usually use something like
  ...
  protected AbstractServer(String agencyId, String objectName) {
    ...
    try {
      ...
      logger.info(
          "Setting up AbstractServer for RMI using secondary " + "port={}",
  ...
  private String rebindErrorMessage(Exception e) {
    ...
    String msg =
        "It appears that the rmiregistry is not running on host "
            + hostname
  ...
  /**
   * The name that the server object is bound to needs both the classname to identify the object but
   * also the agencyId since multiple projects can be running on a machine that uses a single RMI
   * registry.
   *
   * @param agencyId
   * @param className
