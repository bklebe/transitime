...
public class AvlConfig {
  ...
  private static IntegerConfigValue secondsBetweenAvlFeedPolling =
      new IntegerConfigValue(
          "transitclock.avl.feedPollingRateSecs",
          5,
          "How frequently an AVL feed should be polled for new data.");
  ...
  private static DoubleConfigValue alternativeMaxSpeed =
      new DoubleConfigValue(
  ...
  private static IntegerConfigValue maxStopPathsAhead =
      new IntegerConfigValue(
          "transitclock.avl.maxStopPathsAhead", 999, "Max stopPaths ahead to look for match.");
  /**
  ...
  /**
   * For filtering out bad AVL reports. The default values of latitude 15.0 to 55.0 and longitude of
   * -135.0 to -60.0 are for North America, including Mexico and Canada. Can see maps of lat/lon at
  ...
  /**
   * So can filter out unpredictable assignments such as for training coaches, service vehicles, or
   * simply vehicles that are not in service and should not be attempted to be made predictable.
   * Returns empty string, the default value if transitclock.avl.unpredictableAssignmentsRegEx is
   * not set.
   */
