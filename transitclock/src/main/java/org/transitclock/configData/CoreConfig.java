...
public class CoreConfig {
   ...
   * will be sped up by not generating nor logging predictions, not logging AVL data nor storing it
   * in db, and not logging nor storing match data in db.
   *
   * @return
   */
   ...
   * DataDbLogger queue the calling thread will be temporarily suspended so that the separate thread
   * can run to write to the db and thereby empty out the queue.
   *
   * @return
   */
   ...
   * from the end of the block then the spatial match will not be used. This is to prevent a vehicle
   * that has already completed its block from wrongly being assigned to that block again.
   *
   * @return
   */
   ...
   * determining the arrival time at the last stop of the block even if don't get an AVL report near
   * that stop.
   *
   * @return
   */
  ...
  /**
   * How early a vehicle can be and still be matched to a layover. Needs to be pretty large because
   * sometimes vehicles will be assigned to a layover quite early, and want to be able to make the
   * vehicle predictable and generate predictions far in advance. Don't want it to be too large,
   * like 90 minutes, though because then could match incorrectly if vehicle simply stays at
   * terminal.
   *
   * @return
   */
   ...
   * expected to be at a layover and start the next trip. Should usually be a pretty large value so
   * that vehicles are not taken out of service just because they drive a bit away from the stop for
   ...
   * that vehicles moving around during layover have started their trip early. Therefore this value
   * should be limited to just a few minutes since vehicles usually don't leave early.
   *
