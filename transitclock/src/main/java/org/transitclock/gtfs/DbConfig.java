...
public class DbConfig {
  ...
  private StringConfigValue validateTestQuery =
      new StringConfigValue(
          "transitclock.db.validateQuery", "SELECT 1", "query to validate database connection");
   ...
   *
   * <p>NOTE: exits system if config data could not be read in. This is done so that action will be
  ...
  public void read(int configRev) {
    ...
    // Let user know what is going on
    logger.info(
        "Finished reading configuration data from database . " + "Took {} msec.",
        timer.elapsedMsec());
  }
   ...
   * getBlocksForRoute() can be used to retrieve the list of blocks that are associated with a route
   * for a specified service ID.
   *
   * @param blocks
  ...
  /**
   * Returns the list of trip patterns associated with the specified route. Reads the trip patterns
   * from the database and stores them in cache so that subsequent calls get them directly from the
   * cache. The first time this is called it can take a few seconds. Therefore this is not done at
  ...
  public List<TripPattern> getTripPatternsForRoute(String routeId) {
    ...
    if (tripPatternsByRouteMap == null) {
      logger.error(
          "tripPatternsByRouteMap not set when " + "getTripPatternsForRoute() called. Exiting!");
      System.exit(-1);
    }
  ...
  /**
   * Returns cached map of all Trips. Can be slow first time accessed because it can take a while to
   * read in all trips including all sub-data.
   *
   * @return
   */
  ...
  /**
   * For more quickly getting a trip. If trip not already read in yet it only reads in the specific
   * trip from the db, not all trips like getTrips(). If trip ID not found then sees if can match to
   * a trip short name.
   *
  ...
  public Trip getTrip(String tripIdOrShortName) {
    ...
    if (trip == null) {
      logger.debug(
          "Trip for tripIdOrShortName={} not read from db yet " + "so reading it now.",
          tripIdOrShortName);
  ...
  /**
   * For more quickly getting a trip. If trip not already read in yet it only reads in the specific
   * trip from the db, not all trips like getTrips().
   *
   * @param tripShortName
   * @return
   */
  public Trip getTripUsingTripShortName(String tripShortName) {
    ...
    if (trips != null) {
      ...
      if (trip != null) {
        ...
        logger.debug(
            "When reading tripShortName={} found trips " + "but not for current service.",
            tripShortName);
        return null;
      }
    }
  ...
  public int getBlockCount() {
    ...
    for (String serviceId : blocksByServiceMap.keySet()) {
      blockCount +=
          (blocksByServiceMap.get(serviceId) != null
  ...
  /**
   * Returns CalendarDate for the current day. This method is pretty quick since it looks through a
   * hashmap, instead of doing a linear search through a possibly very large number of dates.
   *
  ...
  private static class ValidateSessionThread implements Runnable {
    ...
    public void run() {
      ...
      while (!Thread.interrupted()) {
        ...
        try {
          SQLQuery query =
              service.getGlobalSession().createSQLQuery(dbConfig.getValidateTestQuery());
          query.list();
