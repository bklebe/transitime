...
public class DbWriter {
  ...
  private void actuallyWriteData(Session session, int configRev, boolean cleanupRevs) {
    if (cleanupRevs) {
      ...
      logger.info(
          "Deleting old blocks and associated trips from rev {} of " + "database...", configRev);
    ...
    logger.info(
        "Saving {} blocks (plus associated trips) to database...", gtfsData.getBlocks().size());
    int c = 0;
    ...
    for (Block block : gtfsData.getBlocks()) {
      ...
      if (c % 1000 == 0) {
        logger.info(
            "wrote " + c + " blocks in " + (System.currentTimeMillis() - startTime) / 1000 + "s");
      }
    }
