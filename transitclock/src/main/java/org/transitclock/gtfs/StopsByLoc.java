...
public class StopsByLoc {
  ...
  public static class StopInfo {
    ...
    public StopInfo(
        TripPattern tripPattern, String routeShortName, String stopId, double distanceToStop) {
  ...
  /**
   * For the specified trip pattern, determines the closest stop to the location. Will return null
   * if the best stop is further away than maxDistance. Will return null if stop is last one for
   * trip pattern, indicating that passengers cannot board there.
   *
   * @param tripPattern
   * @param loc
   * @param maxDistance
   * @return The closest stop for the trip pattern that is not the last stop of the trip pattern and
   *     is within maxDistance of the loc.
   */
  private static StopInfo determineClosestStop(
      TripPattern tripPattern, Location loc, double maxDistance) {
    ...
    for (int i = 0; i < stopPaths.size(); ++i) {
      ...
      if (distanceToStop < bestDistance + BIAS_TO_NEXT_STOP_OFFSET) {
        ...
        if (i == stopPaths.size() - 1) return null;
    ...
    return new StopInfo(
        tripPattern, tripPattern.getRouteShortName(), bestStopPath.getStopId(), bestDistance);
  }

  /**
   * Determines from the matchesForDirection parameter the stop that is nearest that actually hash
   * predictions. This way won't return a stop for a trip pattern that is not currently in service.
   * Yet will still return closest viable stop.
   *
  ...
  private static StopInfo determineBestStopBasedOnPredictions(List<StopInfo> matchesForDirection) {
    ...
    for (StopInfo stopInfo : matchesForDirection) {
      List<IpcPredictionsForRouteStopDest> predictionsForStop =
          PredictionDataCache.getInstance()
      ...
      if (!predictionsForStop.isEmpty()
          && (nearestStopWithPrediction == null
              || stopInfo.distanceToStop < nearestStopWithPrediction.distanceToStop)) {
   ...
   */
  private static boolean matchesAreForSameStop(List<StopInfo> matchesForDirection) {
    if (matchesForDirection.size() == 1) return true;

    String firstStopId = matchesForDirection.get(0).stopId;
    for (StopInfo stopInfo : matchesForDirection) {
      if (!stopInfo.stopId.equals(firstStopId)) return false;
    }
  ...
  /**
   * Gets list of stops that are within maxDistance of the specified location. Looks at every trip
   * pattern so can deal with complicated cases such as routes with school service stops just for
   * part of the day.
   *
   * @param loc
   * @param maxDistance
  ...
  public static List<StopInfo> getStops(Location loc, double maxDistance) {
    ...
    for (Route route : dbConfig.getRoutes()) {
      ...
      if (!route.getExtent().isWithinDistance(loc, maxDistance)) continue;
      ...
      for (String directionId : route.getDirectionIds()) {
        ...
        if (matchesForDirection.size() >= 1 && matchesAreForSameStop(matchesForDirection)) {
          ...
          StopInfo stopInfo = determineBestStopBasedOnPredictions(matchesForDirection);

          if (stopInfo != null) results.add(stopInfo);
        }
      }
    }
