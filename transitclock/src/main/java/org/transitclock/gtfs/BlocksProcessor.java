...
public class BlocksProcessor {
  ...
  public BlocksProcessor(GtfsData gtfsData) {
    ...
    if (gtfsData.getTripPatternMap() == null || gtfsData.getTripPatternMap().isEmpty()) {
      logger.error(
          "processStopTimesData() must be called before " + "BlocksProcessor() is. Exiting.");
      System.exit(-1);
    }
  }

  /**
   * For some routes want "unscheduled" assignments that use the longest trip pattern for each
   * direction but have no schedule time associated. Useful for special situations such as baseball
   * game events where transit agency needs to add additional unscheduled service.
   *
   * <p>TODO need to fix this code so that special trip patterns are used that have an unscheduled
   * stop at beginning so that system knows not to make predictions until vehicle has left the
   * terminal.
   *
   * @param configRev
   * @param blocks List of blocks that the unscheduled assignments should be added
   * @param serviceIdsUsed The service IDs that the unscheduled blocks should be created for.
   */
  private void addUnscheduledBlocks(int configRev, List<Block> blocks, Set<String> serviceIdsUsed) {
    ...
    for (GtfsRoute gtfsRoute : gtfsRoutes) {
      if (gtfsRoute.shouldCreateUnscheduledBlock()) {
        ...
        String blockId =
            (gtfsRoute.getRouteShortName() != null
                    ? gtfsRoute.getRouteShortName()
                    : gtfsRoute.getRouteId())
        ...
        for (TripPattern tripPattern : tripPatternsForRoute) {
          ...
          if ("0".equals(tripPattern.getDirectionId()) || tripPattern.getDirectionId() == null) {
            // It is direction "0"
  ...
  /**
   * Actually processes the trips into block assignments. Includes "unscheduled" block assignments
   * for the routes that have been configured for such.
   *
   * @param configRev
  ...
  public List<Block> process(int configRev) {
    ...
    for (String serviceId : tripListByBlocksByServiceMap.keySet()) {
      ...
      for (String blockId : tripListForBlocksMap.keySet()) {
        ...
        if (firstTripForBlock == null || firstTripForBlock.getStartTime() == null) continue;
