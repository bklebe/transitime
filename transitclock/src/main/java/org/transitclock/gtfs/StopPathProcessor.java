...
/**
 * Part of GtfsData class. Processes the shapes.txt data and converts into the segments for the
 * associated stopPaths between the stops.
 *
 * @author SkiBu Smith
 */
public class StopPathProcessor {
   ...
   * @param stopsMap
   * @param tripPatterns
   * @param offsetDistance How much to the right the stopPaths should be offset. If negative then
   *     the stopPaths are offset to the left. If 0.0 then of course the stopPaths are not offset at
   *     all.
   ...
   * @param Indicates that shouldn't use the shape before the first stop of the trip. This can be
   *     useful because sometimes a single shape is used for multiple trip patterns or the shapes
   *     simply have some problem points at the beginning (like sfmta 21-Hayes.
   */
  public StopPathProcessor(
      Collection<GtfsShape> gtfsShapes,
  ...
  /**
   * For determining the stopPaths for a trip pattern when there is no shape defined in the GTFS
   * shapes.txt file for that trip pattern. Simply connects the stops in the trip pattern with a
   * straight line. The first stop gets a path that has a segment of length 0.
   *
   * @param tripPattern
   */
  ...
  /**
   * Offsets the stopPaths to the right the distance specified. Useful for when shape data is street
   * center line. By offsetting the data the stopPaths in the different directions won't overlap on
   * the map when zoomed in adequately. Also, makes debugging easier since won't have confusing
   * overlapping stopPaths.
   *
   * @param gtfsShapes List of GtfsShape objects that represent the original path for the shape.
   * @return
   */
  private List<Location> getOffsetLocations(List<GtfsShape> gtfsShapes) {
    ...
    Location beginningOffsetLoc =
        Geo.rightOffsetBeginningLoc(
            gtfsShape0.getLocation(), gtfsShape1.getLocation(), offsetDistance);
    ...
    for (int i = 0; i < gtfsShapes.size() - 2; ++i) {
      ...
      Location offsetVertex =
          Geo.rightOffsetVertex(
              g1.getLocation(), g2.getLocation(), g3.getLocation(), offsetDistance);
    ...
    Location endOffsetLoc =
        Geo.rightOffsetEndLoc(gtfsShape0.getLocation(), gtfsShape1.getLocation(), offsetDistance);
  ...
  private BestMatch determineBestMatch(
    ...
    for (int shapeIndex = previousShapeIndex; shapeIndex < shapeLocs.size() - 1; ++shapeIndex) {
      ...
      if (stopToShapeDistance < bestStopToShapeDistance - 0.0001) {
        ...
        if (!specialLoopBackToBeginningCase) {
          ...
          bestMatch.distanceAlongPattern =
              dististanceOverPattern - (shapeVector.length() - bestMatch.distanceAlongShape);
        }
      }
    ...
    logger.debug(
        " bestMatch {} expected distanceAlongPattern {}",
        bestMatch,
        stopPath.getShapeDistanceTraveled());
  ...
  /**
   * For the trip pattern, goes through the stops and matches them to the shapes from the shapes.txt
   * file. StopPath segments are created for each stop and the TripPattern is updated accordingly.
   *
  ...
  private void determinePathSegmentsMatchingStopsToShapes(
    ...
    for (int stopIndex = 0; stopIndex < tripPattern.getStopPaths().size(); ++stopIndex) {
      ...
      if (previousLocation == null && bestMatch.stopToShapeDistance > maxStopToPathDistance) {
        ...
        locList.add(stop.getLoc());
      } else if (previousLocation == null && trimPathBeforeFirstStopOfTrip) {
        ...
        Location beginLoc = previousLocation != null ? previousLocation : shapeLocs.get(0);
    ...
    if (numberOfStopsTooFarAway > 0) {
      logger.warn(
          "Found {} stop{} that {} further than the allowable "
  ...
  /**
   * Goes through the locations for the StopPath and filters them to make sure that they are not too
   * short. Don't want lots of tiny segments for curves cluttering up the database and mucking
   * travel times more complicated.
   *
   * @param locations the locations that are to be filtered
   * @param maxDistanceForEliminatingVertices If a vertex is off by less this distance from the line
   *     connecting the previous and next vertex then this one can be filtered out.
   */
  private void filterShortSegments(
      List<Location> locations, double maxDistanceForEliminatingVertices) {
    ...
    for (int beginIndex = 0; beginIndex < locations.size() - 2; ++beginIndex) {
      for (int endIndex = beginIndex + 2; endIndex < locations.size(); ++endIndex) {
        ...
        Vector possibleNewVector = new Vector(locations.get(beginIndex), locations.get(endIndex));
        ...
        for (int vertexIndex = beginIndex + 1; vertexIndex < endIndex; ++vertexIndex) {
          ...
          if (distanceOfVertexToNewVector >= maxDistanceForEliminatingVertices
            ...
            for (int eraseIndex = beginIndex + 1; eraseIndex <= indexToEraseUpTo; ++eraseIndex) {
              logger.debug("Removing vertex #{} {}", eraseIndex, locations.get(beginIndex + 1));
  ...
  /**
   * Determines the path segments for each trip pattern. If shapes.txt GTFS file has a shape for the
   * trip pattern then that data is used. Otherwise will simply connect the stops with straight line
   * stopPaths.
   *
   * <p>When shapes.txt data is used a lot of processing is done. The path segments are offset
   * orthogonally by the offsetDistance which is useful if the shapes.txt info is simply street
   * centerline data. By offsetting the path segments for the different directions won't completely
   * overlap on the map. Then the stops are matched to the shapes in order to create the Paths. Each
   * path is for a separate stop and the path ends at the stop. The path segments are also filtered
   * so that segments that are too short or too long are adjusted.
   */
  public void processPathSegments() {
    ...
    for (TripPattern tripPattern : tripPatterns) {
      ...
      if (gtfsShapesForTripPattern == null) {
        ...
        // centerline data.
        List<Location> offsetLocations = getOffsetLocations(gtfsShapesMap.get(shapeId));

        // Create stopPaths by finding best match to shapes
        determinePathSegmentsMatchingStopsToShapes(offsetLocations, tripPattern);
      }
    }

    // Let user know what is going on
    logger.info(
        "Finished processing and filtering path segment data. " + "Took {} msec.",
        timer.elapsedMsec());
  }
}
