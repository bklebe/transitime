 ...
 * A GTFS stops object.
 *
 * <p>Contains two extension columns beyond the GTFS spec, hidden and adherenc.stop. They are useful
 * for the user interface. They will usually be set using a supplemental stops file.
 *
 * @author SkiBu Smith
 */
public class GtfsStop extends CsvBase {
  ...
  public GtfsStop(CSVRecord record, boolean supplemental, String fileName) {
    ...
    deleteFirstStopFromRoutesStr = getOptionalValue(record, "deleteFirstStopFromRoutes");
  }

  /**
   * Creates a copy of the GtfsStop but updates the latitude and longitude. Useful for transforming
   * coordinates in China so that locations are properly displayed in maps.
   *
   * @param original
   * @param shapePtLat
   * @param shapePtLon
  ...
  public GtfsStop(GtfsStop original, double lat, double lon) {
    ...
    this.deleteFirstStopFromRoutesStr = original.deleteFirstStopFromRoutesStr;
  ...
  /**
   * When combining a regular stop with a supplemental stop need to create a whole new object since
   * this class is Immutable to make it safer to use.
   *
   * @param originalStop
  ...
  public GtfsStop(GtfsStop originalStop, GtfsStop supplementStop) {
    ...
    deleteFromRoutesStr =
        s.deleteFromRoutesStr == null ? o.deleteFromRoutesStr : s.deleteFromRoutesStr;
    deleteFirstStopFromRoutesStr =
        s.deleteFirstStopFromRoutesStr == null
            ? o.deleteFirstStopFromRoutesStr
  ...
  /**
   * Returns ":" separated string indicate route short names where this stop should not be included
   * in the trips. Returns null if not defined.
   *
   * @return
   */
  ...
  /**
   * Returns ":" separated string indicate route short names where this stop should not be included
   * in the trips if it is the first stop of a trip. Returns null if not defined.
   *
   * @return
   */
   ...
   *
   * @param routeShortNameToDeleteFrom The short name of the route currently working on. Used to
   *     determine if the stop really should be deleted.
   * @param routesStr Should be deleteFromRoutesStr or deleteFirstStopFromRoutesStr, depending on
   *     type of stop deletion.
   * @return True if stop deleted
   */
  public boolean shouldDeleteFromRoute(String routeShortNameToDeleteFrom, String routesStr) {
    if (routesStr == null) return false;
    String[] routeShortNames = routesStr.split(":");
    for (String routeShortName : routeShortNames) {
      if (routeShortName.equals(routeShortNameToDeleteFrom)) return true;
    }
