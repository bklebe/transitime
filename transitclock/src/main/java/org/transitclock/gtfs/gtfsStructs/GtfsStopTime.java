...
public class GtfsStopTime extends CsvBase implements Comparable<GtfsStopTime> {
  ...
  public GtfsStopTime(
    ...
    this.arrivalTimeSecs =
        (arrivalTimeStr != null && !arrivalTimeStr.isEmpty())
            ? Time.parseTimeOfDay(arrivalTimeStr)
            : null;
    this.departureTimeSecs =
        (departureTimeStr != null && !departureTimeStr.isEmpty())
            ? Time.parseTimeOfDay(departureTimeStr)
            : null;
    this.stopId = stopId;
  ...
  public GtfsStopTime(CSVRecord record, boolean supplemental, String fileName)
    ...
    if (arrivalTimeStr != null) arrivalTimeSecs = Time.parseTimeOfDay(arrivalTimeStr);
    else arrivalTimeSecs = null;

    // Convert departure_time to seconds in day
    String deparatureTimeStr = getOptionalValue(record, "departure_time");
    if (deparatureTimeStr != null) departureTimeSecs = Time.parseTimeOfDay(deparatureTimeStr);
    else departureTimeSecs = null;

    stopId = getRequiredValue(record, "stop_id");

    String stopSequenceStr = getRequiredUnlessSupplementalValue(record, "stop_sequence");
    ...
    shapeDistTraveled =
        shapeDistTraveledStr == null || shapeDistTraveledStr.trim().isEmpty()
            ? null
            : Double.parseDouble(shapeDistTraveledStr);
  ...
  /**
   * For when need to convert a GtfsStopTime to a subclass. Copies the originalValues but uses
   ...
   *     used.
   */
  protected GtfsStopTime(
      GtfsStopTime originalValues, Integer newArrivalTime, Integer newDepartureTime) {
    ...
    Integer newArrivalTimeSecs =
        newArrivalTime != null ? newArrivalTime : originalValues.arrivalTimeSecs;
    Integer newDepartureTimeSecs =
        newDepartureTime != null ? newDepartureTime : originalValues.departureTimeSecs;
    ...
    if (newArrivalTimeSecs > newDepartureTimeSecs) {
      newArrivalTimeSecs = newDepartureTimeSecs = (newArrivalTimeSecs + newDepartureTimeSecs) / 2;
    }
  ...
  /**
   * For when need to create a GtfsStopTime that is a wait stop. Uses the values from originalValues
   * but uses the new arrival time and sets isWaitStop to true.
   *
   * @param originalValues The original values from the stop_times.txt file
  ...
  /**
   * When combining a regular stop time with a supplemental one need to create a whole new object
  ...
  public GtfsStopTime(GtfsStopTime originalStopTime, GtfsStopTime supplementStopTime) {
    ...
    this.maxDistance = s.maxDistance == null ? o.maxDistance : s.maxDistance;

    this.maxSpeed = s.maxSpeed == null ? o.maxSpeed : s.maxSpeed;

    this.arrivalTimeSecs = s.arrivalTimeSecs == null ? o.arrivalTimeSecs : s.arrivalTimeSecs;
    this.departureTimeSecs =
        s.departureTimeSecs == null ? o.departureTimeSecs : s.departureTimeSecs;
    this.stopId = o.stopId;
    this.stopSequence = s.stopSequence == null ? o.stopSequence : s.stopSequence;
    this.timepointStop = s.timepointStop == null ? o.timepointStop : s.timepointStop;
    this.stopHeadsign = s.stopHeadsign == null ? o.stopHeadsign : s.stopHeadsign;
    this.pickupType = s.pickupType == null ? o.pickupType : s.pickupType;
    this.dropOffType = s.dropOffType == null ? o.dropOffType : s.dropOffType;
    this.shapeDistTraveled =
        s.shapeDistTraveled == null ? o.shapeDistTraveled : s.shapeDistTraveled;
  ...
  public String toString() {
    return "GtfsStopTime ["
        ...
        + (arrivalTimeSecs != null
            ? "arrivalTime=" + Time.timeOfDayStr(arrivalTimeSecs) + ", "
            : "")
        + (departureTimeSecs != null
            ? "departureTime=" + Time.timeOfDayStr(departureTimeSecs) + ", "
            : "")
