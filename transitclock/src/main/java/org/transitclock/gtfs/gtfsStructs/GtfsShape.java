...
public class GtfsShape extends CsvBase implements Comparable<GtfsShape> {
  ...
  /**
   * Creates a GtfsShape object by reading the data from the CSVRecord.
   *
   * @param record
   * @param supplemental
  ...
  public GtfsShape(CSVRecord record, boolean supplemental, String fileName)
    ...
    if (distStr != null) shapeDistTraveled = Double.parseDouble(distStr);
    else shapeDistTraveled = null;

    delete = getOptionalBooleanValue(record, "delete");
  }

  /**
   * Creates a copy of the GtfsShape but updates the latitude and longitude. Useful for transforming
  ...
  /**
   * When combining a regular shape point with a supplemental one need to create a whole new object
  ...
  public GtfsShape(GtfsShape originalShape, GtfsShape supplementShape) {
    ...
    this.shapePtLat = Double.isNaN(s.shapePtLat) ? o.shapePtLat : s.shapePtLat;
    this.shapePtLon = Double.isNaN(s.shapePtLon) ? o.shapePtLon : s.shapePtLon;
