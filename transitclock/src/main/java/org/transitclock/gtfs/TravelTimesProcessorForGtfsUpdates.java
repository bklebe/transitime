...
/**
 * For setting travel times when processing GTFS configuration. Tries to use existing travel times
 * from database. If data for trip exists it is used. If data for trip doesn't exist tries to use
 * data for similar trip (see adequateMatch()). For when there is no matching historic travel times
 * data then uses default values based on the schedule. The idea is that when GTFS data read in
 * sometimes there will be new routes or stopPaths. For these want some kind of travel times so that
 * the prediction software will work. But don't have any AVL data yet for these stopPaths since they
 * are new.
 *
 * @author SkiBu Smith
 */
public class TravelTimesProcessorForGtfsUpdates {
  ...
  /**
   * Returns the GTFS stop time for the specified trip and stop. For processing travel times need to
   * use GTFS stop times instead of the trip times since trip times can be filtered and only contain
   * times for schedule adherence stops.
   *
   * <p>NOTE: this method simply does a linear search for the appropriate stop through the list of
   * travel times associated with the trip. Therefore it could be rather slow! Might need to have
   * GtfsData provide a ordered map of GtfsStopTimes instead of just a List of them.
   *
   * @param tripId
   * @param tripPattern
   * @param stopPathIndex
  ...
  private ScheduleTime getGtfsScheduleTime(
    ...
    for (int stopTimeIdx = 0; stopTimeIdx < gtfsStopTimesList.size(); ++stopTimeIdx) {
      ...
      if (gtfsStopTime.getStopSequence() == gtfsStopSequence) {
        ...
        else return new ScheduleTime(arr, dep);
      }
    }
  ...
  /**
   * Creates a TravelTimesForTrip from the schedule times from the Trip passed in.
   *
   * @param tripPattern
   * @return
   */
  private TravelTimesForTrip determineTravelTimesBasedOnSchedule(Trip trip, GtfsData gtfsData) {
    ...
    TravelTimesForTrip travelTimes =
        new TravelTimesForTrip(
            activeRevisions.getConfigRev(), activeRevisions.getTravelTimesRev(), trip);
    ...
    ScheduleTime previousScheduleTime =
        getGtfsScheduleTime(trip.getId(), tripPattern, firstPath.getGtfsStopSeq(), gtfsData);
    ...
    for (int stopPathWithScheduleTimeIndex = 1;
      ...
      ScheduleTime scheduleTime = getGtfsScheduleTime(trip.getId(), tripPattern, stopSeq, gtfsData);
      ...
      if (stopPathWithScheduleTimeIndex == numberOfPaths - 1) --numberOfStopsWithStopTime;
      ...
      int msecForTravelBetweenScheduleTimes = elapsedScheduleTimeInSecs * 1000 - msecSpentStopped;
      ...
      if (msecForTravelBetweenScheduleTimes == 0
        ...
        if (elapsedScheduleTimeInSecs * Time.MS_PER_SEC > 0
          ...
          msecSpentStopped =
              elapsedScheduleTimeInSecs * Time.MS_PER_SEC
      ...
      for (int stopPathIndex = previousStopPathWithScheduleTimeIndex + 1;
        ...
        if (pathLength > maxTravelTimeSegmentLength) {
          ...
          numberTravelTimeSegments = (int) (pathLength / maxTravelTimeSegmentLength + 1.0);
          travelTimeSegmentsLength = pathLength / numberTravelTimeSegments;
        } else {
        ...
        int stopTimeMsec =
            (stopPathIndex < numberOfPaths - 1) ? msecSpentStopped / numberOfStopsWithStopTime : 0;
  ...
  /**
   * Sees if the travel times for the trip from the database are an adequate match for the trip
   * pattern such that existing travel times can be reused. They are an adequate match if the path
   * IDs are the same
   *
   * @param trip The trip current processing travel times for
   * @param ttForTripFromDb Travel times obtained for trip from the database
   * @return true if the ttForTripFromDb passed in can be used for the trip
   */
  private boolean stopPathsMatch(Trip trip, TravelTimesForTrip ttForTripFromDb) {
    // Convenience variable
    TripPattern tripPattern = trip.getTripPattern();

    // See if stopPaths match. If they don't then return false.
    // First check if trip patterns have same number of stops.
    if (tripPattern.getStopPaths().size() != ttForTripFromDb.numberOfStopPaths()) {
      logger.warn(
          "In ScheduleBasedTravelTimesProcessor.adequateMatch(), "
    ...
    for (int i = 0; i < tripPattern.getStopPaths().size(); ++i) {
      String pathIdFromTripPattern = tripPattern.getStopPathId(i);
      String pathIdFromTravelTimes =
          ttForTripFromDb.getTravelTimesForStopPaths().get(i).getStopPathId();
      if (!pathIdFromTripPattern.equals(pathIdFromTravelTimes)) {
        logger.error(
            "In ScheduleBasedTravelTimesProcessor."
            ...
            tripPattern.getId(),
            ttForTripFromDb.getTripCreatedForId(),
            pathIdFromTripPattern,
            pathIdFromTravelTimes);
        return false;
      }
    }
  ...
  /**
   * Given list of travel times for the trip pattern looks to see one is for the trip specified. If
   * so, the travel times from the database for that trip.
   *
   * @param trip which trip trying to get travel times for
   * @param ttForTripFromDbList list of travel times from the db for the trip pattern. Can be null.
   * @param scheduleBasedOk set to true if OK to use schedule based travel times
   * @return travel times for the trip from the database, or null if no such travel times available
   */
  private TravelTimesForTrip travelTimesForTripFromDb(
      Trip trip, List<TravelTimesForTrip> ttForTripFromDbList, boolean scheduleBasedOk) {
    // If no travel times from db then we don't have a match
    if (ttForTripFromDbList == null) return null;
    ...
    for (TravelTimesForTrip ttForTripFromDb : ttForTripFromDbList) {
      if (ttForTripFromDb.getTripCreatedForId().equals(trip.getId())) {
        if (stopPathsMatch(trip, ttForTripFromDb)
            && (scheduleBasedOk || !ttForTripFromDb.purelyScheduleBased())) {
          // Found good match of travel times from db
          logger.debug(
              "Found exact travel time match for " + "tripId={} from database.", trip.getId());

          if (ttForTripFromDb.isValid()) return ttForTripFromDb;

          logger.error(
              "Found invalid travel times for " + "tripId={} from database: {}",
              trip.getId(),
              ttForTripFromDb);
        }
      }
  ...
  /**
   * Given list of travel times for the trip pattern looks to see one is for the trip pattern
   * specified. If so, the travel times from the database for that trip pattern are used.
   *
   * @param trip which trip trying to get travel times for
   * @param ttForTripFromDbList list of travel times from the db for the trip pattern. Can be null.
   * @param scheduleBasedOk set to true if OK to use schedule based travel times
   * @return travel times for the trip from the database, or null if no such travel times available
   */
  private TravelTimesForTrip travelTimesForTripPatternFromDb(
      Trip trip, List<TravelTimesForTrip> ttForTripFromDbList, boolean scheduleBasedOk) {
    // If no travel times from db then we don't have a match
    if (ttForTripFromDbList == null) return null;
    ...
    for (TravelTimesForTrip ttForTripFromDb : ttForTripFromDbList) {
      ...
      if (stopPathsMatch(trip, ttForTripFromDb)
        ...
        // Found good match of travel times from db
        logger.debug(
            "Found adequate travel time match for "
        ...
        logger.error(
            "Found invalid travel time match for "
  ...
  /**
   * Creates new schedule based travel times for the trip. Also adds the schedule based travel times
   * to travelTimesFromDbMap so that it can possibly be used by other trips. For when historic
   * non-schedule based travel times are not available from the database.
   *
   * @param trip which trip trying to get travel times for
   * @param gtfsData for getting schedule info
   * @param ttForTripFromDbList list of travel times from the db for the trip pattern. Can be null.
   * @param travelTimesFromDbMap so can add newly created schedule based travel times so can also be
   *     used by other trips
   * @return schedule based travel times
   */
  private TravelTimesForTrip scheduleBasedTravelTimes(
      Trip trip,
      GtfsData gtfsData,
      List<TravelTimesForTrip> ttForTripFromDbList,
      Map<String, List<TravelTimesForTrip>> travelTimesFromDbMap) {
    logger.debug(
        "There was not an adequate travel time match for "
    ...
    if (ttForTripFromDbList == null) {
      ...
      travelTimesFromDbMap.put(trip.getTripPattern().getId(), ttForTripFromDbList);
    }
  ...
  /**
   * Determines if schedule from gtfsData is close enough to the one from ttForTripFromDb. If so
   * then true is returned. This method is for trying to reuse schedule based travel times.
   *
   * @param trip which trip trying to get travel times for
   * @param ttForTripFromDb schedule based travel times from db that checking to see if close
   *     enough. If null then returns false since no schedule to be close to.
   * @param gtfsData for getting schedule info
   * @return true if schedule from gtfsData is close enough to the one from ttForTripFromDb
   */
  private boolean scheduleCloseEnough(
      Trip trip, TravelTimesForTrip ttForTripFromDb, GtfsData gtfsData) {
    // If no travel times from database then return false
    if (ttForTripFromDb == null) return false;
    ...
    for (int i = 0; i < scheduleBasedTravelTimes.numberOfStopPaths(); ++i) {
      TravelTimesForStopPath newElement = scheduleBasedTravelTimes.getTravelTimesForStopPath(i);
      TravelTimesForStopPath oldElement = ttForTripFromDb.getTravelTimesForStopPath(i);
      ...
      if (Math.abs(dbTravelTimeMsec - newTravelTimeMsec) >= 60 * Time.MS_PER_SEC) {
        logger.debug(
            "While looking for already existing usable travel "
  ...
  /**
   * Goes through every trip and and associates schedule based travel times with trip if don't have
   * GPS data for it.
   *
   * @param gtfsData
   ...
   */
  private Integer processTrips(
      GtfsData gtfsData, Map<String, List<TravelTimesForTrip>> travelTimesFromDbMap) {
    ...
    for (Trip trip : gtfsData.getTrips()) {
      ...
      logger.debug(
          "Processing travel times for tripId={} which "
      ...
      List<TravelTimesForTrip> ttForTripFromDbList = travelTimesFromDbMap.get(tripPattern.getId());
      ...
      if (travelTimesToUse == null) {
        travelTimesToUse = travelTimesForTripPatternFromDb(trip, ttForTripFromDbList, false);
      }
      ...
      if (travelTimesToUse == null) {
        travelTimesToUse =
            scheduleBasedTravelTimes(trip, gtfsData, ttForTripFromDbList, travelTimesFromDbMap);
      }
  ...
  /**
   * For determining how many travel times have been processed into the travelTimesFromDbMap. There
   * is a travel time for each stop path for each trip.
   *
   * @param travelTimesFromDbMap Keyed on tripPatternId, contains list of TravelTimesForTrip for
   *     each tripPatternId
   * @return Total number of travel times (one per stop path) already processed)
   */
  ...
  /**
   * For trips where travel times not set in database via GPS data default travel times are created
   * by looking at the schedule times and interpolating.
   *
   * @param gtfsData
   */
  public void process(Session session, GtfsData gtfsData) {
    if (!gtfsData.isTripsReadIn()) {
      logger.error(
          "tripsMap not yet read in by GtfsData before "
    ...
    if (!gtfsData.isStopTimesReadIn()) {
      logger.error(
          "GTFS stop times not read in before "
    ...
    // Let user know what is going on
    logger.info(
        "Finished processing travel time data. "
