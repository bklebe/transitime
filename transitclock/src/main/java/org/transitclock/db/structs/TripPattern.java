...
public class TripPattern implements Serializable, Lifecycle {
  ...
  @JoinTable(
      name = "TripPattern_to_Path_joinTable",
      joinColumns = {
  ...
  @Transient private List<Trip> trips = new ArrayList<Trip>();
  ...
  @Transient protected final Map<String, StopPath> stopPathsMap = new HashMap<String, StopPath>();
   ...
   */
  public TripPattern(
      int configRev, String shapeId, List<StopPath> stopPaths, Trip trip, GtfsData gtfsData) {
   ...
   * in the GTFS data then will use the route_long_name. This way the route short name returned will
   * always be something appropriate.
   *
   * @param routeId
   * @param gtfsData
        ...
        session
            .createSQLQuery(
                "DELETE FROM TripPattern_to_Path_joinTable "
                    + "WHERE TripPatterns_configRev="
                    + configRev)
            .executeUpdate();
    rowsUpdated +=
        session
            .createSQLQuery("DELETE FROM StopPaths WHERE configRev=" + configRev)
            .executeUpdate();
    rowsUpdated +=
        session
            .createSQLQuery("DELETE FROM TripPatterns WHERE configRev=" + configRev)
            .executeUpdate();
    return rowsUpdated;
   ...
   * same stops have different stopPaths for connecting them. Therefore should use the shapeId from
   * the Trip passed in.
   *
  ...
  private static String generateTripPatternId(
    ...
    if (tripPatternId.length() + extraNeededForVariation > TRIP_PATTERN_ID_LENGTH)
      tripPatternId =
          tripPatternId.substring(
              tripPatternId.length() + extraNeededForVariation - TRIP_PATTERN_ID_LENGTH);
  ...
  public int hashCode() {
    ...
    //		result = prime * result + ((trips == null) ? 0 : trips.hashCode());  // stack overflow with
    // large db
  ...
  /**
   * Gets the name of the specified stop as obtained by a Core predictor. Cannot be used with other
   * applications.
   *
   * @param i
   * @return
  ...
  public void setHeadsign(String headsign) {
    this.headsign =
        headsign.length() <= HEADSIGN_LENGTH ? headsign : headsign.substring(0, HEADSIGN_LENGTH);
  }
