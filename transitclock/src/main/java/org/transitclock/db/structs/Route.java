...
public class Route implements Serializable {
  ...
  @Column private Integer routeOrder;

  @Column private final boolean hidden;
  ...
  // For getStops()
  @Transient private Collection<Stop> stops = null;

  // For getPathSegments()
  @Transient private Collection<Vector> stopPaths = null;

  // For getOrderedStopsByDirection()
  @Transient private Map<String, List<String>> orderedStopsPerDirectionMap = null;
  ...
  @Transient private Map<String, Map<String, List<Integer>>> stopOrderByDirectionMap = null;
  ...
  /**
   * Constructor. Used for when processing GTFS data. Creates a Route object that can be written to
   * database.
   *
   * @param configRev
   * @param gtfsRoute
  ...
  public Route(
    ...
    this.shortName =
        shortName != null && !shortName.isEmpty() ? shortName : gtfsRoute.getRouteLongName();
   ...
   *
   * <p>If routeOrder is set and is below 1,000 then the route will be at beginning of list and will
   * be ordered by routeOrder. If routeOrder is set and is above 1,000,000 then route will be put at
   * end of list and will be ordered by routeOrder. If routeOrder is not set then will order by
   * route short name. If route short name starts with numbers it will be padded by zeros so that
   * proper numerical order will be used.
   */
  ...
  public static List<Route> getRoutes(Session session, int configRev) throws HibernateException {
    ...
    String hql =
        "FROM Route " + "    WHERE configRev = :configRev" + "    ORDER BY routeOrder, shortName";
  ...
  public int hashCode() {
    ...
    result =
        prime * result + ((tripPatternsForRoute == null) ? 0 : tripPatternsForRoute.hashCode());
  ...
  /**
   * Returns longest trip pattern for the directionId specified. Note: gets trip patterns from Core,
   * which means it works in the core application, not just when processing GTFS data.
   *
   * @param directionId
   * @return
   */
  ...
  /**
   * Gets the order for the stop. Important when have multiple trip patterns for a direction. Allows
   * each stop in the trip to have a unique sequential stop order so that can save the stop order in
   * the db and then use it for db queries to order the stop data for a direction for a route.
   *
   * @param directionId
   * @param stopId
  ...
  public int getStopOrder(String directionId, String stopId, int stopIndex) {
    ...
    if (stopOrderMap == null) {
      logger.error(
          "In Route.getStopOrder() directionId={} is not valid " + "for routeId={}.",
          directionId,
          id);
      return -1;
    }
  ...
  /**
   * The processed name of the route consisting of the combination of the route_short_name plus the
   * route_long_name. So get something like "38 - Geary".
   *
  ...
  /**
   * The long name is either specified by route_long_name in the routes.txt GTFS file or if that is
   * null it will be the short name name.
   *
  ...
  /**
   * For specifying on a per route basis how far AVL report can be from segment and still have it be
   * considered a match.
   *
