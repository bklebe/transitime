...
public class TripPatternKey {
  ...
  public TripPatternKey(String shapeId, List<StopPath> paths) {
    if (paths == null)
      throw new RuntimeException(
          "stopPaths param must be " + "non-null for TripPatternBase() constructor");
   ...
   * construct a TripPattern but can use existing TripPatternBase so don't need to create stop list
   * and such.
   *
   ...
   * elements are not finalized when building up trip patterns. The tripPatternId still hasn't been
   * set. So don't want to compare the entire StopPath objects. Instead, only use
   * StopPath.basicHashCode(). This difference is very important so using an automatically generated
