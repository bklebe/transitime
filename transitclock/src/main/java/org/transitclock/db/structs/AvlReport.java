...
public class AvlReport implements Serializable {
  ...
  @Column private final Float speed; // optional
  ...
  // drivers. Externally though, such as when calling the constructor,
  // should use Float.NaN. It is converted to a null internally.
  @Column private final Float heading; // optional
  ...
  @Transient private final String leadVehicleId;
  ...
  @Column private final Integer passengerCount;
  ...
  /**
   * Hibernate requires a no-args constructor for reading data. So this is an experiment to see what
   * can be done to satisfy Hibernate but still have an object be immutable. Since this constructor
   * is only intended to be used by Hibernate is is declared protected, since that still works. That
   * way others won't accidentally use this inappropriate constructor. And yes, it is peculiar that
   * even though the members in this class are declared final that Hibernate can still create an
   * object using this no-args constructor and then set the fields. Not quite as "final" as one
   * might think. But at least it works.
   */
  protected AvlReport() {
    vehicleId = null;
    time = null;
   ...
   */
  public AvlReport(
      String vehicleId, long time, Location location, float speed, float heading, String source) {
    // Store the values
    this.vehicleId = vehicleId;
    this.time = new Date(time);
   ...
   * @param passengerCount Optional value. Set to the number of passengers on vehicles. Set to null
   *     if not available.
  ...
  /**
   * Makes a copy of the AvlReport but uses the new assignment info passed in. Useful for creating a
   * new AvlReport when a bad assignment is received since one can simply create a new AvlReport
  ...
  /**
   * Returns whether assignment information was set in the AVL data and that assignment is valid. An
   * assignment is not valid if it is configured to be invalid. Examples of such include training
   * vehicles, support vehicles, and simply vehicles set to a special assignment such as 9999 for
   * sfmta.
   *
  ...
  public void setAssignment(String assignmentId, AssignmentType assignmentType) {
    ...
    if (assignmentId == null && assignmentType != AssignmentType.UNSET) {
      logger.error(
          "Tried to use setAssignment() to set assignment to "
  ...
  /**
   * Updates the object to record the current time as the time that the data was actually processed.
   */
  ...
  public static List<AvlReport> getAvlReportsFromDb(
      Date beginTime, Date endTime, String vehicleId, String clause) {
