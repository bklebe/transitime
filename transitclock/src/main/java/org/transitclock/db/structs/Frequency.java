...
public class Frequency implements Serializable {
  ...
  @Column @Id private final int startTime;

  @Column private final int endTime;

  @Column private final int headwaySecs;

  @Column private final boolean exactTimes;
  ...
  public Frequency(int configRev, GtfsFrequency gtfsFrequency) {
    ...
    this.exactTimes =
        (gtfsFrequency.getExactTimes() != null ? gtfsFrequency.getExactTimes() : false);
  }
  ...
  /**
   * The exact_times field determines if frequency-based trips should be exactly scheduled based on
   * the specified headway information. Valid values for this field are:
   *
   * <ul>
   ...
   *       trips are scheduled starting with trip_start_time = start_time + x * headway_secs for all
   *       x in (0, 1, 2, ...) where trip_start_time < end_time.
   ...
   *       end_time, no trip must be scheduled. When exact_times is 1, care must be taken to choose
   *       an end_time value that is greater than the last desired trip start time but less than the
