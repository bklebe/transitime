...
public class ActiveRevisions {
  ...
  public static ActiveRevisions get(Session session) throws HibernateException {
    ...
    try {
      ...
      System.err.println(
          "Exception when reading ActiveRevisions object " + "from database so will create it");
    } finally {
  ...
  /**
   * Updates travelTimeRev member and calls saveOrUpdate(this) on the session. Useful for when want
   * to update the value but don't want to commit it until all other data is also written out
   * successfully.
   *
   * @param session
   * @param travelTimeRev
