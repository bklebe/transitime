...
public class StopPath implements Serializable, Lifecycle {
  ...
  @Column private final int gtfsStopSeq;
  ...
  // a trip.
  @Column private final boolean lastStopInTrip;
  ...
  @Column private final boolean scheduleAdherenceStop;
  ...
  @Column private final Integer breakTime;

  // sacrifice performance for reportability -- use a child table instead of java serialization
  @ElementCollection @OrderColumn private List<Location> locations;
  ...
  // such.
  @Column private double pathLength;

  @Column private Double maxDistance;
  ...
  @Transient private List<VectorWithHeading> vectors = null;

  /** This is used just to get better location of the bustop where bestmatch is done. */
  @Transient private Double shapeDistanceTraveled;
  ...
  public double length() {
    ...
    if (locations == null) {
      logger.error(
          "For stopPathId={} trying to access locations when " + "they have not been set.",
          stopPathId);
      return Double.NaN;
    }
   ...
   * isn't yet fully complete so only compare the key members that signify if a StopPath is unique.
   */
  ...
  public boolean equals(Object obj) {
    ...
    if (Double.doubleToLongBits(pathLength) != Double.doubleToLongBits(other.pathLength))
      return false;
    if (routeId == null) {
  ...
  /**
   * Locations are not available when StopPath is first created so need to be able to set them after
   * construction. Sets the locations member and also determines the pathLength.
   *
   * @param locations
   */
  ...
  public void onLoad(Session arg0, Serializable arg1) {
    ...
    for (int segmentIndex = 0; segmentIndex < locations.size() - 1; ++segmentIndex) {
      VectorWithHeading v =
          new VectorWithHeading(
              nullSafeLocation(locations.get(segmentIndex)),
