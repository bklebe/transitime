...
public class Trip implements Lifecycle, Serializable {
  ...
  @Column @Id private Integer startTime;
  ...
  @Column private Integer endTime;
  ...
  @Column private final boolean noSchedule;
  ...
  @Column private final boolean exactTimesHeadway;
   ...
   *
   * <p>Does not set startTime nor endTime. Those are set separately using addScheduleTimes(). Also
   * doesn't set travel times. Those are set separately using setTravelTimes().
   *
   * @param configRev
   ...
   */
  private String processedHeadsign(
      String gtfsHeadsign, String routeId, TitleFormatter titleFormatter) {
  ...
  public void addScheduleTimes(List<ScheduleTime> newScheduledTimesList) {
    ...
    for (ScheduleTime scheduleTime : newScheduledTimesList) {
      ...
      if (endTime == null
          || (scheduleTime.getArrivalTime() != null && scheduleTime.getArrivalTime() > endTime))
   ...
   * multiple trips for a tripShortName since can have multiple service IDs configured. Therefore a
   ...
   * and arrival time of last stop of trip to be set, even for unscheduled blocks. That way they can
   * determine running time of trip.
   *
  ...
  /**
   * Returns the routeShortName. If it is null then returns the full route name. Causes exception if
   * Core not available, such as when processing GTFS data.
   *
   ...
   * configuration not available such as when processing GTFS data then will return null. Will cause
   * exception if core not available and gtfs data not loaded in db yet since the active revisions
   * will not be set properly yet.
   *
  ...
  public void setHeadsign(String headsign) {
    this.headsign =
        headsign.length() <= TripPattern.HEADSIGN_LENGTH
            ? headsign
            : headsign.substring(0, TripPattern.HEADSIGN_LENGTH);
  }

  /**
   * The block_id is an optional element in GTFS trips.txt file. If it is available it is used. But
   * if it is not then the trip_id is used as next best thing.
   *
   * @return blockId
   */
   ...
   * the Block. If not part of the core project the Block info is not available and -1 is returned.
   *
  ...
  public ScheduleTime getScheduleTime(int stopPathIndex) {
    if (scheduledTimesList instanceof PersistentList) {
      ...
      SharedSessionContractImplementor session = persistentListTimes.getSession();
      if (session == null) {
