...
public class Extent implements Serializable {
  ...
  @Column private double minLat = Double.POSITIVE_INFINITY;

  @Column private double maxLat = Double.NEGATIVE_INFINITY;

  @Column private double minLon = Double.POSITIVE_INFINITY;

  @Column private double maxLon = Double.NEGATIVE_INFINITY;
  ...
  /**
   * Once an Extent has been constructed need to simply add associated Extents (or Locations). Once
   * all have been added the Extent will be the rectangle spanning all of those Locations and
   * Extents.
   *
   * @param l
   */
  ...
  /**
   * If don't have hashCode() and equals() then the objects that include this object will generate a
   * warning when these methods are implemented.
   */
  @Override
  public int hashCode() {
    final int prime = 31;
  ...
  public boolean isWithinDistance(Location loc, double distance) {
    ...
    if (minLat > loc.getLat() + distanceInDegreesLatitude
        || maxLat < loc.getLat() - distanceInDegreesLatitude) return false;
    ...
    if (minLon > loc.getLon() + distanceInDegreesLongitude
        || maxLon < loc.getLon() - distanceInDegreesLongitude) return false;
