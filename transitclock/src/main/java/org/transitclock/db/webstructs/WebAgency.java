...
public class WebAgency {
  ...
  @Transient private Agency agency;
  ...
  /**
   * If the cache of WebAgency objects not read in for a while then reads the WebAgency objects from
   * the db.
   *
  ...
  /**
   * Gets list of web agencies, ordered by their agency names. Uses cached version if possible. But
   * if data not read from db since more than 5 minutes ago then will reread database. This way when
   * this method is used to indicate which agencies are available the list will automatically show
  ...
  /**
   * Returns map of all agencies. Values are cached so won't be automatically updated when agencies
   * are changed in the database. Will update cache if haven't done so in rereadIfOlderThanMsecs.
   * This way will automatically get new agencies that are added yet will not be reading from the
   * database for every page hit that needs list of agencies. Also, agencies automatically removed.
   *
   * @param rereadIfOlderThanMsecs If web agencies read from db more than this time ago then they
   *     are read in again.
  ...
  /**
   * Gets specified WebAgency from the cache. Doesn't cause the cache of WebAgencies to be reloaded
   * from the db.
   *
   * @param agencyId
   * @return The specified WebAgency, or null if it doesn't exist.
  ...
  public static void main(String args[]) {
    ...
    if (args.length <= 5) {
      ...
      System.err.println(
          "Specify params for the WebAgency: agencyId = args[0]; hostName = args[1]; dbName = args[2] ;dbType = args[3]; dbHost = args[4]; dbUserName = args[5]; dbPassword = args[6];");
