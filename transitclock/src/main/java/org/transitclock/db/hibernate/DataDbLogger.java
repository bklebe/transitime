...
public class DataDbLogger {
   ...
   * @param shouldStoreToDb Specifies whether data should actually be written to db. If in playback
   *     mode and shouldn't write data to db then set to false.
   * @param shouldPauseToReduceQueue Specifies if should pause the thread calling add() if the queue
   *     is filling up. Useful for when in batch mode and dumping a whole bunch of data to the db
   *     really quickly.
  ...
  private DataDbLogger(String agencyId, boolean shouldStoreToDb, boolean shouldPauseToReduceQueue) {
    ...
    arrivalDepartureQueue =
        new DbQueue<ArrivalDeparture>(
            agencyId,
            shouldStoreToDb,
            shouldPauseToReduceQueue,
            ArrivalDeparture.class.getSimpleName());
    avlReportQueue =
        new DbQueue<AvlReport>(
            agencyId, shouldStoreToDb, shouldPauseToReduceQueue, AvlReport.class.getSimpleName());
    vehicleConfigQueue =
        new DbQueue<VehicleConfig>(
            agencyId,
            shouldStoreToDb,
            shouldPauseToReduceQueue,
            VehicleConfig.class.getSimpleName());
    predictionQueue =
        new DbQueue<Prediction>(
            agencyId, shouldStoreToDb, shouldPauseToReduceQueue, Prediction.class.getSimpleName());
    matchQueue =
        new DbQueue<Match>(
            agencyId, shouldStoreToDb, shouldPauseToReduceQueue, Match.class.getSimpleName());
    predictionAccuracyQueue =
        new DbQueue<PredictionAccuracy>(
            agencyId,
            shouldStoreToDb,
            shouldPauseToReduceQueue,
            PredictionAccuracy.class.getSimpleName());
    monitoringEventQueue =
        new DbQueue<MonitoringEvent>(
            agencyId,
            shouldStoreToDb,
            shouldPauseToReduceQueue,
            MonitoringEvent.class.getSimpleName());
    vehicleEventQueue =
        new DbQueue<VehicleEvent>(
            agencyId,
            shouldStoreToDb,
            shouldPauseToReduceQueue,
            VehicleEvent.class.getSimpleName());
    vehicleStateQueue =
        new DbQueue<VehicleState>(
            agencyId,
            shouldStoreToDb,
            shouldPauseToReduceQueue,
            VehicleState.class.getSimpleName());
    genericQueue =
        new DbQueue<Object>(
            agencyId, shouldStoreToDb, shouldPauseToReduceQueue, Object.class.getSimpleName());
  }
  ...
  /**
   * Determines set of class names in the queue. Useful for logging error message when queue getting
   * filled up so know what kind of objects are backing the system up.
   *
  ...
  /**
   * Adds an object to be saved in the database to the queue. If queue is getting filled up then an
   * e-mail will be sent out indicating there is a problem. The queue levels at which an e-mail is
   * sent out is specified by levels. If queue has reached capacity then an error message is logged.
   *
