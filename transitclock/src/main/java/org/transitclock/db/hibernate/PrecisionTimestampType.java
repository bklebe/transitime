...
/**
 * The purpose of this class was to have a custom hibernate type for a Timestamp column so that
 * could write fractional seconds. But it turns out that the problems having to do with fractional
 * seconds were based only on the version of the Connector/J JDBC driver that was being used. As
 * long as using at least MySQL 5.6.4 and Connector/J 5.1.23 then fractional seconds already work.
 * The only reason for using this class is if want to have Hibernate log fractional seconds when
 * logging the SQL parameters.
 *
 * <p>To enable use @Type (type="org.transitclock.db.hibernate.PrecisionTimestampType") instead
 * of @Temporal(TemporalType.TIMESTAMP) when declaring a Java object that is to be persisted.
 *
 * @author SkiBu Smith
 */
