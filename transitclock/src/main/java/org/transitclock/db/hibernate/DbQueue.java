...
public class DbQueue<T> {
  ...
  public boolean add(T t) {
    ...
    if (!success) {
      logger.error(
          "DataDbLogger queue is now completely full for "
              + "projectId="
              + projectId
              + "and type "
              + shortType
   ...
   * doesn't commit the data to the db until the transaction commit is done. Therefore the need here
   * isn't true Hibernate batch processing. Instead, need to use a transaction for each batch.
   */
  public void processBatchOfData() {
    ...
    try {
      ...
      logger.debug(
          "Committing {} objects. {} objects still in queue.",
          objectsForThisBatch.size(),
          queueSize());
      ...
      if (rootCause instanceof SocketTimeoutException
        ...
        try {
          ...
          logger.error(
              "Error rolling back transaction after processing "
                  + "batch of data via DataDbLogger.",
              e2);
        }
        ...
        try {
          ...
          logger.error(
              "Error closing session after processing " + "batch of data via DataDbLogger.", e2);
        }
  ...
  /**
   * This is the main method for processing data. It simply keeps on calling processBatchOfData() so
   * that data is batched as efficiently as possible. Exceptions are caught such that this method
  ...
  public void processData() {
    while (true) {
      try {
        logger.debug(
            "DataDbLogger.processData() processing batch of " + "data to be stored in database.");
        processBatchOfData();
      } catch (Exception e) {
        logger.error(
            "Error writing data to database via DataDbLogger. "
  ...
  private boolean shouldKeepTryingBecauseConnectionException(HibernateException e) {
    ...
    boolean keepTryingTillSuccessfull =
        e instanceof JDBCConnectionException || e instanceof GenericJDBCException;
