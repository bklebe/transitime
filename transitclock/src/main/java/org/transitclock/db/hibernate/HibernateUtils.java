...
public class HibernateUtils {
  ...
  private static SessionFactory createSessionFactory(String dbName, boolean readOnly)
    ...
    if (!f.exists()) {
      logger.info(
          "The Hibernate file {} doesn't exist as a regular file "
              + "so seeing if it is in classpath.",
          fileName);
    ...
    if (dbUrl == null || dbUrl.isEmpty()) {
      dbUrl =
          "jdbc:" + DbSetupConfig.getDbType() + "://" + DbSetupConfig.getDbHost() + "/" + dbName;
    ...
    logger.info(
        "For Hibernate factory project dbName={} "
   ...
   * situation want to be able to read in timezone from db so can set default timezone. Problem with
   * postgres is that once a factory is used to generate sessions the database will continue to use
   * the default timezone that was configured at that time. This means that future calls to the db
   * will use the wrong timezone! Through this function one can read in timezone from database, set
   * the default timezone, clear the factory so that future db connections will use the newly
   ...
   *
   * <p>NOTE: Make sure you close the session after the query!! Use a try/catch around the query and
   * close the session in a finally block to make sure it happens. The system only gets a limited
   * number of sessions!!
   *
  ...
  /**
   * Determines the size of a serializable object by serializing it in memory and then measuring the
