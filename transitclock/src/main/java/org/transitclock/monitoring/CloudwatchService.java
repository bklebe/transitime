...
public class CloudwatchService {
  ...
  private ArrayBlockingQueue<MetricScalar> individualMetricsQueue =
      new ArrayBlockingQueue<MetricScalar>(100000);
  ...
  private CloudwatchService() {
    ...
    if (StringUtils.isBlank(environmentName)
      ...
      logger.warn(
          "Cloudwatch monitoring not enabled, please specify environmentName, accessKey, secretKey and endpoint in configuration file");
    } else {
      logger.info(
          "starting Cloudwatch in env {} with accessKey {} and pass {}",
          environmentName,
          accessKey,
          secretKey);
      AmazonCloudWatchClient cloudWatch =
          new AmazonCloudWatchClient(new BasicAWSCredentials(accessKey, secretKey));
   ...
   * @return
   */
  public static synchronized CloudwatchService getInstance() {
    if (singleton == null) singleton = new CloudwatchService();
    return singleton;
  }
  ...
  public void saveMetric(
    ...
    if (metricType == MetricType.SCALAR
        && reportingIntervalTimeUnit == ReportingIntervalTimeUnit.IMMEDIATE) {
    ...
    if (!this.metricMap.containsKey(metricName)) {
      if (metricType == MetricType.SCALAR
        ...
        throw new IllegalArgumentException(
            "SCALAR values must be reported with IMMEDIATE frequency");
      if (reportingIntervalTimeUnit != ReportingIntervalTimeUnit.IMMEDIATE
          && metricType == MetricType.SCALAR)
        throw new IllegalArgumentException(
            "IMMEDIATE frequency can only be specified for SCALAR values");
    ...
    logger.info(
        "saving metric for summarized publication to Cloudwatch [{}]={}", metricName, metricValue);
    this.metricMap.get(metricName).data.add(metricValue);
  }

  private void publishMetric(String metricName, Double metricValue) {
    if (metricName == null || metricValue == null) return;
    ...
    PutMetricDataRequest putMetricDataRequest =
        new PutMetricDataRequest().withNamespace(environmentName).withMetricData(datum);
    try {
  ...
  private synchronized void publishMetricAsPercent(String metricName, Double metricValue) {
    if (cloudWatch == null || !enabled || metricName == null || metricValue == null) return;
  ...
  private class PublishIndividualMetricsTask implements Runnable {
    ...
    public void run() {
      ...
      for (MetricScalar metricScalar : records) {
        logger.info(
            "Publishing individual metric [{}]={}",
            metricScalar.metricName,
            metricScalar.metricValue);
        ...
        for (String metricName : metricMap.keySet()) {
          ...
          try {
            if (metricDefinition == null
                || metricDefinition.reportingIntervalTimeUnit
                    == ReportingIntervalTimeUnit.IMMEDIATE) continue;
            if (metricDefinition.lastUpdate == null) continue;
            ...
            if (dateDiff >= metricDefinition.reportingIntervalInMillis) {
              if (metricDefinition.metricType == MetricType.AVERAGE) {
                ...
                if (metricDefinition.data.size() < 1) return;
  ...
  public static void main(String[] args) {
    ...
    while (i < 100) {
      cloudwatchService.saveMetric(
          "testing", Math.random(), 1, MetricType.AVERAGE, ReportingIntervalTimeUnit.MINUTE, false);
      i++;
    }
  }
}
