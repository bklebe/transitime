...
/**
 * Base class for doing monitoring. If event is first triggered then an ERROR email is sent out. If
 * not triggered anymore then an OK email is sent.
 *
 * @author SkiBu Smith
 */
public abstract class MonitorBase {
  ...
  /**
   * Checks the monitor object to see if state has changed currently triggered. If state changes
   * then sends out notification e-mail.
   *
  ...
  public boolean checkAndNotify() {
    ...
    logger.info(
        "For agencyId={} monitoring type={} isTriggered={} "
    ...
    if (!wasTriggered && isTriggered && !acceptableEvenIfTriggered) {
      ...
      if (emailRecipients.getValue() != null) {
        ...
        logger.error(
            "Could not send ERROR e-mail because "
  ...
  /**
   * When triggered() is called the subclass should set the message. This needs to be done whether
   * or not monitor is triggered since monitoring also sends an OK message when was triggered but
   * isn't anymore.
   *
   * @param message
   */
  ...
  /**
   * For when acceptableEvenIfTriggered() is called. Sets the acceptableEvenIfTriggeredMessage so
   * that it can be included in logging.
   *
   * @param acceptableEvenIfTriggeredMessage
   */
  protected void setAcceptableEvenIfTriggeredMessage(String acceptableEvenIfTriggeredMessage) {
  ...
  /**
   * For when there is no value associated with the monitoring, such as for monitoring db when it is
   * either up or down. The value is set to Double.NaN.
   *
   * @param message
   */
  ...
  /**
   * Returns any message from the last time monitoring was checked. If secondary message is set (not
   * null) then it is added to the message.
   *
   * @return Message describing situation. Should be set whether or not monitor is currently
   *     triggered so can also provide an OK message.
   */
  public String getMessage() {
    if (acceptableEvenIfTriggeredMessage == null) return message;
    else return message + " " + acceptableEvenIfTriggeredMessage;
  }

  /**
   * Abstract method. Returns whether or not a monitor is triggered, meaning it has a problem. The
   * subclass method should also set the message using setMessage().
   *
  ...
  /**
   * Also have a secondary monitor check. The idea is that there can be a primary check (e.g. no AVL
   * data) but then also a secondary check (e.g. active blocks so not expecting AVL data). The
   * monitor subclass can override this method and return true when the secondary monitor indicates
   * that system is is actually ok even if triggered. The subclass should also call
   * setAcceptableEvenIfTriggeredMessage() when the monitor is OK so that the message can be
   * included when logging events.
   *
   * <p>This method is only to be called if the monitor is triggered. It is intended for determining
   * if the monitor being triggered is actually acceptable.
   *
   * @return True if secondary monitor indicates that system is ok and that e-mail message
   *     indicating problem should not be sent out.
   */
  ...
  /**
   * A short description of the monitoring type that is displayed as part of the subject line for
