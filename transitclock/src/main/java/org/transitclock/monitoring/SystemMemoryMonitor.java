...
public class SystemMemoryMonitor extends MonitorBase {
  ...
  LongConfigValue availableFreePhysicalMemoryThreshold =
      new LongConfigValue(
  ...
  /**
   * Gets an operating system value via reflection. Yes, this is a rather obtuse way of getting such
   * values but it appears to work.
   *
   * @param methodName Name of the special internal com.sun.management.OperatingSystemMXBean method
   *     to call
  ...
  public static Object getOperatingSystemValue(String methodName) {
    ...
    try {
      ...
      logger.error(
          "Could not execute " + "OperatingSystemMXBean.{}(). {}", methodName, e.getMessage());
      return null;
    }
  }
