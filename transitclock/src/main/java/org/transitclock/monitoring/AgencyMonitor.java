...
public class AgencyMonitor {
  ...
  private static final String enableSystemMonitoring =
      System.getProperty("transitclock.enableSystemMonitoring");
   ...
   * hasn't been created yet it is created. This is important because each monitor maintains state,
   * such as if notification e-mail sent out.
   *
  ...
  /**
   * Checks all the monitors for the agency and returns all resulting messages whether a monitor is
   * triggered or not. Useful for showing current status of system.
   *
  ...
  /**
   * Checks the core system to make sure it is working properly. If it is then null is returned. If
   * there are any problems then returns the concatenation of all the error messages. Sends out
   * notification e-mails if there is an issue via MonitorBase class. To be called periodically via
   * a MonitoringModule or via Inter Process Communication.
   *
   * @return Null if system OK, or the concatenation of the error message for all the monitoring if
