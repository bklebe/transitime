...
public class ConfigFileReader {
   ...
   * synchronization objects. Using a ReentrantReadWriteLock so that multiple threads with reads can
   * happen simultaneously. This works well because writing/updating data will be very infrequent.
   */
  ...
  /**
   * Reads the specified XML file and stores the results into a HashMap configFileData. The file is
   * expected to be very simple. Attributes are ignored. Only values are used. The keys for the
   * resulting HashMap are based on the XML tags and their nesting. So if one is setting a value of
   * 75 to a param with a key transitclock.predictor.allowableDistanceFromPath the XML would look
   * like: {@code <transitime> <predictor> <allowableDistanceFromPath> 75.0
  ...
  /**
   * Reads a Java properties file and stores all the properties as system properties if they haven't
   * yet been set.
   *
  ...
  private static void readPropertiesConfigFile(String fileName) {
    try (FileInputStream file = new FileInputStream(fileName)) {
      ...
      System.err.println(
          "Exception occurred reading in fileName " + fileName + " . " + e.getMessage());
    }
  }
   ...
   *
   * <p>Reads in data and stores it in the config objects so that it is programmatically accessible.
   * Does a write lock so that readers will only execute when data is not being modified.
   *
  ...
  /**
   * Process the configuration file specified by a file named transitclock.properties that is in the
   * classpath. The processes configuration files specified by the Java system property
   * transitclock.configFiles. The transitclock.configFiles property is a ";" separated list so can
   * specify multiple files. The files can be either in either XML or in Java properties format.
   *
   * <p>All parameters found will be set as system properties unless the system property was already
   * set. This way Java system properties set on the command line have precedence.
   *
   * <p>By making all read parameters Java system properties they can be used for external libraries
   * such as for specifying hibernate or logback config files.
   */
