...
public class CommandsApi {
   ...
   * optionally specify assignmentType="1234" and assignmentType="BLOCK_ID" or ROUTE_ID, TRIP_ID, or
   * TRIP_SHORT_NAME.
   *
   * @param stdParameters
   * @param vehicleId
  ...
  public Response pushAvlData(
      ...
      @Parameter(description = "VehicleId. Unique identifier of the vehicle.", required = true)
          @QueryParam(value = "v")
          String vehicleId,
      @Parameter(description = "GPS epoch time in msec.", required = true) @QueryParam(value = "t")
          long time,
      ...
      @Parameter(
              description =
                  "Heading of AVL report. Degrees. 0 degrees=North. Should be set to Float.NaN if speed not available",
              required = false)
          @QueryParam(value = "h")
          @DefaultValue("NaN")
          float heading,
      @Parameter(
              description =
                  "Indicates the assignmet id of the AVL report according to the assingment tyoe. For example, if assingment type is ROUTE_ID, the assingment ID should be one route_id loaded in the system.",
              required = false)
          @QueryParam(value = "assignmentId")
          String assignmentId,
      @Parameter(
              description =
                  "Indicates the assignmet type of the AV report. This parameter can take the next values: <ul><li>ROUTE_ID</li><li>TRIP_ID</li>TRIP_SHORT_NAME</li> </ul>")
   ...
   * format is:
   *
   * <p>{avl: [{v: "vehicleId1", t: epochTimeMsec, lat: latitude, lon: longitude, s:speed(optional),
   * h:heading(optional)}, {v: "vehicleId2", t: epochTimeMsec, lat: latitude, lon: longitude, s:
   * speed(optional), h: heading(optional)}, {etc...} ] }
   *
  ...
  @Operation(
      ...
      description =
          "Reads in a single AVL report specified by the query string parameters. <p>{avl: [{v: \"vehicleId1\", t: epochTimeMsec, lat: latitude, lon: longitude, s:speed(optional), h:heading(optional)},\r\n"
  ...
  @Operation(
      ...
      description =
          "This is to give the means of manually setting a vehicle unpredictable and unassigned so it will be reassigned quickly.",
      tags = {"command", "vehicle"})
  public Response getVehicles(
      @BeanParam StandardParameters stdParameters,
      @Parameter(description = "List of vechilesId.") @QueryParam(value = "v")
  ...
  public Response pushAvlData(
      ...
      @Parameter(description = "Route short name.", required = true)
          @QueryParam(value = "rShortName")
          String routeShortName,
      @Parameter(description = "Route stop id.", required = true) @QueryParam(value = "s")
          String stopId,
      @Parameter(description = "Direcction id.", required = true) @QueryParam(value = "d")
          String directionId,
      @Parameter(description = "headsign.", required = true) @QueryParam(value = "headsign")
          String headsign)
      throws WebApplicationException {
    // Make sure request is valid
    stdParameters.validate();
    ...
    try {
      ...
      MeasuredArrivalTime time =
          new MeasuredArrivalTime(
              new Date(), stopId, routeId, routeShortName, directionId, headsign);
  ...
  @Operation(
      ...
      description =
          "<font color=\"#FF0000\">Experimental. It will work olny with the correct version.</font> It cancel a trip that has no vechilce assigned.",
      tags = {"command", "trip"})
  public Response cancelTrip(
      ...
      @Parameter(description = "tripId to be marked as canceled.", required = true)
          @PathParam("tripId")
          String tripId,
      @Parameter(description = "start trip time", required = false) @QueryParam(value = "at")
          DateTimeParam at) {
    stdParameters.validate();
    String result = null;
    try {
      ...
      e.printStackTrace();
      throw WebUtils.badRequestException(
          "Could not send request to Core server. " + e.getMessage());
    }
  ...
  public Response reenableTrip(
      @BeanParam StandardParameters stdParameters,
      @Parameter(description = "tripId to remove calceled satate.", required = true)
          @PathParam("tripId")
          String tripId,
      @Parameter(description = "start trip time", required = false) @QueryParam(value = "at")
          DateTimeParam at) {
