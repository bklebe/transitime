...
@OpenAPIDefinition(
    info =
        @Info(
            ...
            version = "1.0",
            description =
                "TheTransitClock is an open source transit information system."
...
public class TransitimeApi {
  ...
  /**
   * Handles the "vehicles" command. Returns data for all vehicles or for the vehicles specified via
   * the query string.
   *
   * <p>A Response object is returned instead of a regular object so that can have one method for
   * the both XML and JSON yet always return the proper media type even if it is configured via the
   * query string "format" parameter as opposed to the accept header.
   *
   ...
   * @param stopId Optional way of specifying a stop so can get predictions for routes and determine
   *     which vehicles are the ones generating the predictions. The other vehicles are labeled as
   *     minor so they can be drawn specially in the UI.
   * @param numberPredictions For when determining which vehicles are generating the predictions so
   *     can label minor vehicles
  ...
  @Operation(
      ...
      description =
          "Returns data for all vehicles or for the vehicles specified via the query string.",
  ...
  public Response getVehicles(
      ...
      @Parameter(description = "Vehicles is list.") @QueryParam(value = "v")
      ...
      @Parameter(
              description =
                  "Specifies a stop so can get predictions for"
  ...
  public Response getVehicleLocation(
    ...
    try {
      ...
      Location matchedLocation =
          new Location(vehicle.getPredictedLatitude(), vehicle.getPredictedLongitude());
   ...
   * vehicles specified via the query string. This data includes things not necessarily intended for
  ...
  public Response getVehiclesDetails(
    ...
    try {
      ...
      ApiVehiclesDetails apiVehiclesDetails =
          new ApiVehiclesDetails(
              vehicles, stdParameters.getAgencyId(), uiTypesForVehicles, onlyAssigned);
  ...
  @Operation(
      ...
      description =
          "Returns a list of vehicles coniguration which inclides description, capacity, type and crushCapacity.",
      tags = {"vehicle"})
  @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
  public Response getVehicleConfigs(@BeanParam StandardParameters stdParameters)
      throws WebApplicationException {
    // Make sure request is valid
    stdParameters.validate();
  ...
  private static Map<String, UiMode> determineUiModesForVehicles(
    ...
    if (routesIdOrShortNames.isEmpty() || stopId == null) {
      ...
      List<String> vehiclesGeneratingPreds =
          determineVehiclesGeneratingPreds(
              stdParameters, routesIdOrShortNames, stopId, numberPredictions);
      for (IpcVehicle ipcVehicle : vehicles) {
        UiMode uiType = UiMode.MINOR;
        if (!vehiclesGeneratingPreds.isEmpty()
            && ipcVehicle.getId().equals(vehiclesGeneratingPreds.get(0))) uiType = UiMode.NORMAL;
   ...
   * predicted vehicle etc. If routeShortNames or stopId not specified then will return empty array.
   *
   * @param stdParameters
   * @param routesIdOrShortNames
   * @param stopId
   ...
   * @param stdParameters StdParametersBean that gets the standard parameters from the URI, query
   *     string, and headers.
   * @param routeStopStrs List of route/stops to return predictions for. If route not specified then
   *     data will be returned for all routes for the specified stop. The route specifier is the
   *     route id or the route short name. It is often best to use route short name for consistency
   *     across configuration changes (route ID is not consistent for many agencies). The stop
   *     specified can either be the stop ID or the stop code. Each route/stop is separated by the
   *     "|" character so for example the query string could have "rs=43|2029&rs=43|3029"
  ...
  public Response getPredictions(
      ...
      @Parameter(
              description =
                  "List of stops to return predictions for. Can use either stop ID or stop code.")
          @QueryParam(value = "s")
  ...
  /**
   * Handles "predictionsByLoc" command. Gets predictions from server and returns the corresponding
   * response.
   *
   * <p>A Response object is returned instead of a regular object so that can have one method for
   * the both XML and JSON yet always return the proper media type even if it is configured via the
  ...
  @Operation(
      ...
      description =
          "Gets a list of the existing routes in the server."
   ...
   *     data for all routes returned in an array. If multiple routes specified then data for those
   *     routes returned in an array.
   * @param stopId optional. If set then only this stop and the remaining ones on the trip pattern
   *     are marked as being for the UI and can be highlighted. Useful for when want to emphasize in
   *     the UI only the stops that are of interest to the user.
   * @param direction optional. If set then only the shape for specified direction is marked as
   *     being for the UI. Needed for situations where a single stop is used for both directions of
   *     a route and want to highlight in the UI only the stops and the shapes that the user is
  ...
  @Operation(
      ...
      description =
          "Provides detailed information for a route includes all stops "
  ...
  public Response getRouteDetails(
      ...
      @Parameter(
              description =
                  "If set then only the shape for specified direction is marked as being for the UI.",
              required = false)
          @QueryParam(value = "d")
          String directionId,
      @Parameter(
              description =
                  "If set then only this stop and the remaining ones on "
    ...
    try {
      ...
      if (routeIdsOrShortNames != null && routeIdsOrShortNames.size() == 1) {
        ...
        if (route == null)
          throw WebUtils.badRequestException(
              "Route for route=" + routeIdOrShortName + " does not exist.");
  ...
  @Operation(
      ...
      description =
          "Returns all stops associated with a route,"
  ...
  public Response getStops(
      ...
      @Parameter(
              description =
                  "if set, retrives only busstops belongind to the route. "
  ...
  @Operation(
      ...
      description =
          "Retrives configuration data for the specified block ID and service ID. "
  ...
  /**
   * Handles the "blocksTerse" command which outputs configuration data for the specified block ID.
   * Does not include trip pattern and schedule data for trips.
   *
   * @param stdParameters
   * @param blockId
   * @return
  ...
  @Operation(
      ...
      description =
          "Retrives configuration data for the specified block ID. It does not include trip patterns."
              + "Every trip is associated with a block.",
      tags = {"base data", "trip", "block"})
  ...
  @Operation(
      summary = "Retrives configuration data for the specified block ID",
      description =
          "Retrives configuration data for the specified block ID. Includes all sub-data such as trips and trip."
              + "Every trip is associated with a block.",
      tags = {"base data", "trip", "block"})
  ...
  @Operation(
      ...
      description =
          "Retrives a list of all blockId for the specified service ID."
              + "Every trip is associated with a block.",
      tags = {"base data", "trip", "block"})
  ...
  /**
   * Gets which blocks are active. Can optionally specify list of routes and how much before a block
   * is supposed to start is it considered active.
   *
   * @param stdParameters StdParametersBean that gets the standard parameters from the URI, query
   *     string, and headers.
   ...
   *     between the block start time minus allowableBeforeTimeSecs and the block end time. Default
   *     value for allowableBeforeTimeSecs is 0.
   * @return
   * @throws WebApplicationException
   */
  ...
  @Operation(
      ...
      description =
          "Retrives a list of active blocks. Optionally can be filered accorditn to routesIdOrShortNames params."
              + "Every trip is associated with a block.",
  ...
  public Response getActiveBlocks(
      ...
      @Parameter(
              description =
                  "if set, retrives only active blocks belongind to the route. "
                      + "It might be routeId or route shrot name.",
              required = false)
          @QueryParam(value = "r")
          List<String> routesIdOrShortNames,
      @Parameter(
              description =
                  "A block will be active if the time is between the"
  ...
  @Operation(
      ...
      description =
          "Retrives a list routes with its  active blocks. Optionally can be filered according to routesIdOrShortNames params."
              + "Every trip is associated with a block.",
  ...
  public Response getActiveBlocksByRoute(
      ...
      @Parameter(
              description =
                  "A block will be active if the time is between the block start time minus"
  ...
  @Operation(
      summary = "Gets which blocks are active by route.",
      description =
          "Retrives a list routes with its  active blocks, without the vechicles. Optionally "
  ...
  public Response getActiveBlocksByRouteWithoutVehicles(
      ...
      @Parameter(
              description =
                  "A block will be active if the time is between the block start "
    ...
    try {
      ...
      VehiclesInterface vehiclesInterface = stdParameters.getVehiclesInterface();
      Collection<IpcActiveBlock> activeBlocks =
          vehiclesInterface.getActiveBlocksWithoutVehicles(
              routesIdOrShortNames, allowableBeforeTimeSecs);
  ...
  @Operation(
      summary = "Gets which blocks are active by route.",
      description =
          "Retrives a list routes with its  active blocks, including the vechicles. "
  ...
  public Response getActiveBlockByRouteWithVehicles(
      ...
      @Parameter(
              description =
                  "if set, retrives only active blocks belongind to the route. It might be routeId or route shrot name.",
              required = false)
          @QueryParam(value = "r")
          String routesIdOrShortName,
      @Parameter(
              description =
                  "A block will be active if the time is between the block start time minus allowableBeforeTimeSecs and the block end time")
          @QueryParam(value = "t")
          @DefaultValue("0")
          int allowableBeforeTimeSecs)
      throws WebApplicationException {
    ...
    try {
      ...
      VehiclesInterface vehiclesInterface = stdParameters.getVehiclesInterface();
      Collection<IpcActiveBlock> activeBlocks =
          vehiclesInterface.getActiveBlocksAndVehiclesByRouteId(
              routesIdOrShortName, allowableBeforeTimeSecs);
  ...
  public Response getActiveBlockByRouteNameWithVehicles(
    ...
    try {
      ...
      VehiclesInterface vehiclesInterface = stdParameters.getVehiclesInterface();
      Collection<IpcActiveBlock> activeBlocks =
          vehiclesInterface.getActiveBlocksAndVehiclesByRouteName(
              routeName, allowableBeforeTimeSecs);
  ...
  @Operation(
      summary =
          "Returns the counters  for the number of current vehicles running early, late or on time. ",
      description =
          "Returns the amount of vehicles running  early, late or on time. "
  ...
  public Response getVehicleAdherenceSummary(
      ...
      @Parameter(
              description =
                  "The number of seconds early a vehicle has to be before it is considered in the early counter.",
              required = false)
      ...
      @Parameter(
              description =
                  "The number of seconds early a vehicle has to be before it is considered in the late counter.",
              required = false)
      ...
      @Parameter(
              description =
                  "A block will be active if the time is between the block start time minus allowableBeforeTimeSecs (t) and the block end time")
          @QueryParam(value = "t")
          @DefaultValue("0")
          int allowableBeforeTimeSecs)
      throws WebApplicationException {
  ...
  public Response getTrip(
      @BeanParam StandardParameters stdParameters,
      @Parameter(description = "Trip id", required = true) @QueryParam(value = "tripId")
          String tripId)
      throws WebApplicationException {
  ...
  @Operation(
      summary = "Gets the configuration data of a trip.",
      description =
          "Gets the configuration data of a trip. Includes all sub-data such as trip patterns",
      tags = {"base data", "trip"})
  ...
  @Operation(
      ...
      description =
          "Retrives a list of all trip patters for the specific routeId or routeShortName.",
      tags = {"base data", "trip"})
  ...
  @Operation(
      ...
      description =
          "Retrives schedule for the specified route.  The data is output such that the stops are listed "
  ...
  @Operation(
      ...
      description =
          "Retrives current calendar infomation. Only teh calendar that applies to current day",
  ...
  @Operation(
      ...
      tags = {"base data", "serviceId"})
  public Response getCurrentServiceIds(@BeanParam StandardParameters stdParameters)
      throws WebApplicationException {
    // Make sure request is valid
    stdParameters.validate();
  ...
  public Response getServerStatus(@BeanParam StandardParameters stdParameters)
    ...
    try {
      ...
      ApiServerStatus apiServerStatus =
          new ApiServerStatus(stdParameters.getAgencyId(), ipcServerStatus);
