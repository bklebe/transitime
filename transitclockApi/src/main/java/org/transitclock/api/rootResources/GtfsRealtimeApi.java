...
public class GtfsRealtimeApi {
  ...
  @Operation(
      ...
      description =
          "Gets real time vehicle position feed. It might be in human readeable format or binary.",
      tags = {"GTFS", "feed"})
  public Response getGtfsRealtimeVehiclePositionsFeed(
      final @BeanParam StandardParameters stdParameters,
      @Parameter(
              description =
                  "If specified as human, it will get the output in human readable format. Otherwise will output data in binary format",
              required = false)
    ...
    String mediaType =
        humanFormatOutput ? MediaType.TEXT_PLAIN : MediaType.APPLICATION_OCTET_STREAM;
