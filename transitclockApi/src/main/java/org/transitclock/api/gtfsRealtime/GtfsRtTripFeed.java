 ...
 *
 * <p>Note: for the trip feed predictions that are schedule based instead of GPS based the
...
public class GtfsRtTripFeed {
  ...
  private static IntegerConfigValue predictionMaxFutureSecs =
  ...
  private static BooleanConfigValue includeTripUpdateDelay =
  ...
  private TripUpdate createTripUpdate(List<IpcPrediction> predsForTrip) {
      ...
      try {
        if (firstPred.getFreqStartTime() > 0) {
          String tripStartTimeStr =
              gtfsRealtimeTimeFormatter.format(new Date(firstPred.getFreqStartTime()));
    ...
    if (!firstPred.isCanceled()) {
      for (IpcPrediction pred : predsForTrip) {
        StopTimeUpdate.Builder stopTimeUpdate =
          ...
          for (Long key : map.keySet()) {
            if (!map.get(key).isEmpty()) {
              FeedEntity.Builder feedEntity =
                  FeedEntity.newBuilder().setId(map.get(key).get(0).getVehicleId());
  ...
  private Map<Long, List<IpcPrediction>> createFreqStartTimePredictionMap(
  ...
  /**
   * Returns map of all predictions for the project. Returns null if there was a problem getting the
   * data via RMI. There is a separate list of predictions for each trip. The map is keyed by
   * tripId.
   *
   * @return Map keyed on tripId of List of Predictions for the trip, or null if could not get data
   *     from server.
   */
