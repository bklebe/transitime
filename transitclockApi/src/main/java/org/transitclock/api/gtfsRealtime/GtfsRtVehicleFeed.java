...
public class GtfsRtVehicleFeed {
   ...
   */
  private VehiclePosition createVehiclePosition(IpcVehicleGtfsRealtime vehicleData)
      throws ParseException {
    ...
    if (vehicleData.getRouteId() != null && vehicleData.getRouteId().length() > 0) {
      String tripStartDateStr =
          gtfsRealtimeDateFormatter.format(new Date(vehicleData.getTripStartEpochTime()));
      ...
      if (vehicleData.getFreqStartTime() > 0) {
        String tripStartTimeStr =
            gtfsRealtimeTimeFormatter.format(new Date(vehicleData.getFreqStartTime()));
  ...
  private FeedMessage createMessage(Collection<IpcVehicleGtfsRealtime> vehicles) {
    ...
    for (IpcVehicleGtfsRealtime vehicle : vehicles) {
      FeedEntity.Builder vehiclePositionEntity = FeedEntity.newBuilder().setId(vehicle.getId());
  ...
  /**
   * Returns collection of all vehicles for the project obtained via RMI. Returns null if there was
   * a problem getting the data via RMI
   *
