...
public class ApiPredictions {
  ...
  @XmlAttribute private String agencyId = null;

  // Optional additional info. Needed for when providing predictions for
  // multiple agencies, such as when getting predictions by location
  @XmlAttribute private String agencyName = null;
  ...
  /**
   * For constructing a ApiPredictions object from a List of IpcPredictionsForRouteStopDest objects.
   *
   * @param vehicles
   */
  public ApiPredictions(List<IpcPredictionsForRouteStopDest> predsForRouteStopDestinations) {
    ...
    for (IpcPredictionsForRouteStopDest predsForRouteStopDest : predsForRouteStopDestinations) {
      ...
      String currentRouteStopStr =
          predsForRouteStopDest.getRouteId() + predsForRouteStopDest.getStopId();
  ...
  /**
   * For setting info about the agency. Needed for when providing predictions for multiple agencies,
   * such as when getting predictions by location
   *
   * @param agencyId
   * @param agencyName
   */
