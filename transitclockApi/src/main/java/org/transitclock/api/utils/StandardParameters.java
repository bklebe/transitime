...
public class StandardParameters {
  ...
  /**
   * Returns the media type to use for the response based on optional accept header and the optional
   * format specification in the query string of the URL. Setting format in query string overrides
   * what is set in accept header. This way it is always simple to generate a http get for
   ...
   *
   * <p>The media type is not determined in the constructor because then an exception would cause an
   * ugly error message because it would be handled before the root-resource class get method is
   * being called.
   *
  ...
  public void validate() throws WebApplicationException {
    ...
    if (!ApiKeyManager.getInstance().isKeyValid(getKey())) {
      ...
      throw WebUtils.badRequestException(
          ...
          "Application key \"" + getKey() + "\" is not valid.");
    }
  }
  ...
  /**
   * Gets the PredictionsInterface for the agencyId specified as part of the standard parameters. If
   * not valid then throws WebApplicationException.
   *
   * @return The VehiclesInterface
   */
