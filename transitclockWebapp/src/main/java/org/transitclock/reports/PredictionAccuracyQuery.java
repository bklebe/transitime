...
public abstract class PredictionAccuracyQuery {
  ...
  public enum IntervalsType {
     ...
     * @return The corresponding IntervalsType, or IntervalsType.PERCENTAGE as the default if text
     *     doesn't match a type.
     */
   ...
   * @throws SQLException
   */
  public PredictionAccuracyQuery(
   ...
   * easily understood value, such as 1 minute. Best way to do this is then have the predictions for
   * that bucket be 45 seconds to 75 seconds so that the indicator for the bucket (1 minute) is in
  ...
  protected void doQuery(
    ...
    if ((beginTimeStr != null && !beginTimeStr.isEmpty())
      ...
      mySqlTimeSql =
          "AND CAST(arrivalDepartureTime AS TIME) BETWEEN CAST(? AS TIME) AND CAST(? AS TIME) ";
    }
    ...
    String mySql =
        "SELECT "
            ...
            + "CAST(? AS DATETIME) "
            + "AND DATE_ADD(CAST(? AS DATETIME), INTERVAL "
            + numDays
            + " day) "
            + mySqlTimeSql
            + "  AND "
            + "abs(unix_timestamp(predictedTime)-unix_timestamp(predictionReadTime)) < 900 " // 15
            // mins
    ...
    try {
      ...
      logger.debug(
          "beginDate {} beginDateStr {} endDateStr {} beginTime {} beginTimeStr {} endTime {} endTimeStr {}",
          beginDate,
          beginDateStr,
          numDays,
          beginTime,
      ...
      while (rs.next()) {
        ...
        logger.debug(
            "predLength={} predAccuracy={} source={}", predLength, predAccuracy, sourceResult);
      }
