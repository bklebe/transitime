...
public class PredAccuracyIntervalQuery extends PredictionAccuracyQuery {
   ...
   * @throws SQLException
   */
  public PredAccuracyIntervalQuery(
      String dbType, String dbHost, String dbName, String dbUserName, String dbPassword)
      throws SQLException {
   ...
   * means that want to know the minimum value such that 70% of the predictions are between the min
   * and the max, then will return the index for the item whose index is at (100%-70%)/2 = 15% in
   * the array.
   *
   * @param data Sorted list of data
   * @param percentage The percentage (0.0 - 100.0%) of prediction accuracy data that should be
  ...
  private Long getMin(List<Integer> data, double percentage) {
    if (percentage == 0.0 || Double.isNaN(percentage)) return null;
  ...
  /**
   * Goes through data array (must already be sorted!) and determines the index of the array that
   * corresponds to the maximum element. For example, if the fraction is specified as 0.70 which
   * means that want to know the minimum value such that 70% of the predictions are between the min
   * and the max, then will return the index for the item whose index is at 85% in the array.
   *
   * @param data Sorted list of data
   * @param percentage The percentage (0.0 - 100.0%) of prediction accuracy data that should be
   *     between the min and the max
  ...
  private Long getMax(List<Integer> data, double percentage) {
    if (percentage == 0.0 || Double.isNaN(percentage)) return null;
    if (percentage == 100.0) return (long) data.get(data.size() - 1);
  ...
  private void addCols(
      ChartJsonBuilder builder,
      IntervalsType intervalsType,
      double intervalPercentage1,
      double intervalPercentage2) {
    if (map.isEmpty()) {
      logger.error(
          "Called PredAccuracyIntervalQuery.getCols() but there " + "is no data in the map.");
      return;
    }
   ...
   *
   * @param builder
   * @param intervalsType Specifies whether should output for intervals standard deviation info,
   *     percentage info, or both.
   * @param intervalPercentage1 For when outputting intervals as fractions. Not used if
   *     intervalsType is STD_DEV.
   * @param intervalPercentage2 For when outputting intervals as fractions. Only used if
  ...
  private void addRows(
    ...
    if (map.isEmpty()) {
      logger.error(
          "Called PredAccuracyIntervalQuery.getRows() but there " + "is no data in the map.");
      return;
    }
    ...
    for (int predBucketIdx = 0;
      ...
      for (String source : map.keySet()) {
        ...
        logger.info(
            "For source {} for prediction bucket minute {} " + "sorted datapoints={}",
            source,
            predBucketSecs,
            listForPredBucket);
        ...
        if (listForPredBucket != null
          ...
          double stdDev = Statistics.getSampleStandardDeviation(dataForPredBucket, mean);
          ...
          if (intervalsType == IntervalsType.PERCENTAGE) {
            ...
            if (intervalPercentage2 == 0.0 || Double.isNaN(intervalPercentage2)) continue;
   ...
   * @param numDaysStr Number of days to run the query for.
   * @param beginTimeStr For specifying time of day between the begin and end date to use data for.
   ...
   *     array of route IDs.
   * @param predSource The source of the predictions. Can be null or "" (for all), "Transitime", or
   *     "Other"
   * @param predType Whether predictions are affected by wait stop. Can be "" (for all),
   *     "AffectedByWaitStop", or "NotAffectedByWaitStop".
   * @param intervalsType Specifies whether should output for intervals standard deviation info,
   *     percentage info, or both.
   * @param intervalPercentage1 For when outputting intervals as percentages. Not used if
   *     intervalsType is STD_DEV.
   * @param intervalPercentage2 For when outputting intervals as percentages. Only used if
   *     intervalsType is PERCENTAGE.
   * @return the full JSON string contain both columns and rows info, or null if no data returned
   *     from query
   * @throws SQLException
