...
public class SqlUtils {
  ...
  /**
   * To be called on request parameters to make sure they don't contain any SQL injection trickery.
   *
   * @param parameter
   ...
   * empty, signifying all routes, or can specify multiple routes. If "r" parameter not specified in
   * request then empty string is returned such that data for all routes will be retrieved. Since
   * "AND" is included in clause if it is not empty string this clause can't be put right after the
   * WHERE clause
   *
   * @param request Http request containing parameters for the query
  ...
  /**
   * Creates a SQL clause for specifying a time range. Looks at the request parameters "beginDate",
   * "numDays", "beginTime", and "endTime"
   *
   * @param request Http request containing parameters for the query
  ...
  public static String timeRangeClause(
      HttpServletRequest request, String timeColumnName, int maxNumDays) {
    ...
    if (beginTime != null && !beginTime.isEmpty() && endTime != null && !endTime.isEmpty()) {
      if (isMysql) {
        timeSql =
            " AND time(" + timeColumnName + ") BETWEEN '" + beginTime + "' AND '" + endTime + "' ";
      } else {
        timeSql =
            " AND " + timeColumnName + "::time BETWEEN '" + beginTime + "' AND '" + endTime + "' ";
      }
    }
