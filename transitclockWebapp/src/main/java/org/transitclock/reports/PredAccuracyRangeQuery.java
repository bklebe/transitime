...
public class PredAccuracyRangeQuery extends PredictionAccuracyQuery {
  ...
  private void addCols(ChartJsonBuilder builder, int maxEarlySec, int maxLateSec) {
    ...
    builder.addNumberColumn(
        "Within Bounds (" + maxEarlySec + " secs early to " + maxLateSec + " secs late)");
    builder.addTooltipColumn();
  ...
  private void addRows(ChartJsonBuilder builder, int maxEarlySec, int maxLateSec) {
    ...
    for (int predBucketIdx = 0;
      ...
      if (dataForSource != null && dataForSource.size() > predBucketIdx) {
        ...
        rowBuilder.addRowElement(
            "Within Bounds: " + ok + " points, " + StringUtils.oneDigitFormat(okPercentage) + "%");
   ...
   * @param maxEarlySec How early in msec a prediction is allowed to be. Should be a positive value.
   * @param maxLateSec How late a in msec a prediction is allowed to be. Should be a positive value.
   * @return the full JSON string contain both cols and rows info, or null if no data returned from
   *     query
   * @throws SQLException
  ...
  public static void main(String args[]) {
    ...
    try {
      PredAccuracyRangeQuery query =
          new PredAccuracyRangeQuery(dbType, dbHost, dbName, dbUserName, dbPassword);
      String jsonString =
          query.getJson(
              beginDate,
              numDays,
